...
function updateObstacles(){
    for(const ob of _danboRaceObstacles()){
        ...
        if(ob.type==='boost'){
            ...
            // Route-specific wind markers breathe and sway without changing the
            // shared boost collision area, strength or timing.
            if(ob.mesh&&ob.mesh.userData&&ob.mesh.userData._journeyBoostMotion){
                var journeyBoost=ob.mesh.userData._journeyBoostMotion;
                journeyBoost.phase+=0.026;
                ob.mesh.children.forEach(function(child,childIndex){
                    if(!child.userData||!child.userData._journeyVane)return;
                    child.rotation.z=child.userData._baseRotZ+Math.sin(journeyBoost.phase+childIndex*0.71)*0.08;
                    var breathe=1+Math.sin(journeyBoost.phase*0.83+childIndex)*0.035;
        ...
        if(ob.type==='spring'){
            ...
            if(ob.mesh&&ob.mesh.userData&&ob.mesh.userData._journeyLauncher){
                ob.data._visualPhase=(ob.data._visualPhase||0)+0.018;
                ob.mesh.children.forEach(function(child,childIndex){
                    if(!child.userData||!child.userData._journeyVane)return;
                    child.rotation.z=child.userData._baseRotZ+Math.sin(ob.data._visualPhase+childIndex*0.88)*0.045;
                });
            }
            for(const egg of allEggs){
                if(!egg.alive||egg.finished||egg.cityNPC)continue;
        ...
        if(ob.type==='seedCrawler'){
            ...
            for(const egg of allEggs){
                ...
                if(dist<ob.data.radius+egg.radius){
                    // If the player is above the seed crawler, stomp it and bounce.
...
function updateCityNPC(egg){if(egg.heldBy)return;
    ...
    if(egg._aiStateTimer<=0){
        ...
        else if(r<0.93) egg._aiState='trailBurst';
    ...
    if(st==='wander'){
        ...
        if(closest){
            ...
            // NPC close combat: light hitstun or knockdown
            ...
            var _npcTravelerId=egg.mesh.userData._travelerId;
            if(egg._npcProjectileCD>0)egg._npcProjectileCD--;
            if(egg._npcSpecialCD>0)egg._npcSpecialCD--;

            // 蜜蕊旅人: 微弱蛋拳 / 蜜蕊旅人拳 / 百花拳击
            if(_npcTravelerId==='blossomTraveler'){
                if(cd2>3&&cd2<25&&Math.random()<0.06&&(!egg._npcProjectileCD||egg._npcProjectileCD<=0)){
                    egg._npcProjectileCD=50;var _nhDir=Math.atan2(cdx2,cdz2);
                    MoveProjectile_execute(egg,_nhDir,{speed:MOVE_PARAMS.blossomTraveler.weakPunch.speed,life:MOVE_PARAMS.blossomTraveler.weakPunch.life,color:MOVE_PARAMS.blossomTraveler.weakPunch.color,ringColor:MOVE_PARAMS.blossomTraveler.weakPunch.ringColor,burns:MOVE_PARAMS.blossomTraveler.weakPunch.burns,isPlayer:false,type:'normal'});
                    _shoutMove(egg,MOVE_PARAMS.blossomTraveler.weakPunch);
                }
                if(cd2<2.5&&egg.onGround&&Math.random()<0.024&&!egg._npcCanopyLiftActive&&(!egg._npcSpecialCD||egg._npcSpecialCD<=0)){
                    egg._npcSpecialCD=60;egg._npcCanopyLiftActive=true;
                    MoveCanopyLift_execute(egg,Math.atan2(cdx2,cdz2),{duration:MOVE_PARAMS.blossomTraveler.canopyLift.duration,jumpMul:1.5,fwdSpeed:MOVE_PARAMS.blossomTraveler.canopyLift.fwdSpeed});
                    _shoutMove(egg,MOVE_PARAMS.blossomTraveler.canopyLift);
                    // NPC immediate hit on nearby target
                    if(cd2<3){closest.vx+=(cdx2/cd2)*0.3;closest.vz+=(cdz2/cd2)*0.3;closest.vy=COMBAT.spin.vy;closest.squash=COMBAT.spin.squash;closest.throwTimer=COMBAT.npcBodySlam.throwTimer;closest._bounces=COMBAT.spin.bounces;_addStunDamage(closest,COMBAT.canopyLift.stunDmg);_dropNpcStolenCoins(closest);if(closest.isPlayer)playHitSound(egg.mesh.position.x,egg.mesh.position.z);}
                }
                if(cd2>2&&cd2<8&&egg.onGround&&Math.random()<0.024&&!egg._npcTrailSweepActive&&(!egg._npcSpecialCD||egg._npcSpecialCD<=0)){
                    egg._npcSpecialCD=60;egg._npcTrailSweepActive=42;egg._npcTrailSweepDir=Math.atan2(cdx2,cdz2);MoveSpin_execute(egg,egg._npcTrailSweepDir,{duration:42});
                    _shoutMove(egg,MOVE_PARAMS.blossomTraveler.trailSweep);
                }
            }
            // Fruitbrew Traveler: seed cast, orchard lift, crescent step
            else if(_npcTravelerId==='fruitbrewTraveler'){
                if(cd2>3&&cd2<25&&Math.random()<0.06&&(!egg._npcProjectileCD||egg._npcProjectileCD<=0)){
                    egg._npcProjectileCD=50;var _nkDir=Math.atan2(cdx2,cdz2);
                    MoveProjectile_execute(egg,_nkDir,{speed:MOVE_PARAMS.fruitbrewTraveler.seedCast.speed,life:MOVE_PARAMS.fruitbrewTraveler.seedCast.life,color:MOVE_PARAMS.fruitbrewTraveler.seedCast.color,ringColor:MOVE_PARAMS.fruitbrewTraveler.seedCast.ringColor,burns:MOVE_PARAMS.fruitbrewTraveler.seedCast.burns,isPlayer:false,type:'normal'});
                    _shoutMove(egg,MOVE_PARAMS.fruitbrewTraveler.seedCast);
                }
                if(cd2<2.5&&egg.onGround&&Math.random()<0.024&&!egg._npcCanopyLiftActive&&(!egg._npcSpecialCD||egg._npcSpecialCD<=0)){
                    egg._npcSpecialCD=60;egg._npcCanopyLiftActive=true;
                    MoveCanopyLift_execute(egg,Math.atan2(cdx2,cdz2),{duration:MOVE_PARAMS.fruitbrewTraveler.canopyLift.duration,jumpMul:1.7,fwdSpeed:MOVE_PARAMS.fruitbrewTraveler.canopyLift.fwdSpeed});
                    _shoutMove(egg,MOVE_PARAMS.fruitbrewTraveler.canopyLift);
                ...
                if(cd2>2&&cd2<8&&egg.onGround&&Math.random()<0.024&&!egg._npcTrailSweepActive&&(!egg._npcSpecialCD||egg._npcSpecialCD<=0)){
                    egg._npcSpecialCD=60;egg._npcTrailSweepActive=42;egg._npcTrailSweepDir=Math.atan2(cdx2,cdz2);MoveSpin_execute(egg,egg._npcTrailSweepDir,{duration:42});
                    _shoutMove(egg,MOVE_PARAMS.fruitbrewTraveler.trailSweep);
                }
            }
            // Herb Traveler: leaf flurry and forest sway
            else if(_npcTravelerId==='herbTraveler'){
                if(cd2<2.5&&Math.random()<0.06&&(!egg._npcSpecialCD||egg._npcSpecialCD<=0)){
                    egg._npcSpecialCD=30;_shoutMove(egg,MOVE_PARAMS.herbTraveler.leafFlurry);
                ...
                if(cd2>3&&cd2<10&&egg.onGround&&Math.random()<0.024&&(!egg._npcSpecialCD||egg._npcSpecialCD<=0)){
                    ...
                    MoveDash_execute(egg,_nhbDir,{isDash:true,speed:2,duration:40});_shoutMove(egg,MOVE_PARAMS.herbTraveler.herbDash);
                    egg.mesh.rotation.y=_nhbDir;
                }
            }
            // Salt Crystal Traveler: prism pulse and crystal glide
            else if(_npcTravelerId==='saltCrystalTraveler'){
                if(cd2<2.5&&Math.random()<0.03&&(!egg._npcSpecialCD||egg._npcSpecialCD<=0)){
                    egg._npcSpecialCD=60;_shoutMove(egg,MOVE_PARAMS.saltCrystalTraveler.electric);
                    for(var _cpa=0;_cpa<4;_cpa++)MoveProjectile_execute(egg,_cpa*Math.PI/2,{speed:0.12,life:18,color:0xBFE8DE,ringColor:0xF3E7BC,isPlayer:false,type:'normal',npcRadius:0.24});
                    if(cd2<2){closest.vx+=(cdx2/(cd2||1))*0.16;closest.vz+=(cdz2/(cd2||1))*0.16;closest._hitStun=12;if(closest.isPlayer)playHitSound(egg.mesh.position.x,egg.mesh.position.z);}
                }
                if(cd2>4&&cd2<12&&egg.onGround&&Math.random()<0.018&&(!egg._npcSpecialCD||egg._npcSpecialCD<=0)){
                    ...
                    egg._npcTravelGlide=30;egg._npcTravelGlideX=Math.sin(_nbrDir)*MAX_SPEED*1.35;egg._npcTravelGlideZ=Math.cos(_nbrDir)*MAX_SPEED*1.35;
                    egg.vx=egg._npcTravelGlideX;egg.vz=egg._npcTravelGlideZ;egg.mesh.rotation.y=_nbrDir;_shoutMove(egg,MOVE_PARAMS.saltCrystalTraveler.crystalGlide);
                }
            }
            // Cloudwing Traveler: wind ribbon and cloud vault
            else if(_npcTravelerId==='cloudwingTraveler'){
                if(cd2>4&&cd2<20&&Math.random()<0.045&&(!egg._npcProjectileCD||egg._npcProjectileCD<=0)){
                    egg._npcProjectileCD=60;var _nsbDir=Math.atan2(cdx2,cdz2);
                    MoveProjectile_execute(egg,_nsbDir,{speed:MOVE_PARAMS.cloudwingTraveler.windRibbon.speed,life:MOVE_PARAMS.cloudwingTraveler.windRibbon.life,color:MOVE_PARAMS.cloudwingTraveler.windRibbon.color,ringColor:MOVE_PARAMS.cloudwingTraveler.windRibbon.ringColor,isPlayer:false,type:'windRibbon'});
                    _shoutMove(egg,MOVE_PARAMS.cloudwingTraveler.windRibbon);
                }
                if(cd2<3&&egg.onGround&&Math.random()<0.024&&!egg._npcCanopyLiftActive&&(!egg._npcSpecialCD||egg._npcSpecialCD<=0)){
                    egg._npcSpecialCD=60;egg._npcCanopyLiftActive=true;MoveCanopyLift_execute(egg,Math.atan2(cdx2,cdz2),{duration:40,jumpMul:1.18,fwdSpeed:0.12});_shoutMove(egg,MOVE_PARAMS.cloudwingTraveler.cloudVault);
                    if(cd2<3){closest.vx+=(cdx2/cd2)*0.4;closest.vz+=(cdz2/cd2)*0.4;closest.vy=0.3;closest.squash=COMBAT.punch.squash;closest.throwTimer=35;closest._bounces=COMBAT.punch.bounces;_addStunDamage(closest,COMBAT.cloudVault.stunDmg);_dropNpcStolenCoins(closest);if(closest.isPlayer)playHitSound(egg.mesh.position.x,egg.mesh.position.z);}
                }
            }
            // Berry Traveler: berry pod, vine taps and twilight sweep
            else if(_npcTravelerId==='berryTraveler'){
                if(cd2>3&&cd2<20&&Math.random()<0.045&&(!egg._npcProjectileCD||egg._npcProjectileCD<=0)){
                    egg._npcProjectileCD=50;var _ncDir=Math.atan2(cdx2,cdz2);
                    MoveProjectile_execute(egg,_ncDir,{speed:MOVE_PARAMS.berryTraveler.berryCast.speed,life:MOVE_PARAMS.berryTraveler.berryCast.life,color:MOVE_PARAMS.berryTraveler.berryCast.color,ringColor:MOVE_PARAMS.berryTraveler.berryCast.ringColor,isPlayer:false,type:'normal',npcRadius:0.5});
                    _shoutMove(egg,MOVE_PARAMS.berryTraveler.berryCast);
                }
                if(cd2<2.5&&Math.random()<0.045&&(!egg._npcSpecialCD||egg._npcSpecialCD<=0)){
                    egg._npcSpecialCD=30;_shoutMove(egg,MOVE_PARAMS.berryTraveler.berryFlurry);
                    if(cd2<2.5){closest.vx+=(cdx2/(cd2||1))*COMBAT.rapidHit.force;closest.vz+=(cdz2/(cd2||1))*COMBAT.rapidHit.force;closest.vy=COMBAT.rapidHit.vy;closest.squash=COMBAT.rapidHit.squash;closest.throwTimer=COMBAT.rapidHit.throwTimer;closest._bounces=COMBAT.rapidHit.bounces;_addStunDamage(closest,COMBAT.rapidHit.stunDmg);_dropNpcStolenCoins(closest);if(closest.isPlayer)playHitSound(egg.mesh.position.x,egg.mesh.position.z);}
                }
                if(cd2>2&&cd2<8&&egg.onGround&&Math.random()<0.018&&!egg._npcTrailSweepActive&&(!egg._npcSpecialCD||egg._npcSpecialCD<=0)){
                    egg._npcSpecialCD=60;egg._npcTrailSweepActive=42;egg._npcTrailSweepDir=Math.atan2(cdx2,cdz2);MoveSpin_execute(egg,egg._npcTrailSweepDir,{duration:42});egg.vy=JUMP_FORCE*0.65;
                    _shoutMove(egg,MOVE_PARAMS.berryTraveler.twilightSweep);
                }
            }
            // Spicy Flame Traveler: close spice gust
            else if(_npcTravelerId==='spicyFlameTraveler'){
                if(cd2<4&&Math.random()<0.024&&!egg._npcTrailSweepActive&&(!egg._npcSpecialCD||egg._npcSpecialCD<=0)){
                    egg._npcSpecialCD=60;egg._npcTrailSweepActive=38;egg._npcTrailSweepDir=Math.atan2(cdx2,cdz2);MoveSpin_execute(egg,egg._npcTrailSweepDir,{duration:38,isSpiceGust:true});
                    _shoutMove(egg,MOVE_PARAMS.spicyFlameTraveler.spiceGust);
                }
                // Normal grab/throw AI remains available through the shared interaction system.
            }
            // Golden Grain Traveler: seed pod and harvest fan
            else if(_npcTravelerId==='goldenGrainTraveler'){
                if(cd2>3&&cd2<25&&Math.random()<0.06&&(!egg._npcProjectileCD||egg._npcProjectileCD<=0)){
                    egg._npcProjectileCD=50;var _ndDir=Math.atan2(cdx2,cdz2);
                    MoveProjectile_execute(egg,_ndDir,{speed:MOVE_PARAMS.goldenGrainTraveler.grainPod.speed,life:MOVE_PARAMS.goldenGrainTraveler.grainPod.life,color:MOVE_PARAMS.goldenGrainTraveler.grainPod.color,ringColor:MOVE_PARAMS.goldenGrainTraveler.grainPod.ringColor,burns:MOVE_PARAMS.goldenGrainTraveler.grainPod.burns,isPlayer:false,type:'grainPod'});
                    _shoutMove(egg,MOVE_PARAMS.goldenGrainTraveler.grainPod);
                }
                if(cd2<5&&Math.random()<0.018&&(!egg._npcSpecialCD||egg._npcSpecialCD<=0)){
                    egg._npcSpecialCD=70;_shoutMove(egg,MOVE_PARAMS.goldenGrainTraveler.harvestFan);
                    if(cd2<4){var _fanDir=Math.atan2(cdx2,cdz2);for(var _fpi=-2;_fpi<=2;_fpi++)MoveProjectile_execute(egg,_fanDir+_fpi*0.13,{speed:0.12,life:16,color:0xD9B85F,ringColor:0x8FAF55,isPlayer:false,type:'grainPod',npcRadius:0.22});closest.vx+=(cdx2/(cd2||1))*0.14;closest.vz+=(cdz2/(cd2||1))*0.14;closest._hitStun=12;_addStunDamage(closest,COMBAT.harvestFan.stunDmg);if(closest.isPlayer)playHitSound(egg.mesh.position.x,egg.mesh.position.z);}
                }
            }
            // Legacy close-grab state remains for compatibility; it is not newly triggered
        ...
        if(_babylonTower){
            ...
            } else if(bt.elevatorEnabled!==false) {
                // Near tower — teleport to cloud world or back
                if(egg.mesh.position.y<(bt.baseY||0)+5&&Math.random()<0.01){
                    ...
                    // Come back down
                    egg.mesh.position.y=(bt.baseY||0)+1;egg.vy=0;
                    egg._aiStateTimer=10;
                }
            }
        }
    } else if(st==='trailBurst'){
    ...
    // ---- NPC canopy-lift animation ----
    if(egg._npcCanopyLiftActive){
        if(egg._canopyLiftActive>0)MoveCanopyLift_update(egg);
        if(egg._canopyLiftActive<=0||egg.vy<=0)egg._npcCanopyLiftActive=false;
    }
    // ---- NPC trail-sweep animation ----
    if(egg._npcTrailSweepActive>0){
        egg._npcTrailSweepActive--;
        if(egg._trailSweepActive>0)MoveSpin_update(egg,null);
        if(egg._npcTrailSweepActive<=0){MoveSpin_end(egg);egg.vx*=0.3;egg.vz*=0.3;}
    }
    // ---- NPC salt-prism glide ----
    if(egg._npcTravelGlide>0){egg._npcTravelGlide--;egg.vx=egg._npcTravelGlideX;egg.vz=egg._npcTravelGlideZ;egg.mesh.rotation.z=Math.sin(egg._npcTravelGlide*0.2)*0.05;if(egg._npcTravelGlide<=0){egg.vx*=0.3;egg.vz*=0.3;egg.mesh.rotation.z=0;}}
    // ---- NPC herb trail step ----
    if(egg._travelDashTimer>0){egg._travelDashTimer--;egg.vx=egg._dashDirX||egg.vx;egg.vz=egg._dashDirZ||egg.vz;egg.mesh.rotation.x=0;egg.mesh.rotation.z=Math.sin(egg._travelDashTimer*0.25)*0.04;if(egg._travelDashTimer<=0){egg.vx*=0.3;egg.vz*=0.3;egg.mesh.rotation.z=0;egg._dashDirX=undefined;egg._dashDirZ=undefined;}}
...
function _dropNpcStolenCoins(egg){
    ...
    for(var di=0;di<egg._stolenCoins.length;di++){
        ...
        if(coinIdx>=0&&coinIdx<cityCoins.length){
            ...
            // Radial scatter: coins fly outward in arcs
...
function updateRaceAI(egg){
    ...
    for(var oi=0;oi<_danboRaceObstacles().length;oi++){
        ...
        if(ob.type==='waystone'&&dz<4&&DANBO_WASM.absDeltaLess(egg.mesh.position.x,(ob.data.x||0),2))
            egg.vx+=(egg.mesh.position.x>(ob.data.x||0)?1:-1)*MOVE_ACCEL*avoidStr*1.5;
        if(ob.type==='seedCrawler'&&dz<3&&!ob.data._squashed){
