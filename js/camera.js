...
function updateCamera(){
    ...
    // Give map-editor tower previews a short, deterministic establishing shot.
    // It automatically releases back to the normal playable camera.
    var _editorPreview=window.DANBO_MAP_EDITOR_GAME_PREVIEW;
    if(gameState==='city'&&_editorPreview&&_editorPreview.focus==='babel-tower'&&_editorPreview.cameraTarget&&Date.now()<_editorPreview.cameraUntil){
        var _editorTarget=_editorPreview.cameraTarget;
        camera.position.set(_editorTarget.x+42,_editorTarget.y+25,_editorTarget.z+58);
        camera.lookAt(_editorTarget.x,_editorTarget.y,_editorTarget.z);
        sun.position.set(_editorTarget.x+RENDER_CONFIG.sunPos.x,RENDER_CONFIG.sunPos.y,_editorTarget.z+RENDER_CONFIG.sunPos.z);
        sun.target.position.set(_editorTarget.x,0,_editorTarget.z);
        _sunMesh.position.set(_editorTarget.x+RENDER_CONFIG.sunPos.x*3,RENDER_CONFIG.sunPos.y*3,_editorTarget.z+RENDER_CONFIG.sunPos.z*3);
    ...
    // Third-person over-the-shoulder camera — over-the-shoulder
    if(_tpsCamMode){
        ...
        var _inMove=!!(playerEgg._trailSweepActive||playerEgg._canopyLiftActive||playerEgg._herbDashTimer||playerEgg._crystalGlideTimer||playerEgg._crystalGlideFalling||playerEgg._cloudVaultTimer||playerEgg._harvestFan);
