...
// A map-editor game preview may request the city being authored before main.js
// builds the initial scene. These values are preview-only and never change the
// saved city definition or normal game progression.
(function(){
    var params;
    try{params=new URLSearchParams(location.search);}catch(e){return;}
    if(!params.get('mapEditorPreview'))return;
    var city=Math.max(0,Math.min(7,Math.round(Number(params.get('city'))||0)));
    var state=params.get('previewState')||'placement';
    if(['placement','unlock-event','normal'].indexOf(state)<0)state='placement';
    currentCityStyle=city;
    window.DANBO_MAP_EDITOR_GAME_PREVIEW={
        active:true,
        city:city,
        state:state,
        focus:params.get('focus')||'',
        token:params.get('mapEditorPreview')||''
    };
})();
...
function _decorateDefaultBuilding(b,bMeshes,col,st,i){
    ...
    if(i%2===0){
        var sNames=['TRAVEL','SHOP','CAFE','HOTEL','STAR','TOY'];
...
function _decorateHopePremiumBuilding(b,bMeshes,col,i){
    ...
    var stoneDark=_citySharedPBR('trim-stone-dark',0xAA9C87,{envMapIntensity:0.14});
    ...
    var lowerMat=_citySharedPBR('hope-ground-course-'+archetype,lowerColor,{envMapIntensity:0.13});
...
function _buildHopeCinematicPlaza(){
    var high=window.DANBO_VISUAL_QUALITY&&DANBO_VISUAL_QUALITY.high;
    var paving=_visualSurfaceMaterial('path',0xA89A84,{normalScale:new THREE.Vector2(0.72,0.72),envMapIntensity:0.34});
    var edge=_visualSurfaceMaterial('stone',0xC7BBA6,{roughness:0.76,normalScale:new THREE.Vector2(0.45,0.45)});
    var garden=_visualSurfaceMaterial('grass',0x326B39,{normalScale:new THREE.Vector2(0.55,0.55)});
    ...
    var shrubCount=high?56:24,shrubMat=softPBR(0x4E8D43,{envMapIntensity:0.20}),shrubs=new THREE.InstancedMesh(new THREE.IcosahedronGeometry(0.48,1),shrubMat,shrubCount),hd=new THREE.Object3D();
    ...
    var trunkMat=_visualSurfaceMaterial('bark',0x6C4933,{normalScale:new THREE.Vector2(0.45,0.45)});
...
function buildCity() {
    ...
    // Earthy ground patches — brown soil showing through the snow cover
    ...
    if(currentCityStyle===0&&typeof _visualScaleUVToMeters==='function')_visualScaleUVToMeters(groundGeo,CITY_SIZE*2,CITY_SIZE*2,2.07);
    ...
    // Cherry valley district — high terrain, a deep gorge and riverside lodges
    ...
    cityColliders.push({x:68,z:0,hw:60,hd:140,h:_pH});
    // === Paths on plateaus ===
    // Paths between trees and traveler lodges
    ...
    buildings.forEach((b,i)=>{
        ...
        const bodyMat=(currentCityStyle===0&&typeof _visualSurfaceMaterial==='function')?_visualSurfaceMaterial('facade',_hopeFacadeColor,{roughness:0.84,bumpScale:0.020,envMapIntensity:0.18,vertexColors:true}):toon(col);
        ...
        if(currentCityStyle===0&&typeof _visualBoxWorldUV==='function')_visualBoxWorldUV(_bodyGeo,3.0);
        if(currentCityStyle===0&&typeof _visualAddVerticalDirtVertexColors==='function')_visualAddVerticalDirtVertexColors(_bodyGeo,0.18);
        ...
        if(currentCityStyle===0){
            ...
            var _corniceMat=_citySharedPBR('cornice',_corniceColor,{envMapIntensity:0.18});
        ...
        // Roof — layered timber profile for the cherry valley district
        if(currentCityStyle===6){
        ...
        var _windowFrameMat=currentCityStyle===0?_citySharedPBR('window-frame',0xE6E0D5,{pastelAmount:0.02,envMapIntensity:0.16}):null;
    ...
    // Explicit tree instances are authored by the map editor.  treeCount stays
    // as the legacy/random layer so existing maps keep working until they are
    // migrated.  A city may intentionally combine both layers.
    var _treeDefinitions=cityFlora&&Array.isArray(cityFlora.trees)?cityFlora.trees:[];
    var _randomTreeCount=(cityFlora&&cityFlora.treeCount!==undefined)?cityFlora.treeCount:((cityLayout&&cityLayout.treeCount!==undefined)?cityLayout.treeCount:(currentCityStyle===6?40:80)); // sakura: fewer random trees (river trees are separate)
    var _treeCount=_treeDefinitions.length+Math.max(0,Number(_randomTreeCount)||0);
    for(let i=0;i<_treeCount;i++){
        var _treeDefinition=i<_treeDefinitions.length?_treeDefinitions[i]:null;
        var tx,tz;
        if(_treeDefinition){
            tx=Number(_treeDefinition.x)||0;tz=Number(_treeDefinition.z)||0;
        ...
        if(skip) continue;
        const tg=new THREE.Group(); tg.position.set(tx,_treeDefinition?(Number(_treeDefinition.y)||0):(currentCityStyle===6?8:0),tz);
        if(currentCityStyle===6){
        ...
        var _trunkMat=_hopeTree&&typeof _visualSurfaceMaterial==='function'?_visualSurfaceMaterial('bark',0x765039,{bumpScale:0.13}):toon(0x8B5E3C);
        ...
        if(_treeDefinition){
            _treeTier=_treeDefinition.tier||_treeDefinition.kind||'authored';
            _treeScale=Math.max(0.35,Math.min(2.25,Number(_treeDefinition.scale)||1));
        }else if(currentCityStyle===6){
        ...
        var _treeWidthVar=_treeDefinition?Math.max(0.65,Math.min(1.55,Number(_treeDefinition.scaleX)||1)):((_treeTier==='sapling'?0.78:(_treeTier==='young'?0.88:(_treeTier==='landmark'?1.04:0.94)))+Math.random()*0.18);
        var _treeDepthVar=_treeDefinition?Math.max(0.65,Math.min(1.55,Number(_treeDefinition.scaleZ)||1)):((_treeTier==='sapling'?0.82:(_treeTier==='young'?0.90:(_treeTier==='landmark'?1.02:0.94)))+Math.random()*0.16);
        var _treeHeightVar=_treeDefinition?Math.max(0.65,Math.min(1.55,Number(_treeDefinition.scaleY)||1)):((_treeTier==='sapling'?0.94:(_treeTier==='young'?0.96:0.98))+Math.random()*0.16);
        tg.scale.set(_treeScale*_treeWidthVar,_treeScale*_treeHeightVar,_treeScale*_treeDepthVar);
        tg.rotation.y=_treeDefinition?(Number(_treeDefinition.rotationY)||0)*Math.PI/180:Math.random()*Math.PI*2;
        if(_treeDefinition){
            tg.name='danbo-editor-tree-'+i;
            tg.userData.editorTreeIndex=i;
            tg.userData.editorTreeId=_treeDefinition.id||('tree-'+i);
        }
        ...
        cityGroup.add(tg);
        cityProps.push({group:tg, x:tx, z:tz, radius:TREE_CONFIG.collisionRadius*_treeScale*Math.max(_treeWidthVar,_treeDepthVar), type:'tree', editorTreeIndex:_treeDefinition?i:undefined, grabbed:false, origY:tg.position.y, throwVx:0, throwVy:0, throwVz:0, throwTimer:0, weight:TREE_CONFIG.weight});
    }

// ---- Tiered civic wishing fountain — skipped in the cherry valley district ----
    ...
    var stoneD=currentCityStyle===0&&typeof _visualSurfaceMaterial==='function'?_visualSurfaceMaterial('stone',0x858982,{bumpScale:0.050,envMapIntensity:0.12}):toon(0xAA9988);
    var marbleM=currentCityStyle===0&&typeof _visualSurfaceMaterial==='function'?_visualSurfaceMaterial('stone',0xDDD6C9,{roughness:0.57,bumpScale:0.022,clearcoat:0.08,clearcoatRoughness:0.74,envMapIntensity:0.32}):toon(0xEEE8DD);
    ...
    // Pool floor
    var poolFloor=new THREE.Mesh(new THREE.CylinderGeometry(6.5,6.5,0.15,currentCityStyle===0?40:24),currentCityStyle===0&&typeof _visualSurfaceMaterial==='function'?_visualSurfaceMaterial('stone',0x6C9FA6,{bumpScale:0.045,envMapIntensity:0.10}):toon(0x88BBCC));
    ...
    if(currentCityStyle===0){
        ...
        // Traditional wooden water wheels built from a rim, spokes and paddles.
        window._waterWheels=[];
    ...
    var _rabbitFur=softPBR(0xE9D8C8,{pastelAmount:0.05,envMapIntensity:0.12});
    ...
    var _deerAntler=softPBR(0x76513A,{pastelAmount:0.02,envMapIntensity:0.08});
    ...
    if(currentCityStyle===6){try{
        ...
        // Helper: build a compact timber lodge with roof, porch, windows and collider
        function _buildTimberLodge(x,z,w,d,h,wallColor){
        ...
        // Helper: build an elevated timber lodge on the plateau
        function _buildElevatedTimberLodge(x,z,w,d,h,wallColor,baseY,faceDir){
        ...
        // === 1. Mixed traveler lodges, shops and cafes ===
        var _leftBlds=[
            // Left bank: one line (x=-25), alternating traveler lodges and shops
            {x:-30,z:-100,w:13,d:14,h:15,c:0,face:1},       // tall valley lodge
            {x:-30,z:-85,w:10,d:11,h:8,c:0xEEDDBB,face:1},  // warm timber shop
            {x:-30,z:-70,w:13,d:14,h:14,c:0,face:1},        // stepped valley lodge
            {x:-30,z:-55,w:10,d:11,h:8,c:0xDDCCAA,face:1},  // lane shop
            // z=-40: the valley traveler rest hall is built separately
            {x:-30,z:-20,w:10,d:11,h:8,c:0xCCBB99,face:1},  // tea room
            {x:-30,z:-5,w:13,d:14,h:13,c:0,face:1},         // overlook lodge
            {x:-30,z:10,w:10,d:11,h:8,c:0xEECCBB,face:1},   // food shop
            {x:-30,z:25,w:13,d:14,h:15,c:0,face:1},         // riverside lodge
            {x:-30,z:40,w:10,d:11,h:8,c:0xDDBBAA,face:1},   // keepsake hall
            {x:-30,z:55,w:13,d:14,h:12,c:0,face:1},         // lantern lodge
            {x:-30,z:70,w:10,d:11,h:8,c:0xEEDDCC,face:1},   // meal shop
            {x:-30,z:85,w:13,d:14,h:14,c:0,face:1},         // northern lodge
            {x:-30,z:100,w:10,d:11,h:8,c:0xCCBBAA,face:1}   // sweets shop
        ];
        var _rightBlds=[
            {x:30,z:-95,w:13,d:14,h:14,c:0,face:-1},        // south valley lodge
            {x:30,z:-80,w:10,d:11,h:8,c:0xDDCCAA,face:-1},  // noodle shop
            {x:30,z:-65,w:13,d:14,h:13,c:0,face:-1},        // gorge lodge
            {x:30,z:-50,w:10,d:11,h:8,c:0xEECCBB,face:-1},  // snack shop
            {x:30,z:-35,w:13,d:14,h:15,c:0,face:-1},        // riverside lodge
            {x:30,z:-20,w:10,d:11,h:8,c:0xCCBB99,face:-1},  // waterside cafe
            {x:30,z:-5,w:13,d:14,h:12,c:0,face:-1},         // garden lodge
            {x:30,z:10,w:10,d:11,h:8,c:0xDDBBAA,face:-1},   // dumpling shop
            {x:30,z:25,w:13,d:14,h:15,c:0,face:-1},         // valley bathhouse
            {x:30,z:40,w:10,d:11,h:8,c:0xEEDDBB,face:-1},   // preserved-food shop
            {x:30,z:55,w:13,d:14,h:13,c:0,face:-1},         // spring lodge
            {x:30,z:70,w:10,d:11,h:8,c:0xDDCCAA,face:-1},   // dessert shop
            {x:30,z:85,w:13,d:14,h:14,c:0,face:-1},         // hill lodge
            {x:30,z:100,w:10,d:11,h:8,c:0xCCBBAA,face:-1}   // warm drink shop
        ];
        ...
        for(var _jbi=0;_jbi<_allJpnB.length;_jbi++){
            ...
            _buildElevatedTimberLodge(jb.x,jb.z,jb.w,jb.d,jb.h,jb.c,_pH,jb.face||0);
        }

        // === 2. Valley traveler rest hall — low, garden-like hot-spring lodge ===
        // The asymmetrical terraces, broad pavilion roofs and ground-level lighting give
        // this landmark its own quiet rest-stop silhouette instead of a tall stacked facade.
        var _svX=-30,_svZ=-40,_svY=_pH,_svMeshes=[];
        var _svCream=toon(0xE9DFC8),_svSage=toon(0x78947B),_svWood=toon(0x59483A);
        var _svRoof=toon(0x3F4B4A),_svStone=toon(0x8B8E82),_svGlow=toon(0xFFD99A,{emissive:0xE6A85B,emissiveIntensity:0.42});
        function _svAdd(mesh){mesh.castShadow=true;mesh.receiveShadow=true;cityGroup.add(mesh);_svMeshes.push(mesh);return mesh;}
        // A broad stone terrace anchors two offset pavilions and keeps the overall mass low.
        var _svBase=_svAdd(new THREE.Mesh(new THREE.BoxGeometry(20,0.65,15),_svStone));
        _svBase.name='sakura-traveler-rest-hall-base';_svBase.position.set(_svX,_svY+0.32,_svZ);
        var _svMain=_svAdd(new THREE.Mesh(new THREE.BoxGeometry(17.5,5.2,12.5),_svCream));
        _svMain.name='sakura-traveler-rest-hall-main';_svMain.position.set(_svX,_svY+3.25,_svZ);
        var _svUpper=_svAdd(new THREE.Mesh(new THREE.BoxGeometry(10.5,3.7,8.2),_svSage));
        _svUpper.name='sakura-traveler-rest-hall-upper';_svUpper.position.set(_svX-2.6,_svY+7.65,_svZ-0.8);
        var _svEntry=_svAdd(new THREE.Mesh(new THREE.BoxGeometry(6.2,3.2,4.8),_svWood));
        _svEntry.position.set(_svX+4.6,_svY+2.15,_svZ+6.6);
        // Wide faceted pavilion roofs replace repeated floor bands and a single tall cap.
        var _svMainRoof=_svAdd(new THREE.Mesh(new THREE.ConeGeometry(1,2.25,4),_svRoof));
        _svMainRoof.position.set(_svX,_svY+6.6,_svZ);_svMainRoof.rotation.y=Math.PI/4;_svMainRoof.scale.set(10.8,1,8.0);
        var _svUpperRoof=_svAdd(new THREE.Mesh(new THREE.ConeGeometry(1,1.65,4),_svRoof));
        _svUpperRoof.position.set(_svX-2.6,_svY+10.25,_svZ-0.8);_svUpperRoof.rotation.y=Math.PI/4;_svUpperRoof.scale.set(6.8,1,5.4);
        var _svEntryRoof=_svAdd(new THREE.Mesh(new THREE.ConeGeometry(1,1.0,4),toon(0x65736C)));
        _svEntryRoof.position.set(_svX+4.6,_svY+4.2,_svZ+6.6);_svEntryRoof.rotation.y=Math.PI/4;_svEntryRoof.scale.set(4.2,1,3.4);
        // Irregular timber bays and warm windows create a horizontal, restful facade rhythm.
        var _svBayX=[-6.2,-3.5,-0.2,3.4,6.1],_svBayW=[1.8,2.2,1.6,2.4,1.5];
        for(var _svi=0;_svi<_svBayX.length;_svi++){
            var _svWin=_svAdd(new THREE.Mesh(new THREE.BoxGeometry(_svBayW[_svi],2.15,0.12),_svGlow));
            _svWin.position.set(_svX+_svBayX[_svi],_svY+3.15,_svZ+6.31);
            var _svSill=_svAdd(new THREE.Mesh(new THREE.BoxGeometry(_svBayW[_svi]+0.35,0.14,0.38),_svWood));
            _svSill.position.set(_svX+_svBayX[_svi],_svY+1.98,_svZ+6.45);
        }
        // A side terrace and planted screen connect the lodge to the valley path.
        var _svDeck=_svAdd(new THREE.Mesh(new THREE.BoxGeometry(7.2,0.35,4.0),_svWood));
        _svDeck.position.set(_svX-9.2,_svY+1.05,_svZ+2.0);
        for(var _svp=0;_svp<4;_svp++){
            var _svPost=_svAdd(new THREE.Mesh(new THREE.CylinderGeometry(0.13,0.16,2.7,6),_svWood));
            _svPost.position.set(_svX-11.6+_svp*1.6,_svY+2.4,_svZ+3.65);
        }
        var _svPlanter=_svAdd(new THREE.Mesh(new THREE.BoxGeometry(5.8,0.55,0.8),toon(0x75624C)));
        _svPlanter.position.set(_svX-2.2,_svY+6.25,_svZ+4.55);
        for(var _svb=0;_svb<5;_svb++){
            var _svBush=_svAdd(new THREE.Mesh(new THREE.IcosahedronGeometry(0.46+(_svb%2)*0.12,1),toon(_svb%2?0x6F9A66:0x87AA70)));
            _svBush.position.set(_svX-4.5+_svb*1.15,_svY+6.82,_svZ+4.55);
        }
        // Three low path lights form an offset arrival sequence instead of rooftop corner lanterns.
        var _svLampPos=[[-5.8,9.3],[-1.0,10.2],[5.7,9.1]];
        for(var _svli=0;_svli<_svLampPos.length;_svli++){
            var _svStem=_svAdd(new THREE.Mesh(new THREE.CylinderGeometry(0.09,0.13,1.35,6),_svWood));
            _svStem.position.set(_svX+_svLampPos[_svli][0],_svY+0.68,_svZ+_svLampPos[_svli][1]);
            var _svLamp=_svAdd(new THREE.Mesh(new THREE.SphereGeometry(0.28,8,6),_svGlow));
            _svLamp.scale.set(1,1.35,1);_svLamp.position.set(_svStem.position.x,_svY+1.55,_svStem.position.z);
        }
        // Freestanding circular wayfinding sign: a neutral steam-and-leaf symbol, no branded text.
        var _svSignC=document.createElement('canvas');_svSignC.width=192;_svSignC.height=192;
        var _svCtx=_svSignC.getContext('2d');
        _svCtx.clearRect(0,0,192,192);_svCtx.fillStyle='#4C5B52';_svCtx.beginPath();_svCtx.arc(96,96,82,0,Math.PI*2);_svCtx.fill();
        _svCtx.strokeStyle='#F3D8A0';_svCtx.lineWidth=10;_svCtx.lineCap='round';
        for(var _svs=0;_svs<3;_svs++){_svCtx.beginPath();_svCtx.moveTo(62+_svs*31,118);_svCtx.bezierCurveTo(48+_svs*31,96,76+_svs*31,82,62+_svs*31,59);_svCtx.stroke();}
        _svCtx.strokeStyle='#A9C88B';_svCtx.lineWidth=8;_svCtx.beginPath();_svCtx.moveTo(66,139);_svCtx.quadraticCurveTo(102,122,130,137);_svCtx.stroke();
        var _svSignTex=new THREE.CanvasTexture(_svSignC);
        var _svSign=_svAdd(new THREE.Mesh(new THREE.CircleGeometry(1.25,32),new THREE.MeshBasicMaterial({map:_svSignTex,transparent:true,side:THREE.DoubleSide,toneMapped:false})));
        _svSign.name='sakura-traveler-rest-hall-sign';_svSign.position.set(_svX+8.1,_svY+2.15,_svZ+9.5);
        var _svSignPole=_svAdd(new THREE.Mesh(new THREE.CylinderGeometry(0.11,0.14,2.6,6),_svWood));
        _svSignPole.position.set(_svX+8.1,_svY+1.15,_svZ+9.46);
        cityColliders.push({x:_svX,z:_svZ,hw:10,hd:7.5,h:_svY+11.2,y:_svY});
        cityBuildingMeshes.push({meshes:_svMeshes,x:_svX,z:_svZ,hw:10,hd:7.5,h:_svY+11.2});
    ...
    // ===============================================================
    //  Silver Snow Village — snowy island, deep-blue lake and outer rest district
    // ===============================================================
    if(currentCityStyle===7){try{
        ...
        // Helper: steep-roof snow house built at island height
        var _by7=3; // island surface Y
        function _buildSteepRoofHouse(x,z,w,d,h){
            var ms=[];
        ...
        // === 1. Deep-blue ring lake between the island and outer shore ===
        ...
        // === 2. Steep-roof village spread across the large island ===
        var _steepRoofHouseList=[];
        ...
        for(var _gx=-100;_gx<=100;_gx+=35){
            for(var _gz=-100;_gz<=100;_gz+=35){
                ...
                _steepRoofHouseList.push({x:_gx+(Math.random()-0.5)*20,z:_gz+(Math.random()-0.5)*20,w:_gw,d:_gd,h:_gh});
            }
        }
        for(var _gi2=0;_gi2<_steepRoofHouseList.length;_gi2++){
            var g2=_steepRoofHouseList[_gi2];
            _buildSteepRoofHouse(g2.x,g2.z,g2.w,g2.d,g2.h);
        }
        ...
        // === 4. Winter rest district on the outer shore ===
    ...
    if(currentCityStyle===5){
        // Layout: Lunaglow Haven on the left, open survey plains on the right.
        ...
        for(var ci=0;ci<30;ci++){
            ...
            // Skip if inside Lunaglow Haven zone (x<-50) or too close to center
        ...
        // Place Apollo on the open survey plain.
        ...
        rover.rotateY(0.5);
        cityGroup.add(rover);
        // ---- Grand Lunar City "Lunaglow Haven" (lunar garden) ----
        ...
        // Separated regolith terraces preserve the crater edge without reading as a fortress wall.
        for(var cri=0;cri<12;cri++){
            var cra=cri/12*Math.PI*2+(cri%2)*0.045;
            var craterTerrace=new THREE.Mesh(new THREE.CapsuleGeometry(0.72,3.7,4,8),toon(cri%3===0?0x727A7B:0x666E76));
            craterTerrace.rotation.z=Math.PI/2;craterTerrace.rotation.y=-cra+Math.PI/2;
            craterTerrace.scale.set(1,1,0.82);craterTerrace.position.set(Math.cos(cra)*18,0.7+(cri%3)*0.08,Math.sin(cra)*18);
            lunarCity.add(craterTerrace);
        }
        // Crater floor (sunken)
        ...
        craterWall.position.y=-2.6;lunarCity.add(craterWall);
        // Low central ecology atrium — broad and layered instead of a military skyline.
        var _havenBldgMeshes=[]; // collect for occlusion
        var atriumBase=new THREE.Mesh(new THREE.CylinderGeometry(4.8,5.4,2.2,14),toon(0x5C7180));
        atriumBase.position.y=0.1;lunarCity.add(atriumBase);_havenBldgMeshes.push(atriumBase);
        var atriumRoof=new THREE.Mesh(new THREE.SphereGeometry(4.5,20,10,0,Math.PI*2,0,Math.PI/2),
            new THREE.MeshPhongMaterial({color:0xA9D7D0,transparent:true,opacity:0.28,side:THREE.DoubleSide,shininess:25}));
        atriumRoof.scale.y=0.48;atriumRoof.position.y=1.2;lunarCity.add(atriumRoof);_havenBldgMeshes.push(atriumRoof);
        var gardenCore=new THREE.Mesh(new THREE.DodecahedronGeometry(1.35,1),toon(0x72A98B));
        gardenCore.scale.set(1.5,0.75,1.5);gardenCore.position.y=1.45;lunarCity.add(gardenCore);_havenBldgMeshes.push(gardenCore);
        // Three soft canopy leaves give the research garden an asymmetric landmark.
        for(var cli=0;cli<3;cli++){
            var cla=cli/3*Math.PI*2+0.35;
            var canopyLeaf=new THREE.Mesh(new THREE.SphereGeometry(1.45,12,8),toon([0x78B79A,0x90C7A4,0x6EA690][cli]));
            canopyLeaf.scale.set(1.8,0.38,0.85);canopyLeaf.position.set(Math.cos(cla)*1.6,2.6+cli*0.12,Math.sin(cla)*1.6);
            canopyLeaf.rotation.y=-cla+0.35;lunarCity.add(canopyLeaf);_havenBldgMeshes.push(canopyLeaf);
        }
        // Petal-shaped research pavilions radiate from the garden without forming a tower ring.
        for(var lbi=0;lbi<9;lbi++){
            var lba=lbi/9*Math.PI*2+0.12;var lbr=7.2+(lbi%3)*0.7;
            var pavilion=new THREE.Group();
            var pavilionBase=new THREE.Mesh(new THREE.CapsuleGeometry(0.85,2.1,5,10),toon([0x657C86,0x708A8B,0x5F7581][lbi%3]));
            pavilionBase.rotation.z=Math.PI/2;pavilionBase.scale.set(1,1,0.72);pavilionBase.position.y=0.7;pavilion.add(pavilionBase);
            var pavilionRoof=new THREE.Mesh(new THREE.SphereGeometry(1.15,12,7),toon([0x9FC9B4,0x91BBC1,0xB8CFA9][lbi%3]));
            pavilionRoof.scale.set(1.65,0.34,0.82);pavilionRoof.position.y=1.65;pavilion.add(pavilionRoof);
            var windowBand=new THREE.Mesh(new THREE.BoxGeometry(1.8,0.28,0.08),new THREE.MeshBasicMaterial({color:lbi%2?0x8FE8FF:0xF3D98B,transparent:true,opacity:0.62}));
            windowBand.position.set(0,0.9,0.72);pavilion.add(windowBand);
            pavilion.position.set(Math.cos(lba)*lbr,0,Math.sin(lba)*lbr);pavilion.rotation.y=-lba+Math.PI/2;lunarCity.add(pavilion);
            pavilion.traverse(function(c){if(c.isMesh)_havenBldgMeshes.push(c);});
        }
        // Low habitat terraces vary in width and roof curve, keeping the horizon open.
        for(var ori=0;ori<12;ori++){
            var ora=ori/12*Math.PI*2+0.22;var orr=12.2+(ori%2)*1.3;
            var orw=2.2+(ori%3)*0.45,ord=1.6+(ori%2)*0.35,orh=1.35+(ori%4)*0.18;
            var habitat=new THREE.Group();
            var habitatBody=new THREE.Mesh(new THREE.BoxGeometry(orw,orh,ord),toon([0x607582,0x687D7C,0x586E78][ori%3]));
            habitatBody.position.y=orh/2-0.4;habitat.add(habitatBody);
            var habitatRoof=new THREE.Mesh(new THREE.SphereGeometry(1,12,7),toon([0x91B7A8,0xA6C6BC,0x86AFAE][ori%3]));
            habitatRoof.scale.set(orw*0.58,0.38,ord*0.62);habitatRoof.position.y=orh-0.15;habitat.add(habitatRoof);
            var habitatLight=new THREE.Mesh(new THREE.BoxGeometry(orw*0.55,0.22,0.06),new THREE.MeshBasicMaterial({color:ori%3===0?0xF5D88D:0x80DDE4,transparent:true,opacity:0.58}));
            habitatLight.position.set(0,orh*0.55,ord/2+0.04);habitat.add(habitatLight);
            habitat.position.set(Math.cos(ora)*orr,0,Math.sin(ora)*orr);habitat.rotation.y=-ora+Math.PI/2;lunarCity.add(habitat);
            habitat.traverse(function(c){if(c.isMesh)_havenBldgMeshes.push(c);});
        }
        ...
        for(var sli=0;sli<16;sli++){
            ...
            slLight.position.set(Math.cos(sla)*slr,2.1,Math.sin(sla)*slr);lunarCity.add(slLight);_havenBldgMeshes.push(slLight);
        }
        // Three visitor terraces sit below the crater rim as calm arrival gardens.
        for(var spi2=0;spi2<3;spi2++){
            var spa2=spi2/3*Math.PI*2+Math.PI/6;var spr=18.3;
            var terrace=new THREE.Group();
            var terraceBase=new THREE.Mesh(new THREE.CylinderGeometry(2.8,3.3,0.45,10),toon(0x60736F));terraceBase.position.y=0.2;terrace.add(terraceBase);
            var terraceGarden=new THREE.Mesh(new THREE.CircleGeometry(2.25,12),toon([0x739B7A,0x6E9290,0x8B9E72][spi2]));terraceGarden.rotation.x=-Math.PI/2;terraceGarden.position.y=0.44;terrace.add(terraceGarden);
            for(var tsi=0;tsi<3;tsi++){
                var stone=new THREE.Mesh(new THREE.DodecahedronGeometry(0.28+tsi*0.06,0),toon(0xA9B4A7));
                stone.position.set(-0.9+tsi*0.9,0.72,Math.sin(tsi*2.1)*0.65);terrace.add(stone);
            }
            terrace.position.set(Math.cos(spa2)*spr,1.1,Math.sin(spa2)*spr);lunarCity.add(terrace);
        }
        // A curved public moonwalk replaces the former launch rail.
        for(var mci=0;mci<11;mci++){
            var mwa=-0.7+mci*0.14,mwr=20.5;
            var moonwalk=new THREE.Mesh(new THREE.BoxGeometry(3.4,0.28,1.35),toon(mci%2?0x657978:0x708584));
            moonwalk.position.set(Math.cos(mwa)*mwr,1.15,Math.sin(mwa)*mwr);moonwalk.rotation.y=-mwa+Math.PI/2;lunarCity.add(moonwalk);
            if(mci%2===0){
                var guideLamp=new THREE.Mesh(new THREE.SphereGeometry(0.2,6,4),new THREE.MeshBasicMaterial({color:0xA7F0DC,transparent:true,opacity:0.68}));
                guideLamp.position.set(Math.cos(mwa)*(mwr+1.15),1.65,Math.sin(mwa)*(mwr+1.15));lunarCity.add(guideLamp);
            }
        }
        ...
        // Low navigation beacons trace the walking routes without dominating the skyline.
        for(var fvi=0;fvi<16;fvi++){
            var fva=fvi/16*Math.PI*2,fvr=9.8+(fvi%2)*5.2;
            var beaconStem=new THREE.Mesh(new THREE.CylinderGeometry(0.06,0.09,0.8,5),toon(0x71817D));
            beaconStem.position.set(Math.cos(fva)*fvr,0.4,Math.sin(fva)*fvr);lunarCity.add(beaconStem);
            var beaconGlow=new THREE.Mesh(new THREE.OctahedronGeometry(0.18,0),new THREE.MeshBasicMaterial({color:fvi%2?0x9EE6D0:0xF1D69B,transparent:true,opacity:0.72}));
            beaconGlow.position.set(Math.cos(fva)*fvr,0.95,Math.sin(fva)*fvr);lunarCity.add(beaconGlow);_havenBldgMeshes.push(beaconGlow);
        }
        // Place Lunaglow Haven on flat ground (left side, x<0)
        lunarCity.position.set(-200,0,0);
        lunarCity.scale.set(8,8,8);
        cityGroup.add(lunarCity);
        // Lunaglow Haven city doors — 4 entrances (N/S/E/W) on crater rim
        var _havenDoorAngles=[0,Math.PI/2,Math.PI,Math.PI*1.5];
        var _havenDoorR=18.5; // on crater rim
        for(var vdi=0;vdi<4;vdi++){
            var vda=_havenDoorAngles[vdi];
            ...
            // Position on rim
            doorG.position.set(Math.cos(vda)*_havenDoorR,1,Math.sin(vda)*_havenDoorR);
            doorG.rotation.y=vda+Math.PI; // face outward
            lunarCity.add(doorG);
        }
        // Lunaglow Haven collider zone (flat)
        window._moonClimateZones=[];
        // Climate-zone boundaries preserve the four public entrances and existing collision range.
        window._moonClimateZones.push({x:-200,y:0,z:0,r:160,
            doors:[{a:0,w:0.25},{a:Math.PI/2,w:0.25},{a:Math.PI,w:0.25},{a:Math.PI*1.5,w:0.25}]
        });
        window._moonCities=[
            {cx:-200,cy:0,cz:0,r:160,scale:8,name:{zhs:'\u6708\u8F89\u6E2F',zht:'\u6708\u8F1D\u6E2F',ja:'\u6708\u8F1D\u306E\u6E2F',en:'Lunaglow Haven'},flatX:-200,flatZ:0}
        ];
        // ---- Tranquil Vale (second city, far side) ----
        var tranquilVale=new THREE.Group();
        // Deep crater rim
        var grRim=new THREE.Mesh(new THREE.TorusGeometry(10,2,8,20),toon(0x556666));
        grRim.rotation.x=Math.PI/2;grRim.position.y=0.5;tranquilVale.add(grRim);
        // Crater bowl (sunken floor)
        var grFloor=new THREE.Mesh(new THREE.CylinderGeometry(9,9,0.5,20),toon(0x334455));
        grFloor.position.y=-3;tranquilVale.add(grFloor);
        // Crater inner wall (sloped)
        var grWall=new THREE.Mesh(new THREE.CylinderGeometry(9,10.5,4,20,1,true),toon(0x445566));
        grWall.position.y=-3;tranquilVale.add(grWall);
        // Blue glow from within (Tranquil Vale's signature blue lighting)
        ...
        grGlowFloor.rotation.x=-Math.PI/2;grGlowFloor.position.y=-2.5;tranquilVale.add(grGlowFloor);
        ...
        for(var gri=0;gri<3;gri++){
            ...
            grRing.rotation.x=Math.PI/2;grRing.position.y=-2.4;tranquilVale.add(grRing);
        }
        // Survey hangars and habitat modules (inside crater)
        for(var ghi=0;ghi<6;ghi++){
            ...
            gh.position.set(Math.cos(gha)*ghr,-2,Math.sin(gha)*ghr);gh.rotation.y=gha;tranquilVale.add(gh);
            ...
            ghd.position.set(Math.cos(gha)*(ghr+1.5),-1.9,Math.sin(gha)*(ghr+1.5));ghd.rotation.y=gha;tranquilVale.add(ghd);
            ...
            ghL.position.set(Math.cos(gha)*(ghr+1.5),-1.2,Math.sin(gha)*(ghr+1.5));tranquilVale.add(ghL);
        }
        // Inner buildings — survey operations structures
        for(var gbi=0;gbi<8;gbi++){
            ...
            gb2.position.set(Math.cos(gba)*gbr,gbh/2-2.5,Math.sin(gba)*gbr);tranquilVale.add(gb2);
            // Blue window strips
            for(var gwi=0;gwi<Math.floor(gbh/0.8);gwi++){
                ...
                gw.position.set(Math.cos(gba)*gbr,gwi*0.8-2,Math.sin(gba)*gbr+0.53);tranquilVale.add(gw);
            }
        }
        // A low seed-library glasshouse anchors the valley without a command-tower silhouette.
        var seedLibrary=new THREE.Mesh(new THREE.CylinderGeometry(2.4,2.8,1.5,12),toon(0x536F65));seedLibrary.position.y=-1.7;tranquilVale.add(seedLibrary);
        var seedLibraryRoof=new THREE.Mesh(new THREE.SphereGeometry(2.3,14,8,0,Math.PI*2,0,Math.PI/2),
            new THREE.MeshPhongMaterial({color:0xA4D1B7,transparent:true,opacity:0.26,side:THREE.DoubleSide,shininess:18}));
        seedLibraryRoof.scale.y=0.5;seedLibraryRoof.position.y=-0.95;tranquilVale.add(seedLibraryRoof);
        // Short asymmetrical sensor leaves replace the former needle-spire cluster.
        for(var gsi=0;gsi<7;gsi++){
            var gsa=gsi/7*Math.PI*2+0.2;var gsr=4.0+(gsi%3)*1.3;
            var sensorLeaf=new THREE.Mesh(new THREE.SphereGeometry(0.62,9,6),toon(gsi%2?0x718F78:0x668B83));
            sensorLeaf.scale.set(1.55,0.28,0.72);sensorLeaf.position.set(Math.cos(gsa)*gsr,-0.8+(gsi%2)*0.22,Math.sin(gsa)*gsr);
            sensorLeaf.rotation.y=-gsa+0.25;tranquilVale.add(sensorLeaf);
            var sensorLight=new THREE.Mesh(new THREE.OctahedronGeometry(0.15,0),new THREE.MeshBasicMaterial({color:gsi%2?0x9DE3C8:0x91CDE3,transparent:true,opacity:0.65}));
            sensorLight.position.set(Math.cos(gsa)*gsr,-0.2+(gsi%2)*0.22,Math.sin(gsa)*gsr);tranquilVale.add(sensorLight);
        }
        // Place Tranquil Vale on flat ground (left side, behind Lunaglow Haven)
        tranquilVale.position.set(-200,0,-200);
        tranquilVale.scale.set(8,8,8);
        cityGroup.add(tranquilVale);
        // Tranquil Vale city doors — 4 entrances (N/S/E/W) on rim
        var _grDoorR=10.5;
        for(var gdi=0;gdi<4;gdi++){
            ...
            tranquilVale.add(gdoorG);
        }
        // Tranquil Vale collider zone (flat)
        window._moonCities.push({cx:-200,cy:0,cz:-200,r:100,scale:8,name:{zhs:'\u9759\u6D77\u8C37',zht:'\u975C\u6D77\u8C37',ja:'\u9759\u6D77\u306E\u8C37',en:'Tranquil Vale'},flatX:-200,flatZ:-200});
        window._moonClimateZones.push({x:-200,y:0,z:-200,r:100,
            doors:[{a:0,w:0.3},{a:Math.PI/2,w:0.3},{a:Math.PI,w:0.3},{a:Math.PI*1.5,w:0.3}]
        });
        // Open climate canopies: asymmetric airflow ribs, leaf-like anchors and quiet sensor lights.
        // They deliberately avoid a solid hemisphere or a red/orange wireframe force-field silhouette.
        window._moonClimateCanopies=[
            _buildMoonClimateCanopy(-200,0,160,{rib:0x8FD8CE,accent:0xD7E6A2,anchor:0x6FA58B},0.4),
            _buildMoonClimateCanopy(-200,-200,100,{rib:0x9CCBDD,accent:0xD8CFA0,anchor:0x718F91},2.1)
        ];
        for(var _mcci=0;_mcci<window._moonClimateCanopies.length;_mcci++)scene.add(window._moonClimateCanopies[_mcci].group);
        // Moon city building colliders — flat box colliders
        window._moonBldgColliders=[];
        // Lunaglow Haven central tower
        cityColliders.push({x:-200,z:0,hw:12,hd:12,h:100});
        // Lunaglow Haven ring buildings
        ...
        // Tranquil Vale central tower
        cityColliders.push({x:-200,z:-200,hw:8,hd:8,h:70});
        // Add moon city meshes to building occlusion array — per collider zone
        var _havenAllMeshes=[];lunarCity.traverse(function(c){if(c.isMesh)_havenAllMeshes.push(c);});
        var _grAllMeshes=[];tranquilVale.traverse(function(c){if(c.isMesh)_grAllMeshes.push(c);});
        // Lunaglow Haven central tower
        cityBuildingMeshes.push({meshes:_havenAllMeshes,x:-200,z:0,hw:12,hd:12,h:100});
        // Lunaglow Haven ring buildings — each ring building gets an occlusion entry
        for(var _obi=0;_obi<12;_obi++){
            var _oba2=_obi/12*Math.PI*2;var _obr2=70;
            cityBuildingMeshes.push({meshes:_havenAllMeshes,x:-200+Math.cos(_oba2)*_obr2,z:Math.sin(_oba2)*_obr2,hw:10,hd:10,h:50});
        }
        // Tranquil Vale
        ...
        for(var ri2=0;ri2<25;ri2++){
            ...
            // Skip if inside Lunaglow Haven zone
        ...
        // ---- Large craters with rims across the survey terrain ----
        var _bigCraters=[];
        ...
        // ---- Additional historical landing flags across the survey plain ----
        ...
        // ---- More footprint trails across the open plain ----
        ...
        // ---- Regolith mounds across the open survey plain ----
        ...
        // ---- Return-to-Earth portal inside Lunaglow Haven city ----
        ...
        // Place inside Lunaglow Haven, near central tower (local coords, will be scaled by 8)
        earthPortalG.position.set(-200+8*5,0,8*5); // offset from Lunaglow Haven center
        ...
        // ---- Moon city props (inside Lunaglow Haven) ----
        // Oxygen tanks
        var _havenPropsData=[
        ...
        for(var vpi=0;vpi<_havenPropsData.length;vpi++){
            var vpd=_havenPropsData[vpi];
        ...
        // ---- Original lunar survey drones ----
        window._moonDrones=[];
        var droneKinds=['petalScout','surveyPod','cargoSkiff','relaySeed'];
        var dronePalettes=[0x7BC9B3,0xE7B86B,0x91A8D8,0xC98BB9];
        var droneZones=[{cx:220,cz:-240},{cx:260,cz:180},{cx:-80,cz:280},{cx:-260,cz:-80}];
        for(var gi=0;gi<40;gi++){
            var kind=droneKinds[gi%droneKinds.length],zoneIndex=gi%droneZones.length,zone=droneZones[zoneIndex];
            var drone=_buildMoonDrone(kind,dronePalettes[zoneIndex]);
            var altitude=28+Math.random()*70;
            if(kind==='relaySeed')altitude=100+Math.random()*80;
            var px=zone.cx+(Math.random()-0.5)*260,pz=zone.cz+(Math.random()-0.5)*260;
            drone.group.position.set(px,altitude,pz);drone.group.scale.set(2,2,2);scene.add(drone.group);
            window._moonDrones.push({group:drone.group,kind:kind,zone:zoneIndex,px:px,py:altitude,pz:pz,
                orbitRadius:45+Math.random()*100,orbitAngle:Math.random()*Math.PI*2,
                verticalPhase:Math.random()*Math.PI*2,speed:0.0015+Math.random()*0.0025,
                rotors:drone.rotors||[],beacon:drone.beacon||null});
        }
    }
}

function _buildMoonClimateCanopy(cx,cz,r,palette,phase){
    var group=new THREE.Group(),ribs=[],nodes=[];
    group.position.set(cx,0,cz);
    // Seven offset airflow ribs form an open, low canopy rather than a closed force-field dome.
    for(var ri=0;ri<7;ri++){
        var a=ri/7*Math.PI*2+phase;
        var endA=a+Math.PI*(0.62+(ri%3)*0.08);
        var start=new THREE.Vector3(Math.cos(a)*r,1.2,Math.sin(a)*r);
        var end=new THREE.Vector3(Math.cos(endA)*r,1.2,Math.sin(endA)*r);
        var midA=(a+endA)*0.5+0.18*Math.sin(ri*1.7);
        var control=new THREE.Vector3(Math.cos(midA)*r*0.24,r*(0.22+(ri%2)*0.05),Math.sin(midA)*r*0.24);
        var curve=new THREE.QuadraticBezierCurve3(start,control,end);
        var ribMat=new THREE.MeshBasicMaterial({color:ri%2?palette.rib:palette.accent,transparent:true,opacity:0.18});
        var rib=new THREE.Mesh(new THREE.TubeGeometry(curve,28,Math.max(0.42,r*0.0042),5,false),ribMat);
        group.add(rib);ribs.push({mesh:rib,baseOpacity:0.16+(ri%3)*0.025,phase:ri*0.73});
        // Small sensor seeds travel visually along selected ribs without looking like impact ripples.
        for(var ni=0;ni<3;ni++){
            var point=curve.getPoint((ni+1)/4);
            var node=new THREE.Mesh(new THREE.OctahedronGeometry(Math.max(0.65,r*0.006),0),
                new THREE.MeshBasicMaterial({color:ni===1?palette.accent:palette.rib,transparent:true,opacity:0.58}));
            node.position.copy(point);group.add(node);nodes.push({mesh:node,baseY:point.y,phase:phase+ri*0.8+ni*1.6});
        }
        // Flattened leaf anchors make the ground connection read as environmental infrastructure.
        var anchor=new THREE.Mesh(new THREE.SphereGeometry(Math.max(2.4,r*0.025),10,7),toon(palette.anchor));
        anchor.scale.set(1.9,0.34,0.82);anchor.position.copy(start);anchor.position.y=1.05;anchor.rotation.y=-a+0.35;group.add(anchor);
    }
    // Three high, translucent climate sails overlap loosely instead of enclosing the city.
    for(var si=0;si<3;si++){
        var sail=new THREE.Mesh(new THREE.CircleGeometry(r*(0.16+si*0.018),18),
            new THREE.MeshBasicMaterial({color:si===1?palette.accent:palette.rib,transparent:true,opacity:0.035,side:THREE.DoubleSide,depthWrite:false}));
        sail.scale.set(1.9,0.72,1);sail.position.set((si-1)*r*0.16,r*(0.18+si*0.035),(1-si)*r*0.12);
        sail.rotation.set(-0.58+si*0.16,phase+si*0.82,0.22-si*0.18);group.add(sail);
    }
    return {group:group,ribs:ribs,nodes:nodes,phase:phase};
}

function _findMoonClimateZone(px,py,pz){
    if(!window._moonClimateZones)return null;
    for(var zi=0;zi<window._moonClimateZones.length;zi++){
        var zone=window._moonClimateZones[zi];
        var dx=px-zone.x,dy=py-zone.y,dz=pz-zone.z;
        if(dx*dx+dy*dy+dz*dz<zone.r*zone.r)return zone;
    }
    return null;
}

function _buildMoonDrone(kind,color){
    var g=new THREE.Group(),mat=toon(color,{}),trim=toon(0xE6EEF2,{});
    var glow=new THREE.MeshBasicMaterial({color:0x8FE8FF,transparent:true,opacity:0.72}),rotors=[];
    if(kind==='petalScout'){
        var core=new THREE.Mesh(new THREE.DodecahedronGeometry(0.85,0),mat);g.add(core);
        for(var i=0;i<3;i++){var fin=new THREE.Mesh(new THREE.ConeGeometry(0.34,1.15,5),trim);var a=i*Math.PI*2/3;fin.position.set(Math.cos(a)*1.05,0,Math.sin(a)*1.05);fin.rotation.z=Math.PI/2;fin.rotation.y=-a;g.add(fin);}
        var eye=new THREE.Mesh(new THREE.SphereGeometry(0.22,8,6),glow);eye.position.set(0,0,0.82);g.add(eye);
    }else if(kind==='surveyPod'){
        var hull=new THREE.Mesh(new THREE.CylinderGeometry(0.72,0.92,1.55,8),mat);hull.rotation.x=Math.PI/2;g.add(hull);
        for(var j=-1;j<=1;j+=2){var vane=new THREE.Mesh(new THREE.BoxGeometry(1.15,0.10,0.55),trim);vane.position.x=j*1.05;vane.rotation.z=j*0.16;g.add(vane);rotors.push(vane);}
        var lens=new THREE.Mesh(new THREE.CircleGeometry(0.34,10),glow);lens.position.z=0.94;g.add(lens);
    }else if(kind==='cargoSkiff'){
        var deck=new THREE.Mesh(new THREE.BoxGeometry(2.5,0.5,1.45),mat);deck.geometry.translate(0,0.05,0);g.add(deck);
        var canopy=new THREE.Mesh(new THREE.SphereGeometry(0.55,10,6,0,Math.PI*2,0,Math.PI/2),trim);canopy.scale.set(1.15,0.65,0.9);canopy.position.set(0,0.32,0.1);g.add(canopy);
        for(var k=-1;k<=1;k+=2){var pack=new THREE.Mesh(new THREE.BoxGeometry(0.52,0.62,0.82),toon(0x8C705C));pack.position.set(k*0.9,-0.05,-0.1);g.add(pack);}
    }else{
        var seed=new THREE.Mesh(new THREE.CapsuleGeometry(0.65,2.4,5,9),mat);seed.rotation.x=Math.PI/2;g.add(seed);
        var ring=new THREE.Mesh(new THREE.TorusGeometry(1.35,0.08,6,18),trim);ring.rotation.x=Math.PI/2;g.add(ring);rotors.push(ring);
        var beacon=new THREE.Mesh(new THREE.OctahedronGeometry(0.25,0),glow);beacon.position.y=1.15;g.add(beacon);
    }
    var marker=new THREE.Mesh(new THREE.OctahedronGeometry(0.16,0),glow);marker.position.y=-0.85;g.add(marker);
    return {group:g,rotors:rotors,beacon:marker};
}
