...
function _danboPortalDismissManagedExternally(key){
    // The shop doorway and keeper use their own proximity loop in
    // cosmetics.js.  Keep their dismissal key until that loop observes the
    // player walking away; the generic portal loop must not clear it merely
    // because there is no regular portal under the player this frame.
    return typeof key==='string'&&(
        key.indexOf('hidden:shopHouse:')===0||
        key.indexOf('hidden:shopKeeper:')===0
    );
}
...
function updateCity(){
    ...
    // Independent blade arc flight (after Cloudwing Traveler interrupted)
    if(window._cloudMarkDrift&&window._cloudMark&&window._cloudMark.visible){
        var _gaf=window._cloudMarkDrift;
        _gaf.life--;
        window._cloudMark.position.x+=Math.sin(_gaf.faceY)*0.03;
        window._cloudMark.position.z+=Math.cos(_gaf.faceY)*0.03;
        window._cloudMark.material.opacity=Math.max(0,_gaf.life/40)*0.85;
        if(_gaf.life<=0){window._cloudMark.visible=false;window._cloudMarkDrift=null;}
    }
    ...
    for(const p of portals){
        ...
        if(pv){
            ...
            // conventional vortex. The soft-shell silhouette always remains readable.
    ...
    // Skip portal triggers while trail bursting
    ...
    var _portalBlocked=!!(playerEgg._herbDashTimer||playerEgg._crystalGlideTimer||playerEgg._trailSweepActive||playerEgg._canopyLiftActive||playerEgg._cloudVaultTimer||playerEgg.throwTimer>0||playerEgg._crystalPulseTimer||playerEgg._harvestFan||playerEgg._leafFlurryTimer||playerEgg._berryFlurryTimer);
    if(_pipeTraveling||_pipeArrivalCooldown>0||_trailBurstActive||_portalBlocked||window._danboPluginTransition){_portalPromptPortal=null;if(_pp)_pp.style.display='none';} else {
    ...
    // Check travel beacons (stable internal warp-pipe fields use the same proximity system)
    ...
    var _isVoluntary=playerEgg&&_nearGround&&!playerEgg.throwTimer&&!playerEgg._stunTimer&&!playerEgg.heldBy&&!playerEgg._herbDashTimer&&!playerEgg._crystalGlideTimer&&!playerEgg._crystalGlideFalling&&!playerEgg._trailSweepActive&&!playerEgg._canopyLiftActive&&!playerEgg._cloudVaultTimer&&!playerEgg._electrocuted&&!playerEgg._elecFlying;
    var _portalTriggerDist=_nearP&&Number(_nearP._triggerDist)||PORTAL_CONFIG.triggerDist;
    var _portalConfirmDist=_nearP&&Number(_nearP._confirmDist)||PORTAL_CONFIG.confirmDist;
    var _portalAction=_nearP?_danboPortalAction(_nearD,_portalTriggerDist,_portalConfirmDist,_nearP._isWarpPipe,_isVoluntary):0;
    if(_nearP&&_portalAction>0){
        _portalPromptPortal=_nearP;
        if(_pp)_pp.style.display=_nearP._showPrompt===false?'none':'block';
        ...
        } else if(!_portalConfirmOpen&&_nearP._showPrompt!==false){
            if(_pt)_pt.textContent=_nearP.name+' \u2014 '+_nearP.desc+'  ('+L('walkIn')+')';
        }
    } else if(!_portalConfirmOpen){
        _portalPromptPortal=null;
        if(_pp)_pp.style.display='none';
        if(!_danboPortalDismissManagedExternally(_portalDismissed))_portalDismissed=null;
    }
    ...
    for(const c of cityCoins){
        ...
        if(DANBO_WASM.within3D(px,py,pz,c.mesh.position.x,c.mesh.position.y,c.mesh.position.z,1.5)){
            ...
            playCoinSound();
            // Tower of Babel unlock requirement is authored by the city map.
            if(!_babylonTriggered&&_babelTowerShouldTrigger()){_triggerBabylonEvent();}
        }
    }
    ...
    if(typeof Explorer!=='undefined'){
        if(currentCityStyle===5){
            var _moonNames=I18N.cityNames[_langCode]||I18N.cityNames.en;
            Explorer.discoverHidden('moon_city',_moonNames[5]||'\u6708\u8F89\u4E4B\u57CE');
        }
        else if(currentCityStyle<=4&&py>40){
            var _cloudDiscoveryNames={zhs:'云之岛',zht:'雲之島',ja:'雲の島',en:'Cloud Island'};
            Explorer.discoverHidden('cloud_world',_cloudDiscoveryNames[_langCode]||_cloudDiscoveryNames.en);
        }
    }
    ...
    for(const npc of cityNPCs){
        ...
        // Fire effect (Fruitbrew Traveler canopyLift)
        if(npc._onFire>0){
            npc._onFire--;
            ...
            // Fire stun (harvest fan) — freeze then knockback
            if(npc._fireStun>0){
                npc._fireStun--;
        ...
        // Electrocution effect (Salt Crystal Traveler electric) — flash skeleton
    ...
    if(_babylonRising&&_babylonTower){
        _babylonRiseY+=_babylonTower.riseSpeed||0.29;
        ...
        if(_babylonRiseY>=0){
            ...
            _finalizeBabylonTowerCollision();
        }
        _babylonTower.group.position.y=_babylonTower.baseY+_babylonRiseY;
    }
    // ---- Tower of Babel door collision (4 doors: N/S/E/W) ----
    if(_babylonTower&&!_pipeTraveling&&!_portalConfirmOpen&&!_babylonElevator&&!_trailBurstActive){
        var bt=_babylonTower;
        var _babelDoors=bt.doors||[];
        var _nearestBabelDoor=9999;
        for(var bdi=0;bdi<_babelDoors.length;bdi++){
            ...
            if(bt.elevatorEnabled!==false&&bt.showInstructions!==false&&bdd<(bt.interactionRadius||4.2)&&py<bt.baseY+3.5&&!_babylonPromptDismissed&&!_babylonRising){
        ...
        if(bt.elevatorEnabled!==false&&bt.showInstructions!==false&&tdist<(bt.interactionRadius||4.2)&&py>=bt.topY-2&&py<=bt.topY+4&&!_babylonPromptDismissed&&!_babylonRising){
            _showBabylonPrompt(-1);
        }
    ...
    if(_babylonElevator&&_babylonTower&&playerEgg){
        ...
        var elevSpeed=bt2.elevatorSpeed||0.35;
        ...
        // Arrived at bottom
        if(_babylonElevDir===-1&&_babylonElevY<=bt2.baseY+1){
            _babylonElevator=false;
            var exitDoor=bt2.doors&&bt2.doors[0];
            playerEgg.mesh.position.set(exitDoor?exitDoor.x:bt2.x,bt2.baseY+1,exitDoor?exitDoor.z:bt2.z+bt2.baseD/2+1);
    ...
    // Open climate-canopy airflow: slow, asynchronous light breathing and sensor drift.
    if(window._moonClimateCanopies){
        var _canopyTime=Date.now()*0.001;
        for(var _mci=0;_mci<window._moonClimateCanopies.length;_mci++){
            var _mcan=window._moonClimateCanopies[_mci];
            for(var _mcri=0;_mcri<_mcan.ribs.length;_mcri++){
                var _mcrib=_mcan.ribs[_mcri];
                _mcrib.mesh.material.opacity=_mcrib.baseOpacity+Math.sin(_canopyTime*0.42+_mcrib.phase)*0.035;
            }
            for(var _mcni=0;_mcni<_mcan.nodes.length;_mcni++){
                var _mcnode=_mcan.nodes[_mcni],_mcpulse=Math.sin(_canopyTime*0.7+_mcnode.phase);
                _mcnode.mesh.position.y=_mcnode.baseY+_mcpulse*0.55;
                _mcnode.mesh.rotation.y+=0.003+(_mcni%3)*0.0007;
                _mcnode.mesh.scale.setScalar(0.94+(_mcpulse+1)*0.035);
            }
        }
    }
    // ---- Moon rover movement ----
    if(window._moonRover){
        ...
        // Keep the rover on the open survey plain (x>20, within bounds).
    ...
    // ---- Original lunar survey-drone animation ----
    if(window._moonDrones){
        var _droneTime=Date.now()*0.001;
        var _insideMoonShelter=playerEgg?_findMoonClimateZone(playerEgg.mesh.position.x,playerEgg.mesh.position.y,playerEgg.mesh.position.z):null;
        var _droneZones=[{cx:220,cz:-240},{cx:260,cz:180},{cx:-80,cz:280},{cx:-260,cz:-80}];
        for(var _mdi=0;_mdi<window._moonDrones.length;_mdi++){
            var _md=window._moonDrones[_mdi],_mz=_droneZones[_md.zone]||_droneZones[0];
            _md.orbitAngle+=_md.speed;
            _md.group.position.x=_mz.cx+Math.cos(_md.orbitAngle)*_md.orbitRadius;
            _md.group.position.z=_mz.cz+Math.sin(_md.orbitAngle)*_md.orbitRadius;
            _md.group.position.y=_md.py+Math.sin(_droneTime*0.65+_md.verticalPhase)*5;
            _md.group.rotation.y=-_md.orbitAngle+Math.PI/2;
            for(var _mri=0;_mri<_md.rotors.length;_mri++)_md.rotors[_mri].rotation.z+=0.008+_mri*0.002;
            if(_md.beacon){_md.beacon.rotation.y+=0.025;_md.beacon.material.opacity=0.48+Math.sin(_droneTime*2+_mdi)*0.2;}
            _md.group.visible=!_insideMoonShelter;
        }
    }
}
...
function _showChatBubble(egg,msg,duration){
    ...
    // Create a 3D sprite bubble above the traveler
...
function updateHeldEggs(eggPool){
    ensureStruggleBar();
    // City NPCs remain in allEggs while a legacy race is active.  Callers can
    // provide the scene-local participant list so grab/throw interactions do
    // not cross the city/race boundary.  City mode keeps the original default.
    var activeEggs=eggPool||allEggs;
    var playerIsHeld=false;
    for(var i=0;i<activeEggs.length;i++){
        ...
        if(!egg.heldBy){
            ...
            var _inSpin=egg._crystalGlideTimer||egg._crystalGlideFalling||egg._cloudVaultTimer||egg._herbDashTimer;
    ...
    for(var n=0;n<activeEggs.length;n++){
        var npc=activeEggs[n];
        ...
        for(var m=0;m<activeEggs.length;m++){
            var target=activeEggs[m];
    ...
    for(var np2=0;np2<activeEggs.length;np2++){
        var npc2=activeEggs[np2];
        ...
        if(npc2._npcPropHoldTimer<=0){
            ...
            for(var nt=0;nt<activeEggs.length;nt++){
                var tgt=activeEggs[nt];
    ...
    for(var tpi=0;tpi<cityProps.length;tpi++){
        ...
        // Hit eggs
        for(var tpe=0;tpe<activeEggs.length;tpe++){
            var tpeg=activeEggs[tpe];
    ...
    for(var ti=0;ti<_danboRaceObstacles().length;ti++){
        ...
        for(var te=0;te<activeEggs.length;te++){
            var teg=activeEggs[te];
    ...
    for(var g=0;g<activeEggs.length;g++){if(activeEggs[g].grabCD>0)activeEggs[g].grabCD--;}
}
...
function cancelPortalConfirm(){
    // touchend can be followed by a synthetic click on some mobile browsers.
    // Once the dialog has closed, ignore that second cancellation event so it
    // cannot overwrite the dismissal key recorded by the first event.
...
function confirmPortalEnter(){
    ...
    if(ri>=0){
        ...
        _babylonElevY=(_bDir===1)?_babylonTower.baseY+1:_babylonTower.topY;
        if(sfxEnabled){
...
document.getElementById('portal-no').addEventListener('click',function(e){
    // Stop the closing tap/click from falling through to the shop prompt that
    // sits behind the modal and immediately opening the same dialog again.
    if(e){e.preventDefault();e.stopPropagation();}
    cancelPortalConfirm();
});
document.getElementById('portal-no').addEventListener('touchend',function(e){
    e.preventDefault();e.stopPropagation();cancelPortalConfirm();
},{passive:false});
...
function _showBabylonPrompt(dir){
    ...
    var upDesc={zhs:'\u4E58\u5750\u7535\u68AF\u524D\u5F80云之岛\uFF1F',zht:'\u4E58\u5750\u96FB\u68AF\u524D\u5F80雲之島\uFF1F',ja:'\u30A8\u30EC\u30D9\u30FC\u30BF\u30FC\u3067雲の島\u3078\uFF1F',en:'Take elevator to Cloud Island?'};
...
// Use the same localization table as the selection map and HUD so an area
// never reveals a stale or differently translated title.
var _areaNames=I18N.cityNames;
var _areaNameCloud={zhs:'云之岛',zht:'雲之島',ja:'雲の島',en:'Cloud Island'};
function _showAreaName(name){
    ...
    text.style.animation='areaNameReveal 3.5s ease-out forwards';
...
function enterCity(spawnX,spawnZ,spawnY){
    gameState='city';
    _trailBurstActive=false;_trailBurstTimer=0;_trailBurstTimerMax=0;_trailBurstSpeed=0;if(_trailBurstBar)_trailBurstBar.visible=false;
    showScreen(null);
    stopSelectBGM();
    stopRaceBGM();
    startBGM();
    ...
    // Map-editor previews keep the authored unlock rules intact, but can place
    // the player beside the tower so layout/event checks do not require a long
    // walk from the normal city spawn.
    var editorPreview=window.DANBO_MAP_EDITOR_GAME_PREVIEW;
    var babelPreview=editorPreview&&editorPreview.active&&editorPreview.focus==='babel-tower'&&currentCityStyle!==5?editorPreview:null;
    var babelPreviewDef=babelPreview&&typeof _babelTowerDefinition==='function'?_babelTowerDefinition(false):null;
    var hasExplicitSpawn=spawnX!==undefined||spawnZ!==undefined;
    if(babelPreview){
        // Keep all three editor checks deterministic and independent of the
        // player's persistent wallet. This value exists only in the preview
        // window and does not change the authored unlock requirement.
        coins=0;
        var previewCoinHUD=document.getElementById('coin-hud');
        if(previewCoinHUD)previewCoinHUD.textContent='⭐ 0';
        if(babelPreviewDef&&babelPreview.state!=='normal'){
            babelPreview.cameraUntil=Date.now()+(babelPreview.state==='unlock-event'?9000:3200);
            babelPreview.cameraTarget={
                x:Number(babelPreviewDef.x)||0,
                y:Math.max(18,Math.min(42,(Number(babelPreviewDef.h)||82)*0.42)),
                z:Number(babelPreviewDef.z)||0
            };
        }
    }
    ...
    if(!hasExplicitSpawn&&babelPreviewDef&&(babelPreview.state==='placement'||babelPreview.state==='unlock-event')){
        var towerX=Number(babelPreviewDef.x)||0,towerZ=Number(babelPreviewDef.z)||0;
        var towerHalf=Math.max(Number(babelPreviewDef.w)||16,Number(babelPreviewDef.d)||16)*0.5;
        sx=towerX;sz=towerZ+towerHalf+18;
    }
    var sy=spawnY!==undefined?spawnY:0;
    var hasExplicitHeight=spawnY!==undefined;
    if(!hasExplicitHeight&&currentCityStyle===5){
        if(sx===0&&sz===5){sx=50;sz=0;}
        sy=0.5;
    } else if(!hasExplicitHeight&&sx===0&&sz===0){
        sy=15; // above fountain, land on pillar top
    }
    const skin=CHARACTERS[selectedChar];
    playerEgg=createEgg(sx,sz,skin.color,skin.accent,true,undefined,skin.id);
    ...
    if(babelPreview&&babelPreviewDef){
        if(babelPreview.state==='placement'){
            // Direct placement review: show the completed tower without changing
            // coins, the saved unlock threshold, or normal game progression.
            _babylonTriggered=true;_babylonRising=false;_babylonRiseY=0;
            _buildBabylonTower(babelPreviewDef,{riseOffset:0});
            _finalizeBabylonTowerCollision();
            if(typeof _floatToast==='function')_floatToast('🗼 编辑器预览：巴别塔已强制显示','#FFE08B','15%','11%',650,2200);
        }else if(babelPreview.state==='unlock-event'){
            // Event review: keep it hidden for one second, then exercise the same
            // rise path used by a real unlock.
            if(typeof _floatToast==='function')_floatToast('🎬 编辑器预览：即将模拟巴别塔升起','#FFE08B','15%','11%',650,1800);
            window._danboBabelPreviewTimer=setTimeout(function(){
                window._danboBabelPreviewTimer=0;
                if(gameState==='city'&&currentCityStyle===babelPreview.city&&!_babylonTriggered)_triggerBabylonEvent();
            },1000);
        }else{
            if(!_babylonTriggered&&_babelTowerShouldTrigger())_triggerBabylonEvent();
            if(typeof _floatToast==='function')_floatToast('🎮 正常规则：收集 '+_babelTowerUnlockRequirement()+' 枚金币解锁巴别塔','#CFE9FF','15%','11%',650,2400);
        }
    }else if(!_babylonTriggered&&_babelTowerShouldTrigger()){
        // Normal game behavior remains unchanged.
        _triggerBabylonEvent();
    }
    // original area name reveal
...
function checkThrownEggImpact(eggList){
    for(var i=0;i<eggList.length;i++){
        ...
        for(var j=0;j<eggList.length;j++){
            ...
            if(dist<1.3){
                // Impact response: victim moves up and away
...
function animate(now){
    ...
    // The embedded map-editor game is paused while the separate full game
    // preview window is open. Keep a lightweight rAF heartbeat so it can
    // resume immediately, but skip simulation, post-processing and WebGL.
    if(window.DANBO_MAP_EDITOR_PAUSED){
        _lastFrameTime=now;
        _accumulator=0;
        return;
    }
...
function _gameUpdate(){
    ...
    var _pluginBusy=!!(window._danboPluginTransition||_activePlugin);
    if(_pluginBusy){
        ...
        // legacy-race still runs through the shared racing branch below. Other
        // plugins own their complete update loop and continue to pause it.
        if(!_activePlugin||_activePlugin.id!=='legacy-race')return;
    }
    ...
    if(gameState==='city'){
        ...
        // Safety: release player if the holder does not belong to this race.
        if(playerEgg&&playerEgg.heldBy&&raceEggs.indexOf(playerEgg.heldBy)===-1){
        ...
        for(const egg of raceEggs){
            if(!egg.isPlayer){
                ...
                if(egg.mesh.position.y<_fallLimit){
                    ...
                    egg.heldBy=null;egg.holding=null;
                }
                if(_pfActive){
        ...
        updateHeldEggs(raceEggs);
                // Race coin collection + animation
        for(var ci=0;ci<_danboRaceCoins().length;ci++){
            var rc=_danboRaceCoins()[ci];
            if(rc.collected)continue;
            var collectibleMotion=rc.mesh&&rc.mesh.userData&&(rc.mesh.userData._raceCollectibleMotion||rc.mesh.userData._herbLeafMotion);
            if(collectibleMotion){
                rc.bobPhase+=collectibleMotion.speed;
                rc.mesh.position.y=rc.fy+1.2+Math.sin(rc.bobPhase)*collectibleMotion.amplitude;
                // Remove the previous frame's visual-only drift before applying
                // the next one, so magnet movement and the stored pickup centre remain intact.
                rc.mesh.position.x-=collectibleMotion.lastDx;
                rc.mesh.position.z-=collectibleMotion.lastDz;
                var driftWave=Math.sin(rc.bobPhase*0.73+collectibleMotion.driftPhase);
                collectibleMotion.lastDx=driftWave*collectibleMotion.drift;
                collectibleMotion.lastDz=Math.cos(rc.bobPhase*0.57+collectibleMotion.driftPhase)*collectibleMotion.drift*0.32;
                rc.mesh.position.x+=collectibleMotion.lastDx;
                rc.mesh.position.z+=collectibleMotion.lastDz;
                rc.mesh.rotation.x=collectibleMotion.baseTiltX+Math.sin(rc.bobPhase*0.81+collectibleMotion.driftPhase)*collectibleMotion.sway;
                rc.mesh.rotation.z=collectibleMotion.baseTiltZ+Math.cos(rc.bobPhase*0.69+collectibleMotion.driftPhase)*collectibleMotion.sway;
                rc.mesh.rotation.y+=collectibleMotion.spin;
            }else{
                rc.bobPhase+=0.05;
