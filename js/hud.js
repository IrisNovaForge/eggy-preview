...
// ---- Stun stars (Traveler style spinning stars above head) ----
...
// ---- Traveler trail burst state ----
var _trailBurstActive=false, _trailBurstTimer=0, _trailBurstTimerMax=0, _trailBurstSpeed=0;
var _trailBurstBar=null;
function _createTrailBurstBar(){
    var canvas=document.createElement('canvas');
    canvas.width=256;canvas.height=40;
    var tex=new THREE.CanvasTexture(canvas);
    tex.minFilter=THREE.LinearFilter;
...
function _drawTrailBurstBar(sprite,pct){
    var ctx=sprite._ctx,w=256,h=40;
    ctx.clearRect(0,0,w,h);
    function rr(x,y,rw,rh,rad){ctx.beginPath();ctx.moveTo(x+rad,y);ctx.lineTo(x+rw-rad,y);ctx.quadraticCurveTo(x+rw,y,x+rw,y+rad);ctx.lineTo(x+rw,y+rh-rad);ctx.quadraticCurveTo(x+rw,y+rh,x+rw-rad,y+rh);ctx.lineTo(x+rad,y+rh);ctx.quadraticCurveTo(x,y+rh,x,y+rh-rad);ctx.lineTo(x,y+rad);ctx.quadraticCurveTo(x,y,x+rad,y);ctx.closePath();}
    ctx.fillStyle='rgba(0,0,0,0.85)';
...
function _playSprintTick(pct){
    ...
    o2.type='triangle';
    o2.frequency.setValueAtTime(baseFreq*1.08,t+0.05);
    o2.frequency.exponentialRampToValueAtTime(baseFreq*1.28,t+0.08);
...
function _updateSprintBar(holdingF){
    ...
    if(holdingF){
        ...
        if(playerEgg._sprintHoldFrames>=18){
            ...
            // Trail-burst preparation: squash into ball and vibrate while charging
            var _chgPct=_sprintCharge/_sprintChargeMax;
            playerEgg.squash=1.0; // remain upright while gathering momentum
            playerEgg.vx*=0.1;playerEgg.vz*=0.1; // stay in place while charging
            var _chgBody=playerEgg.mesh.userData.body;
            if(_chgBody){_chgBody.scale.set(1-_chgPct*0.025,1+_chgPct*0.045,1-_chgPct*0.025);_chgBody.rotation.x=0;}
        }
        playerEgg._wasSprintHolding=true;
    } else {
        // Trigger trail burst on release if charge was high enough
        if(playerEgg._wasSprintHolding&&_sprintCharge>=_sprintChargeMax&&!_trailBurstActive){
            var sdPct=_sprintCharge/_sprintChargeMax;
            _trailBurstActive=true;
            _trailBurstTimer=Math.floor(80+sdPct*160);
            _trailBurstTimerMax=_trailBurstTimer;
            _trailBurstSpeed=MAX_SPEED*4.0;
            ...
            // Trail-burst wind sound
            if(sfxEnabled){var ctx=ensureAudio();if(ctx){var ct=ctx.currentTime;var o=ctx.createOscillator();var f=ctx.createBiquadFilter();var g=ctx.createGain();o.type='triangle';o.frequency.setValueAtTime(180,ct);o.frequency.exponentialRampToValueAtTime(420,ct+0.24);f.type='lowpass';f.frequency.setValueAtTime(900,ct);g.gain.setValueAtTime(0.08,ct);g.gain.exponentialRampToValueAtTime(0.001,ct+0.32);o.connect(f);f.connect(g);g.connect(ctx.destination);o.start(ct);o.stop(ct+0.34);}}
        }
        ...
        if(!_trailBurstActive&&_sprintCharge<=0){playerEgg.squash=1.0;var _rsB=playerEgg.mesh.userData.body;if(_rsB){_rsB.scale.set(1,1,1);}}
    }
