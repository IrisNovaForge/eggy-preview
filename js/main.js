...
applyCityTheme();
// Initialize the shared 3D traveler selection state
if(typeof _updateTravelerSelect==='function')_updateTravelerSelect(0);
...
document.getElementById('confirm-btn').addEventListener('click',()=>{
    ...
    var _enterSelectedCharacter=function(){
        // A traveler's birthCity is selection-screen presentation metadata only.
        // Every new journey begins in Hope City; the homeland miniature remains
        // character-specific on the selection screen via DANBO_CHARACTER_SELECT_CITY_MAP.
        var targetCity=0;
        var finishEnter=function(loadOK){
            // If a release asset failed to load, retain the already available
            // city rather than leaving the selection screen permanently locked.
            if(loadOK===false){
                console.error('Hope City could not be loaded; entering the current city instead.');
                targetCity=currentCityStyle;
            }
            // A lazily loaded city carries a neutral base name. Reapply the
            // active language before theme/HUD construction reads it.
            var localizedNames=I18N.cityNames[_langCode]||I18N.cityNames.en;
            if(CITY_STYLES[targetCity]&&localizedNames[targetCity])CITY_STYLES[targetCity].name=localizedNames[targetCity];
            if(currentCityStyle!==targetCity){
                if(typeof _resetViewMode==='function')_resetViewMode();
                _prevCityStyle=currentCityStyle;
                currentCityStyle=targetCity;
                _cameraZoom=1.0;
                clearCity();
                buildCity();
                buildPortals();
            ...
            var spawn=(window.CITY_DEFAULT_SPAWNS||CITY_DEFAULT_SPAWNS)[targetCity]||{x:0,y:15,z:0};
            showScreen(null);
            spawnCityNPCs();
            enterCity(spawn.x,spawn.z,spawn.y);
            _selectConfirmed=false;_startTriggered=false; // reset for next time
        };
        if(typeof _ensureCityDataLoaded==='function')_ensureCityDataLoaded(targetCity,finishEnter);
        else finishEnter(true);
    };
...
// A window launched by the map editor is a focused authoring check rather than
// a normal play session. Enter the requested city immediately so the landmark
// and its preview state are visible without crossing the title/select screens.
// Ordinary game URLs never set this object and retain the normal flow.
if(window.DANBO_MAP_EDITOR_GAME_PREVIEW&&window.DANBO_MAP_EDITOR_GAME_PREVIEW.active){
    setTimeout(function(){
        if(gameState!=='city'){
            showScreen(null);
            spawnCityNPCs();
...
function goBackToCity(){
    // The platformer uses style 99 while active. Preserve its real source city
    // before clearing the flag so returning through Exploration Gate rebuilds
    // the city the player actually came from.
    var _returnCity=(_pfActive&&typeof _pfSavedCity==='number'&&_pfSavedCity>=0)?_pfSavedCity:currentCityStyle;
