...
function updateEggPhysics(egg, isCity){
    ...
    if(isCity){
        ...
        if(egg.mesh.position.y<-5){
            if(currentCityStyle===5){
                // Moon: respawn inside Lunaglow Haven
        ...
        // Moon climate-zone boundary — preserve city access through the four public entrances.
        if(currentCityStyle===5&&window._moonClimateZones){
            for(var _si=0;_si<window._moonClimateZones.length;_si++){
                var _zone=window._moonClimateZones[_si];
                var _sdx=egg.mesh.position.x-_zone.x;
                var _sdz=egg.mesh.position.z-_zone.z;
                var _sdist=DANBO_WASM.dist2D(egg.mesh.position.x,egg.mesh.position.z,_zone.x,_zone.z);
                // Check if the traveler is near the climate boundary (inside or crossing).
                if(_sdist<_zone.r+3&&_sdist>_zone.r-6){
                    // Check if near a door opening
                    var _sAngle=Math.atan2(_sdz,_sdx);
                    if(_sAngle<0)_sAngle+=Math.PI*2;
                    var _atDoor=false;
                    for(var _di=0;_di<_zone.doors.length;_di++){
                        var _da=_zone.doors[_di].a;var _dw=_zone.doors[_di].w;
                        var _diff=DANBO_WASM.angleDiff(_sAngle,_da);
                        if(_diff<_dw){_atDoor=true;break;}
                    }
                    if(!_atDoor){
                        // Keep the traveler on the correct side of the climate boundary.
                        if(_sdist<_zone.r){
                            // Inside near the boundary (not at a door): block exit.
                            var _moveDir=egg.vx*_sdx+egg.vz*_sdz; // positive = moving outward
                            if(_moveDir>0){
                                // Moving outward through wall — push back in
                                var _pushR=_zone.r-1;
                                egg.mesh.position.x=_zone.x+(_sdx/_sdist)*_pushR;
                                egg.mesh.position.z=_zone.z+(_sdz/_sdist)*_pushR;
                                // Reflect velocity along the boundary normal.
                            ...
                            // Outside and crossing away from an entrance: push outward.
                            var _pushR2=_zone.r+0.5;
                            egg.mesh.position.x=_zone.x+(_sdx/_sdist)*_pushR2;
                            egg.mesh.position.z=_zone.z+(_sdz/_sdist)*_pushR2;
        ...
        // Travel beacon — now handled in gameloop.js updateCity portal check
        if(egg.isPlayer){
            // Cloud world moon pipe — proximity prompt
            if(_cloudWorldPipe&&!_pipeTraveling&&!_portalConfirmOpen&&!_trailBurstActive){
    ...
    // ============================================================
    var _attackAnim=!!(egg._herbDashTimer||egg._crystalGlideTimer||egg._cloudVaultTimer);
    if(body){
        if(egg._herbDashTimer>0){
            egg.mesh.scale.set(1,1,1);
            // Visual handled by player.js quaternion
        } else if(egg._crystalGlideTimer>0||egg._crystalGlideFalling){
            // Rotation handled in FINAL block at end of function
        } else if(egg._cloudVaultTimer>0){
            // Cloudwing Traveler cloudVault: handled elsewhere
        } else {
            // Normal walk lean
            var _isSpinning2=(egg.isPlayer&&_trailBurstActive)||(egg._npcSpinTimer&&egg._npcSpinTimer>=30&&egg._npcSpinTimer<90);
            if(!_isSpinning2){
    ...
    if(egg._herbDashTimer>0){
        ...
        var _hdP3=(egg._herbDashTotal||60)-egg._herbDashTimer;
        if(_hdP3<8){egg.vy=0.08;egg.vx=(egg._dashDirX||0)*0.3;egg.vz=(egg._dashDirZ||0)*0.3;}
        else if(egg._herbDashTimer>5){egg.vx=egg._dashDirX;egg.vz=egg._dashDirZ;egg.mesh.position.y=Math.max(egg.mesh.position.y,0.8);}
        else{egg.vx*=0.7;egg.vz*=0.7;}
        if(egg._herbDashTimer<=0&&body){body.rotation.x=0;}
    } else if(egg._crystalGlideTimer>0){
    ...
    var _isCrystalGliding=(egg._crystalGlideTimer>0||egg._crystalGlideFalling)&&!egg._herbDashTimer;
    if(!_isCrystalGliding){
    ...
    if(speed>0.01&&!_isCrystalGliding){
        // Skip facing update during backstep or post-dash bounce or Golden Grain Traveler attack
        if(!(egg.isPlayer&&(egg._dashBounceTimer>0||egg._herbDashTimer>0||(egg._atkAnim>0&&egg.mesh.userData._travelerId==='goldenGrainTraveler')))){
    ...
    // ---- FINAL: Herb Traveler herbDash tilt (MUST be last to override everything) ----
    if(egg._herbDashTimer>0&&!egg._crystalGlide&&egg._dashDirX!==undefined){
        var _hdT=egg._herbDashTotal||60;
        var _hdP=_hdT-egg._herbDashTimer;
        var _hdA=0;
        if(_hdP<8){_hdA=(_hdP/8)*1.57;}
        else if(egg._herbDashTimer>5){_hdA=1.57;}
        else{var _l2=(5-egg._herbDashTimer)/5;_hdA=egg._herbDashBounced?-1.57*(1-_l2):1.57*(1-_l2);}
    ...
    // ---- FINAL: Salt Crystal Traveler roll spin — body self-rotation (pivot at body center, no sway) ----
    if(_isCrystalGliding){
        if(egg._crystalGlideAngle===undefined)egg._crystalGlideAngle=0;
        egg._crystalGlideAngle+=0.6;
        // Mesh only handles facing direction — no X/Z rotation on mesh
        if(egg._crystalGlideDirX!==undefined){
            egg.mesh.rotation.y=Math.atan2(egg._crystalGlideDirX,egg._crystalGlideDirZ);
        ...
        if(_brB)_brB.rotation.x=egg._crystalGlideAngle;
