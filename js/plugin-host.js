(function(){
    ...
    function selectedCharacterSnapshot(){
        ...
        var roster=(typeof CHARACTERS!=='undefined'&&CHARACTERS)||[];
        var rosterCharacter=roster[idx]||roster[0]||{};
        var key=rosterCharacter.id||ch.id||('traveler'+idx);
        var canonicalKey=key;
        var legacyId=rosterCharacter.legacyId||ch.legacyId||'';
        var localizedName=
            (typeof I18N!=='undefined'&&I18N.charNames&&(I18N.charNames[_langCode]||I18N.charNames.en||[])[idx])||
        ...
        var snap={
            ...
            nameKey:rosterCharacter.nameKey||ch.nameKey||canonicalKey,
            legacyId:legacyId,
            legacyCharacterId:legacyId,
            aliases:[legacyId].filter(function(value,index,array){
                return value&&value!==key&&array.indexOf(value)===index;
            }),
            type:canonicalKey,
            name:localizedName,
    ...
    function bridgeEnterMarkup(){
        // The real transition is drawn in the Three.js scene so it matches the old mini-game journey-bridge atmosphere.
    ...
    function cleanupSceneJourneyBridge(){
    ...
    function startSceneJourneyBridge(durationMs){
        cleanupSceneJourneyBridge();
        if(typeof THREE==='undefined'||typeof scene==='undefined'||!scene||typeof playerEgg==='undefined'||!playerEgg||!playerEgg.mesh)return;
        var group=new THREE.Group();group.name='danbo-plugin-journey-bridge';
        scene.add(group);
        ...
        // 2026-06-17 old journey-bridge composition: tiny ground rings that grow, 6 golden runes, 14 participant beams, 30 white particles.
        for(var i=0;i<7;i++){
    ...
    function start(pluginId,options){
        ...
        startSceneJourneyBridge(bridgeDuration);
