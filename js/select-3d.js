...
(function(){
    ...
    var COPY={};
    ['zhs','zht','ja','en'].forEach(function(code){
        var titles=I18N.charTitles[code]||I18N.charTitles.en;
        var descriptions=I18N.charDescriptions[code]||I18N.charDescriptions.en;
        COPY[code]=titles.map(function(title,index){return[title,descriptions[index]||''];});
    });
    var _fallbackEmoji=['🌼','🌲','💎','☁️','🍎','🫐','🌶️','🌾'];
    // Stable character ID -> matching city style shown by the selection-page map.
    var CHARACTER_SELECT_CITY_BY_ID={
        blossomTraveler:6,herbTraveler:7,forestEgg:7,
        saltCrystalTraveler:2,crystalEgg:2,
        cloudwingTraveler:0,angelEgg:0,fruitbrewTraveler:4,candyEgg:4,
        berryTraveler:5,starEgg:5,spicyFlameTraveler:3,rockEgg:3,
        goldenGrainTraveler:1,windEgg:1
    };
    var CHARACTER_SELECT_CITY_MAP=CHARACTERS.map(function(character,index){
        var mapped=Number.isInteger(character.birthCity)?character.birthCity:CHARACTER_SELECT_CITY_BY_ID[character.canonicalId||character.id];
        if(mapped===undefined)mapped=CHARACTER_SELECT_CITY_BY_ID[character.id];
        return mapped===undefined?index:mapped;
    });
    window.DANBO_CHARACTER_SELECT_CITY_MAP=CHARACTER_SELECT_CITY_MAP.slice();
    window.DANBO_CHARACTER_SELECT_CITY_BY_ID=Object.assign({},CHARACTER_SELECT_CITY_BY_ID);
    ...
    function makeStage(ch,index){
        ...
        (model.userData._cloudwingDetails||[]).forEach(markCaster);
        (model.userData._saltCrystalEars||model.userData._crystalEars||[]).forEach(markCaster);
        (model.userData._fruitbrewDetails||[]).forEach(markCaster);
        (model.userData._flowerDetails||[]).forEach(markCaster);
        (model.userData._forestLeaves||[]).forEach(markCaster);
        (model.userData._spiceDetails||model.userData._rockDetails||[]).forEach(markCaster);
        (model.userData._berryDetails||[]).forEach(markCaster);
        (model.userData._goldenGrainDetails||[]).forEach(markCaster);
        (model.userData._regionalMarks||[]).forEach(markCaster);
        (model.userData._travelGear||[]).forEach(markCaster);
    ...
    function makeSceneMap(characterIndex){
        ...
        if(index===0){
            ...
            addTree(-1.72,.10,.74,'pine',0xb98fa5);addTree(1.73,.18,.68,'pine',0x9f8798);
            [-.48,-.18,.17,.48].forEach(function(x,ci){
                var heights=[.74,1.24,1.00,.64],crystal=add(new THREE.ConeGeometry(.17+(ci%2)*.03,heights[ci],6),new THREE.MeshPhysicalMaterial({color:[0xf0cbd9,0xfff1e8,0xdcb7c8,0xead8cb][ci],roughness:.22,metalness:.02,transparent:true,opacity:.92,clearcoat:.54}),x,.43+heights[ci]*.5,0);
            ...
            for(var volcanicStone=0;volcanicStone<7;volcanicStone++){var ra=volcanicStone*Math.PI*2/7,rr=1.42+(volcanicStone%2)*.16,r=add(new THREE.DodecahedronGeometry(.13+(volcanicStone%3)*.025,0),pbr(0x493b40,.96,0),Math.cos(ra)*rr,.45,Math.sin(ra)*rr);r.rotation.set(ra*.31,ra,0);}
            ...
            var chiliPodMat=pbr(0xd94436,.42,0),chiliLeafMat=pbr(0x548a45,.78,0);
            [[-1.06,.70,.58],[1.02,-.74,.52]].forEach(function(cp,ci){
                var stem=add(new THREE.CylinderGeometry(.014,.022,.27,7),chiliLeafMat,cp[0],.54,cp[1]);stem.rotation.z=(ci?-.18:.18);
                var pod=add(new THREE.SphereGeometry(.085,12,8),chiliPodMat,cp[0]+(ci?-.05:.05),.47,cp[1]+.02);pod.scale.set(.62,1.45,.56);pod.rotation.z=ci?.48:-.48;
                var leaf=add(new THREE.SphereGeometry(.07,10,7),chiliLeafMat,cp[0]+(ci?.05:-.05),.63,cp[1]);leaf.scale.set(1.28,.38,.60);leaf.rotation.z=ci?-.55:.55;
            });
        }else if(index===4){
    ...
    function makeCuteSceneMap(characterIndex){
        ...
        if(index===0){
            ...
            house(-1.28,.58,0xf0dedf,0xc99cad,.61);house(1.28,.58,0xead7dc,0xb88fa4,.58);
            roundHouse(-1.28,-.58,0xf8ece7,0xd2a5b7,.50);roundHouse(1.28,-.58,0xeadbd8,0xb78ea1,.45);
            tree(-.45,-1.18,0xaa879b,.68,false);tree(.45,-1.18,0xaa879b,.62,false);
            tree(-.45,1.18,0xbe99aa,.45,false);tree(.45,1.18,0x9f8292,.40,false);
            [-.32,0,.34].forEach(function(x,i){
                var h=[.70,1.18,.84][i],crystal=add(new THREE.ConeGeometry(.22,h,6),matte([0xf0cbd9,0xfff1e8,0xdcb7c8][i],.30),x,.22+h*.5,0);
            ...
            [[-.86,-.88],[.92,.82]].forEach(function(p,ci){
                var stem=add(new THREE.CylinderGeometry(.014,.022,.24,7),matte(0x548a45,.78),p[0],.40,p[1]);stem.rotation.z=ci?-.18:.18;
                var pod=add(new THREE.SphereGeometry(.078,10,7),matte(0xd94436,.44),p[0]+(ci?-.04:.04),.34,p[1]);pod.scale.set(.60,1.40,.54);pod.rotation.z=ci?.46:-.46;
            });
        }else if(index===4){
    ...
    var cardGestureKindsById={
        blossomTraveler:'blossom-traveler-wave',herbTraveler:'herb-traveler-sway',
        saltCrystalTraveler:'salt-crystal-skip',crystalEgg:'salt-crystal-skip',
        cloudwingTraveler:'cloudwing-float',angelEgg:'cloudwing-float',
        fruitbrewTraveler:'fruitbrew-bounce',candyEgg:'fruitbrew-bounce',
        berryTraveler:'berry-hop',starEgg:'berry-hop',
        spicyFlameTraveler:'spicy-flame-nod',rockEgg:'spicy-flame-nod',
        goldenGrainTraveler:'golden-grain-sway',windEgg:'golden-grain-sway'
    };
    var cardGestureKinds=CHARACTERS.map(function(character){return cardGestureKindsById[character.canonicalId||character.id]||cardGestureKindsById[character.id]||'gentle-wave';});
    ...
    function setSelected(idx){
        ...
        if(mapCity){
            ...
            mapCity.textContent=(cityNames&&cityNames[mapStyle])||['希望之城','金穗田园','盐晶冰原','熔岩峡谷','果香小镇','月辉之城','樱霞花谷','银雪之乡'][mapStyle];
        }
    ...
    window._play3DSelectCardGesture=function(idx){
        ...
        // Compatibility alias retained for clients that used the first card-wave API.
    ...
    function renderRect(el,item,hero,t){
        ...
        item.model.position.y=item.baseY+(hero?Math.sin(heroAge*1.35)*.018:0);
        ...
        item.model.rotation.x=0;item.model.rotation.z=0;
        if(hero){
            // Regional idle language: small enough to preserve the calm hero pose,
            // different enough that the eight travelers do not feel cloned.
            var heroType=item.model.userData._charType||'egg';
            if(heroType==='egg'){
                item.model.position.y+=Math.sin(heroAge*.82)*.010;item.model.rotation.z=Math.sin(heroAge*.72)*.014;
            }else if(heroType==='bull'){
                item.model.position.y+=Math.sin(heroAge*.68)*.006;item.model.rotation.z=Math.sin(heroAge*.58)*.010;
            }else if(heroType==='cat'){
                item.model.position.y+=Math.max(0,Math.sin(heroAge*1.15))*.010;item.model.rotation.y=Math.sin(heroAge*.72)*.16;
            }else if(heroType==='rooster'){
                item.model.position.y+=Math.sin(heroAge*.92)*.034;item.model.rotation.z=Math.sin(heroAge*.66)*.012;
            }else if(heroType==='dog'){
                item.model.position.y+=Math.pow(Math.max(0,Math.sin(heroAge*1.20)),2)*.018;item.model.rotation.z=Math.sin(heroAge*.78)*.013;
            }else if(heroType==='monkey'){
                item.model.position.y+=Math.sin(heroAge*1.05)*.013;item.model.rotation.z=Math.sin(heroAge*1.05)*.028;
            }else if(heroType==='bear'){
                item.model.position.y+=Math.max(0,Math.sin(heroAge*1.42))*.006;item.model.rotation.z=-Math.sin(heroAge*.70)*.009;
            }else if(heroType==='cockroach'){
                item.model.position.y+=Math.sin(heroAge*.70)*.008;item.model.rotation.z=Math.sin(heroAge*.62)*.018;
            }
        }
        ...
        if(gestureActive){
            ...
            var detailLists=[
                ud._flowerDetails||[],ud._forestLeaves||[],ud._saltCrystalSparkles||ud._crystalSparkles||[],ud._cloudwingDetails||[],
                ud._fruitbrewDetails||[],ud._berryDetails||[],ud._spiceDetails||ud._rockDetails||[],ud._goldenGrainDetails||[]
            ];
