...
function _drawSF2Map(highlightX,highlightY){
    ...
    // 1) Central homeland — Flower Isle (蜜蕊旅人)
    ...
    // 3) Salt Crystal Isle (盐晶旅人) — upper right
    _isle(300,52,32,22,'#E8C8D5','#FFF0E8');
    ctx.fillStyle='#D9A9BD';ctx.beginPath();ctx.moveTo(300,38);ctx.lineTo(307,50);ctx.lineTo(300,62);ctx.lineTo(293,50);ctx.closePath();ctx.fill();
    ...
    // 4) Cloudsea Isle (云翼旅人) — floating on a cloud, top-center
    ...
    ctx.strokeStyle='#78BFE6';ctx.lineWidth=2;ctx.beginPath();ctx.ellipse(200,32,8,3,-.2,0,Math.PI*2);ctx.stroke();
    // 5) Orchard Isle (果酿旅人) — right
    _isle(335,120,34,24,'#F7B6D2','#FBE0EC');
    ctx.strokeStyle='#648B45';ctx.lineWidth=2;ctx.beginPath();ctx.moveTo(335,120);ctx.lineTo(335,108);ctx.stroke();
    ctx.fillStyle='#72A858';ctx.beginPath();ctx.ellipse(330,110,6,3,-.45,0,Math.PI*2);ctx.fill();
    ['#E95B65','#F1904C','#D95076'].forEach(function(col,ci){ctx.fillStyle=col;ctx.beginPath();ctx.arc(327+ci*8,118+(ci%2)*3,3.6,0,Math.PI*2);ctx.fill();});
    // 6) Nightwood Isle (浆果旅人) — lower left
    _isle(95,165,30,22,'#B9A7F0','#E7DEFA');
    ctx.strokeStyle='#47764D';ctx.lineWidth=2;ctx.beginPath();ctx.arc(95,163,9,.1,Math.PI*1.2);ctx.stroke();
    [[90,165],[98,160],[101,168]].forEach(function(p){ctx.fillStyle='#9F3F7A';ctx.beginPath();ctx.arc(p[0],p[1],3,0,Math.PI*2);ctx.fill();});
    // 7) Volcanic Spice Isle (辣焰旅人) — left
    _isle(55,105,26,20,'#B86A58','#E8B572');
    ctx.fillStyle='#D94335';ctx.beginPath();ctx.ellipse(55,103,5,9,-.48,0,Math.PI*2);ctx.fill();
    ctx.strokeStyle='#4F8A48';ctx.lineWidth=2;ctx.beginPath();ctx.moveTo(52,96);ctx.quadraticCurveTo(48,92,51,89);ctx.stroke();
    // 8) Fieldland Isle (金穗旅人) — lower right
    _isle(320,175,24,18,'#BFEAD0','#E4F6EC');
    ctx.strokeStyle='#A87A35';ctx.lineWidth=2;ctx.beginPath();ctx.moveTo(320,181);ctx.lineTo(320,163);ctx.stroke();
    for(var wi=0;wi<3;wi++){[-1,1].forEach(function(s){ctx.fillStyle='#E3B94F';ctx.beginPath();ctx.ellipse(320+s*4,166+wi*5,4,2.2,s*.55,0,Math.PI*2);ctx.fill();});}
...
function _drawCuteMiniPortrait(ctx,ch,size){
    ...
    if(ch.type==='dog'){[-1,1].forEach(function(s){ctx.strokeStyle='#6D914F';ctx.lineWidth=1.4;ctx.beginPath();ctx.moveTo(cx+s*size*.10,cy-size*.23);ctx.lineTo(cx+s*size*.24,cy-size*.39);ctx.stroke();ctx.fillStyle='#78B766';ctx.beginPath();ctx.ellipse(cx+s*size*.20,cy-size*.34,size*.07,size*.03,s*.5,0,Math.PI*2);ctx.fill();ctx.fillStyle=s<0?'#E65E66':'#F08B52';ctx.beginPath();ctx.arc(cx+s*size*.25,cy-size*.40,size*.035,0,Math.PI*2);ctx.fill();});}
    ...
    else if(ch.type==='monkey'){ctx.strokeStyle='#47764D';ctx.lineWidth=1.5;ctx.beginPath();ctx.arc(cx,cy-size*.21,size*.24,Math.PI*1.1,Math.PI*1.9);ctx.stroke();[-1,0,1].forEach(function(s){ctx.fillStyle='#9F3F7A';ctx.beginPath();ctx.arc(cx+s*size*.065,cy-size*.35+Math.abs(s)*size*.02,size*.032,0,Math.PI*2);ctx.fill();});}
    ...
    else if(ch.type==='rooster'){ctx.fillStyle='rgba(255,255,255,.88)';[-1,0,1].forEach(function(s){ctx.beginPath();ctx.arc(cx+s*size*.10,cy-size*.31,size*.09,0,Math.PI*2);ctx.fill();});ctx.strokeStyle='#78BFE6';ctx.lineWidth=1.4;ctx.beginPath();ctx.ellipse(cx,cy-size*.40,size*.13,size*.04,-.18,0,Math.PI*2);ctx.stroke();}
    else if(ch.type==='cockroach'){[-1,1].forEach(function(s){ctx.strokeStyle='#A87A35';ctx.lineWidth=1.2;ctx.beginPath();ctx.moveTo(cx+s*size*.12,cy-size*.16);ctx.lineTo(cx+s*size*.20,cy-size*.39);ctx.stroke();for(var gi=0;gi<2;gi++){ctx.fillStyle='#F0C75C';ctx.beginPath();ctx.ellipse(cx+s*size*(.18+gi*.02),cy-size*(.30+gi*.06),size*.035,size*.02,s*.55,0,Math.PI*2);ctx.fill();}});}
