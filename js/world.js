...
// ============================================================
const RACES = [
    {id:'crazy-track',name:'🌀 疯狂赛道', desc:'旋转臂与传送带！', x:40, z:0, color:0xFF4444},
    {id:'hammer-storm',name:'🔨 锤子风暴', desc:'大锤与摆锤！小心！', x:35, z:20, color:0xFF8800},
    {id:'extreme-challenge',name:'⚡ 极限挑战', desc:'所有障碍加速！', x:20, z:35, color:0x8844FF},
    {id:'champion-road',name:'👑 冠军之路', desc:'最终决战！', x:0, z:40, color:0xFFD700},
    {id:'verdant-breeze-hills',name:'💎 \u7fe0\u98ce\u4e18\u9675', desc:'\u7fe0\u98ce\u4e0e\u5c71\u91ce\u7684\u8f7b\u5feb\u65c5\u9014\uff01', x:-20, z:35, color:0x44DD44},
    {id:'flame-valley',name:'🔥 \u706b\u7130\u5c71\u8c37', desc:'\u52a0\u901f\u5e26\u4e0e\u5ca9\u6d46\u5730\u5f62\uff01', x:-35, z:20, color:0xFF4400},
    {id:'frost-slide',name:'\u2744\ufe0f \u51b0\u971c\u6ed1\u9053', desc:'\u6ed1\u51b0\u5730\u5f62\u4e0e\u5f39\u7c27\uff01', x:-40, z:0, color:0x44CCFF},
    {id:'rainbow-sky',name:'🌈 \u5f69\u8679\u5929\u7a7a', desc:'\u7a7a\u4e2d\u5e73\u53f0\u4e0e\u91d1\u5e01\u96e8\uff01', x:-35, z:-20, color:0xFF88FF},
    {id:'moss-shadow-trail',name:'🌿 \u82d4\u5f71\u6797\u5f84', desc:'\u82d4\u5f71\u6591\u9a73\u7684\u68ee\u6797\u901a\u9053\u4e0e\u81ea\u7136\u969c\u788d\uff01', x:-20, z:-35, color:0x44BB44},
    {id:'lava-castle',name:'🔥 \u5ca9\u6d46\u57ce\u5821', desc:'\u5ca9\u6d46\u5730\u5f62\u4e0e\u706b\u7403\uff01', x:0, z:-40, color:0xDD4400},
    {id:'cloud-heaven',name:'\u2601\ufe0f \u4e91\u7aef\u5929\u5802', desc:'\u7a7a\u4e2d\u5e73\u53f0\u4e0e\u5f39\u7c27\uff01', x:20, z:-35, color:0x88CCFF},
    {id:'obstacle-fortress',name:'🏰 \u969c\u788d\u8981\u585e', desc:'\u6700\u7ec8\u5173\u5361\uff01\u5168\u969c\u788d\uff01', x:35, z:-20, color:0x884422}
];
...
function _danboPortalTheme(index,fallback,kindOverride){
    var themes=[
    ...
    if(kindOverride&&kindOverride!=='auto'){
        for(var ti=0;ti<themes.length;ti++){
            if(themes[ti][0]===kindOverride){p=themes[ti];break;}
        }
    }
...
function _danboPortalGeometry(){
    ...
    // The gate uses the project's soft shell silhouette instead of a generic
...
function _danboBuildRacePortal(race,index,options){
    ...
    options=options||{};
    var theme=_danboPortalTheme(index,race.color,options.theme),geo=_danboPortalGeometry(),g=new THREE.Group();g.name='cinematic-themed-portal';
    ...
    // A small travel crest and paired gem lanterns give the gate a friendly
    ...
    for(var ei=0;ei<defs.length;ei++){
        ...
        try{
            ...
            portals.push({
                ...
                _editorSpecialIndex:(typeof entrance.editorSpecialIndex==='number')?entrance.editorSpecialIndex:undefined,
...
function _danboRacePortalDefinitions(){
    if(!window.DANBO_CITY_REGISTRY||typeof DANBO_CITY_REGISTRY.getSpecialObjects!=='function')return [];
    var list=DANBO_CITY_REGISTRY.getSpecialObjects(currentCityStyle)||[];
    return list.map(function(def,index){return {def:def,index:index};}).filter(function(entry){
        return entry.def&&entry.def.type==='racePortal';
    });
}

function _danboExplorationPortalDefinition(){
    if(!window.DANBO_CITY_REGISTRY||typeof DANBO_CITY_REGISTRY.getSpecialObjects!=='function')return null;
    var list=DANBO_CITY_REGISTRY.getSpecialObjects(currentCityStyle)||[];
    for(var i=0;i<list.length;i++){
        if(list[i]&&list[i].type==='explorationPortal')return {def:list[i],index:i};
    }
    return null;
}

function _danboRaceIndexFromDefinition(def){
    var target=def&&def.target||{};
    var raceId=target.raceId||def.raceId;
    if(raceId){
        for(var i=0;i<RACES.length;i++)if(RACES[i].id===raceId)return i;
    }
    var legacy=Number(target.legacyRaceIndex);
    if(!Number.isFinite(legacy))legacy=Number(def.raceIndex);
    return Number.isFinite(legacy)&&legacy>=0&&legacy<RACES.length?Math.floor(legacy):-1;
}

function _danboAddRacePortal(race,index,def,editorSpecialIndex){
    def=def||{};
    var visual=def.visual||{},interaction=def.interaction||{},sign=def.sign||{};
    var customColor=Number(visual.primaryColor),portalRace=Object.assign({},race);
    if(Number.isFinite(customColor))portalRace.color=customColor;
    var portalVisual=_danboBuildRacePortal(portalRace,index,{theme:visual.theme||'auto'});
    var g=portalVisual.group;
    var portalX=Number.isFinite(Number(def.x))?Number(def.x):race.x;
    var portalY=Number.isFinite(Number(def.y))?Number(def.y):(currentCityStyle===7?3:0);
    var portalZ=Number.isFinite(Number(def.z))?Number(def.z):race.z;
    g.position.set(portalX,portalY,portalZ);
    var objectScale=Math.max(0.65,Math.min(1.6,Number(def.scale)||1));
    g.scale.setScalar(objectScale);
    if(Number.isFinite(Number(def.rotationY)))g.rotation.y=Number(def.rotationY)*Math.PI/180;
    else{
        var portalLen=Math.sqrt(portalX*portalX+portalZ*portalZ)||1;
        g.rotation.y=Math.atan2(-portalZ/portalLen,portalX/portalLen);
    }
    if(editorSpecialIndex!==undefined){
        g.userData.editorSpecialIndex=editorSpecialIndex;
        g.userData.editorSpecialType='racePortal';
        g.traverse(function(child){
            child.userData=child.userData||{};
            child.userData.editorSpecialIndex=editorSpecialIndex;
            child.userData.editorSpecialType='racePortal';
        });
    }
    cityGroup.add(g);
    var portal={
        mesh:g,ring:portalVisual.ring,inner:portalVisual.inner,name:race.name,desc:race.desc,
        raceIndex:index,x:portalX,z:portalZ,y:portalY,color:portalRace.color,_visual:portalVisual,
        _triggerDist:Math.max(2,Math.min(16,Number(interaction.triggerDistance)||PORTAL_CONFIG.triggerDist)),
        _confirmDist:Math.max(1,Math.min(12,Number(interaction.confirmDistance)||PORTAL_CONFIG.confirmDist)),
        _showPrompt:interaction.showPrompt!==false,
        _editorSpecialIndex:editorSpecialIndex
    };
    portals.push(portal);
    if(sign.visible!==false)_danboMakePortalSign(g,race.name,portalVisual.theme.b);
    return portal;
}
...
function buildPortals() {
    ...
    var authoredPortals=_danboRacePortalDefinitions();
    if(authoredPortals.length){
        authoredPortals.forEach(function(entry){
            if(entry.def.enabled===false)return;
            var raceIndex=_danboRaceIndexFromDefinition(entry.def);
            if(raceIndex<0){
                console.warn('[RacePortal] unknown target',entry.def.target||entry.def.raceId||entry.def.raceIndex);
                return;
            }
            _danboAddRacePortal(RACES[raceIndex],raceIndex,entry.def,entry.index);
        });
    }else{
        RACES.forEach(function(race,i){_danboAddRacePortal(race,i,{},undefined);});
    }

    // ---- Platformer mini-game portal ----
    if(currentCityStyle!==5){
        var _pfEntry=_danboExplorationPortalDefinition(),_pfDef=_pfEntry?_pfEntry.def:{};
        if(!_pfEntry||_pfDef.enabled!==false){
        var _pfPX=Number(_pfDef.x),_pfPY=Number(_pfDef.y),_pfPZ=Number(_pfDef.z);
        if(!Number.isFinite(_pfPX))_pfPX=PORTAL_POSITIONS.platformerPortal.x;
        if(!Number.isFinite(_pfPY))_pfPY=0;
        if(!Number.isFinite(_pfPZ))_pfPZ=PORTAL_POSITIONS.platformerPortal.z;
        var _pfScale=Math.max(.65,Math.min(1.6,Number(_pfDef.scale)||1));
        var _pfInteraction=_pfDef.interaction||{},_pfSign=_pfDef.sign||{};
        ...
        var _pfPortalG=_pfVisual.group;_pfPortalG.name='exploration-gate-portal';
        _pfPortalG.position.set(_pfPX,_pfPY,_pfPZ);_pfPortalG.scale.setScalar(_pfScale);
        if(Number.isFinite(Number(_pfDef.rotationY)))_pfPortalG.rotation.y=Number(_pfDef.rotationY)*Math.PI/180;
        ...
        var _pfEditorIndex=_pfEntry?_pfEntry.index:-1;
        _pfPortalG.userData.editorSpecialIndex=_pfEditorIndex;
        _pfPortalG.userData.editorSpecialType='explorationPortal';
        _pfPortalG.traverse(function(child){
            child.userData=child.userData||{};
            child.userData.editorSpecialIndex=_pfEditorIndex;
            child.userData.editorSpecialType='explorationPortal';
        });
        cityGroup.add(_pfPortalG);
        var _pfName={zhs:'\uD83C\uDF44 \u63A2\u7D22\u4E4B\u95E8',zht:'\uD83C\uDF44 \u63A2\u7D22\u4E4B\u9580',ja:'\uD83C\uDF44 \u63A2\u7D22\u306E\u9580',en:'\uD83C\uDF44 Exploration Gate'};
        ...
        portals.push({mesh:_pfPortalG,ring:_pfVisual.ring,inner:_pfVisual.inner,
            ...
            raceIndex:-1,x:_pfPX,z:_pfPZ,y:_pfPY,color:0x66C879,_hiddenType:'platformer',_targetStyle:-99,_visual:_pfVisual,
            _triggerDist:Math.max(2,Math.min(16,Number(_pfInteraction.triggerDistance)||PORTAL_CONFIG.triggerDist)),
            _confirmDist:Math.max(1,Math.min(12,Number(_pfInteraction.confirmDistance)||PORTAL_CONFIG.confirmDist)),
            _showPrompt:_pfInteraction.showPrompt!==false,
            _editorSpecialIndex:_pfEditorIndex});
        if(_pfSign.visible!==false)_danboMakePortalSign(_pfPortalG,_pfName[_langCode]||_pfName.en,_pfVisual.theme.b);
        }
    }
...
// The former 12-sided flat cylinder read like a yellow marker. This version keeps
...
function _areaDisplayName(area){
    ...
    if(area==='cloud'){
        var cloudCompletionNames={
            zhs:'\u2601\uFE0F 云之岛 \u63A2\u7D22 100%\uFF01',
            zht:'\u2601\uFE0F 雲之島 \u63A2\u7D22 100%\uFF01',
            ja:'\u2601\uFE0F 雲の島 \u63A2\u7D22 100%\uFF01',
            en:'\u2601\uFE0F Cloud Island Exploration 100%!'
        };
        return cloudCompletionNames[_langCode]||cloudCompletionNames.en;
    }
...
// ---- Travel beacons (stable internal warp-pipe data remains for map compatibility) ----
function buildWarpPipes(){
    warpPipeMeshes.forEach(function(wp){cityGroup.remove(wp.group);});
    warpPipeMeshes=[];
    // No ground travel beacons on the moon area (only reachable from Cloud Island)
    if(currentCityStyle===5)return;
    ...
    for(var pi2=0;pi2<Math.min(targets.length,positions.length);pi2++){
        ...
        var pMat=new THREE.MeshPhysicalMaterial({color:pColor,roughness:0.42,metalness:0.08,transmission:0.12,transparent:true,opacity:0.72});
        // Faceted stone-and-light plinth: deliberately avoids a hollow tube silhouette.
        var base=new THREE.Mesh(new THREE.CylinderGeometry(3.0,3.5,0.65,7),toon(0x6C7480,{}));base.position.y=0.32;g.add(base);
        var inset=new THREE.Mesh(new THREE.CylinderGeometry(2.35,2.55,0.18,7),toon(pColor,{emissive:pColor,emissiveIntensity:0.18,}));inset.position.y=0.72;g.add(inset);
        // Two leaning wayfinder pylons frame an open passage.
        for(var _bp=-1;_bp<=1;_bp+=2){
            var pylon=new THREE.Mesh(new THREE.BoxGeometry(0.62,6.2,0.78),pMat);
            pylon.position.set(_bp*2.25,3.65,0);pylon.rotation.z=-_bp*0.11;g.add(pylon);
            var cap=new THREE.Mesh(new THREE.OctahedronGeometry(0.54,0),toon(pColor,{emissive:pColor,emissiveIntensity:0.38,}));
            cap.position.set(_bp*1.91,6.7,0);g.add(cap);
        }
        // Floating compass crystal and slow light motes mark the destination.
        var beacon=new THREE.Mesh(new THREE.OctahedronGeometry(0.9,1),new THREE.MeshPhysicalMaterial({color:pColor,emissive:pColor,emissiveIntensity:0.42,roughness:0.28,transparent:true,opacity:0.8}));
        beacon.position.y=5.7;g.add(beacon);
        var sMat=new THREE.MeshBasicMaterial({color:pColor,transparent:true,opacity:0.55});
        for(var si=0;si<10;si++){
            var sp=new THREE.Mesh(new THREE.OctahedronGeometry(0.16+0.03*(si%3),0),sMat);
            var a=si/10*Math.PI*2;
            sp.position.set(Math.cos(a)*(1.1+(si%2)*0.35),1.1+(si%5)*0.82,Math.sin(a)*(0.65+(si%3)*0.15));
            sp.rotation.set(a*0.3,a,a*0.2);g.add(sp);
        }
        // Label sign
...
function clearCity(){
    ...
    if(window._danboBabelPreviewTimer){
        clearTimeout(window._danboBabelPreviewTimer);
        window._danboBabelPreviewTimer=0;
    }
    ...
    window._playerTravelProjectile=null;
    // Remove city NPCs
    ...
    // Remove moon survey drones
    if(window._moonDrones){for(var gi=0;gi<window._moonDrones.length;gi++){scene.remove(window._moonDrones[gi].group);}window._moonDrones=null;}
    window._moonClimateZones=null;
    // Remove the city climate-canopy groups from the scene.
    if(window._moonClimateCanopies){for(var _sdi=0;_sdi<window._moonClimateCanopies.length;_sdi++){scene.remove(window._moonClimateCanopies[_sdi].group);}window._moonClimateCanopies=null;}
...
function applyCityTheme(){
    ...
    var _lowVisualQuality=!!(window.DANBO_VISUAL_QUALITY&&DANBO_VISUAL_QUALITY.low);
    scene.environmentIntensity=!isMoon?(_lowVisualQuality?(RENDER_CONFIG.lowEnvironmentIntensity||0.90):(RENDER_CONFIG.environmentIntensity||0.82)):0.28;
    ...
    if(typeof R!=='undefined'){
        R.toneMappingExposure=_lowVisualQuality?(RENDER_CONFIG.lowToneExposure||0.66):(RENDER_CONFIG.toneExposure||0.58);
    }
    ...
    sun.visible=true;
    sun.intensity=isMoon?0:(_lowVisualQuality?(RENDER_CONFIG.lowSunIntensity||5.2):(RENDER_CONFIG.sunIntensity||4.5));
...
function updatePipeTravel(){
    ...
    if(_pipeTimer===Math.floor(_pipeDuration*0.4)){
        ...
        var _pipeCityNames=I18N.cityNames[_langCode]||I18N.cityNames.en;
        if(CITY_STYLES[currentCityStyle]&&_pipeCityNames[currentCityStyle])CITY_STYLES[currentCityStyle].name=_pipeCityNames[currentCityStyle];
        clearCity();
        buildCity();
        buildPortals();
        buildCityCoins();
    ...
    if(_pipeTimer>=_pipeDuration){
        ...
        if(currentCityStyle===5){
            // Moon flat: spawn inside Lunaglow Haven city
        ...
        // original area name reveal after pipe travel
...
function switchCity(targetStyle){
    ...
    var _switchCityNames=I18N.cityNames[_langCode]||I18N.cityNames.en;
    if(CITY_STYLES[currentCityStyle]&&_switchCityNames[currentCityStyle])CITY_STYLES[currentCityStyle].name=_switchCityNames[currentCityStyle];
    ...
    playerEgg=createEgg(0,0,skin.color,skin.accent,true,undefined,skin.id);
    playerEgg.finished=false;playerEgg.alive=true;
    if(currentCityStyle===5){
            // Moon flat: spawn on the open survey plain.
...
function spawnCityNPCs() {
    ...
    for(let i=0;i<npcCount;i++){
        ...
        if(currentCityStyle===5){
            // Moon: half the NPCs inside Lunaglow Haven, half on the survey plain.
            if(i<12){
                // Inside Lunaglow Haven (local coords scaled by 8, center at -200,0)
        ...
        // Weighted character selection: more Spicy Flame Traveler
        var _npcCharIdx=i%CHARACTERS.length;
        if(currentCityStyle===6){
            var _wr=Math.random();
            if(_wr<0.25)_npcCharIdx=6; // 25% Spicy Flame Traveler (bear)
            else _npcCharIdx=Math.floor(Math.random()*CHARACTERS.length);
        }
        const npc=createEgg(nx2,nz2,col,AI_COLORS[(i+4)%AI_COLORS.length],false,undefined,CHARACTERS[_npcCharIdx].id);
...
function _babelTowerDefinition(includeDisabled){
    var def=_citySpecialObject('babelTower')||_citySpecialObject('babel-tower');
    if(!def&&currentCityStyle>=0&&currentCityStyle<=4){
        // Compatibility for old ground-city saves created before the tower was
        // promoted to a declarative functional landmark.
        def={id:'babel-tower',type:'babelTower',x:12,y:0,z:0,w:16,d:16,h:82.8,enabled:true,unlockCoins:10};
    }
    return def&&(includeDisabled||def.enabled!==false)?def:null;
}
function _babelTowerUnlockRequirement(){
    var def=_babelTowerDefinition(false);
    return def?Math.max(0,Math.round(_specialNumber(def,'unlockCoins',10))):Infinity;
}
function _babelTowerShouldTrigger(){
    return currentCityStyle!==5&&!!_babelTowerDefinition(false)&&coins>=_babelTowerUnlockRequirement();
}
...
function addClouds(){
    ...
    // ---- Moon travel beacon at the center of Cloud Island ----
...
function _buildBabylonTower(def,options){
    options=options||{};
    if(_babylonTower)return _babylonTower;
    def=def||_babelTowerDefinition(!!options.editor);
    if(!def||def.enabled===false)return null;
    var tower=def.tower||{},event=def.event||{},elevator=def.elevator||{},cloudExit=def.cloudExit||{},interaction=def.interaction||{};
    var layers=Math.max(8,Math.min(14,Math.round(_specialNumber(tower,'layers',12))));
    var layerH=Math.max(4,Math.min(7,_specialNumber(tower,'layerHeight',6.4)));
    var baseW=Math.max(10,_specialNumber(def,'w',16)),baseD=Math.max(10,_specialNumber(def,'d',16));
    var taper=Math.max(0.5,Math.min(1.1,_specialNumber(tower,'taper',0.82)));
    // Never allow the authored taper to invert upper floors.
    taper=Math.min(taper,(Math.min(baseW,baseD)-4)/Math.max(1,layers-1));
    var objectScale=Math.max(0.8,Math.min(1.25,_specialNumber(def,'scale',1)));
    var rotationY=_specialNumber(def,'rotationY',0)*Math.PI/180;
    var towerX=_specialNumber(def,'x',12),towerY=_specialNumber(def,'y',0),towerZ=_specialNumber(def,'z',0);
    var riseOffset=Number.isFinite(Number(options.riseOffset))?Number(options.riseOffset):_babylonRiseY;
    var editorIndex=Number.isFinite(Number(options.editorSpecialIndex))?Number(options.editorSpecialIndex):_citySpecialObjects().indexOf(def);
    var g=new THREE.Group();g.name='danbo-babel-tower';
    var colors=[0xD8AD6A,0xD0A15B,0xC7944D,0xBC8741,0xB07B36,0xA36E2D,0x956126,0x87541F];
    for(var i=0;i<layers;i++){
        var w=Math.max(4,baseW-i*taper),d=Math.max(4,baseD-i*taper);
        var mat=toon(colors[Math.min(i,colors.length-1)]);
        var mesh=new THREE.Mesh(new THREE.BoxGeometry(w,layerH,d),mat);
        mesh.position.y=i*layerH+layerH/2;mesh.castShadow=true;mesh.receiveShadow=true;g.add(mesh);
        var ledge=new THREE.Mesh(new THREE.BoxGeometry(w+0.6,0.38,d+0.6),toon(colors[Math.max(0,Math.min(i-1,colors.length-1))]));
        ledge.position.y=i*layerH+layerH;ledge.castShadow=true;ledge.receiveShadow=true;g.add(ledge);
    }
    var localTopY=layers*layerH;
    var topW=Math.max(3.2,baseW-(layers-1)*taper);
    var archMat=toon(0x926024);
    var arch1=new THREE.Mesh(new THREE.BoxGeometry(0.8,4,0.8),archMat);
    arch1.position.set(-topW/3,localTopY+2,0);g.add(arch1);
    var arch2=new THREE.Mesh(new THREE.BoxGeometry(0.8,4,0.8),archMat);
    arch2.position.set(topW/3,localTopY+2,0);g.add(arch2);
    var archTop=new THREE.Mesh(new THREE.BoxGeometry(topW*0.8,0.8,1.2),toon(0x744619));
    archTop.position.set(0,localTopY+4,0);g.add(archTop);
    // A translucent energy lift preserves the familiar function without looking
    // like a generic solid pipe.
    var pipeMat=new THREE.MeshPhongMaterial({color:0x65F0A0,transparent:true,opacity:0.32,side:THREE.DoubleSide,depthWrite:false});
    var pipeBody=new THREE.Mesh(new THREE.CylinderGeometry(1.8,1.8,localTopY+2,16,1,true),pipeMat);
    pipeBody.position.y=(localTopY+2)/2;g.add(pipeBody);
    var rimMat=toon(0x55EE99,{emissive:0x1A8C4C,emissiveIntensity:0.45});
    var pipeRim=new THREE.Mesh(new THREE.TorusGeometry(1.8,0.3,8,20),rimMat);
    pipeRim.position.y=0.2;pipeRim.rotation.x=Math.PI/2;g.add(pipeRim);
    var pipeRimTop=pipeRim.clone();pipeRimTop.position.y=localTopY+1;g.add(pipeRimTop);
    var orbMat=new THREE.MeshBasicMaterial({color:0xA4FFD0,transparent:true,opacity:0.58,depthWrite:false});
    var orbCount=(window.DANBO_VISUAL_QUALITY&&DANBO_VISUAL_QUALITY.low)?8:14;
    for(var oi=0;oi<orbCount;oi++){
        var orb=new THREE.Mesh(new THREE.SphereGeometry(0.25,6,4),orbMat);
        var oa=oi/orbCount*Math.PI*6;
        orb.position.set(Math.cos(oa),oi/orbCount*localTopY+1,Math.sin(oa));g.add(orb);
    }
    var arrowMat=toon(0xFFF06A,{emissive:0xE89C24,emissiveIntensity:0.38});
    for(var ai=0;ai<5;ai++){
        var arrow=new THREE.Mesh(new THREE.ConeGeometry(0.6,1.2,6),arrowMat);
        arrow.position.set(0,4+ai*(localTopY-8)/4,0);g.add(arrow);
    }
    var canvas=document.createElement('canvas');canvas.width=256;canvas.height=64;
    var ctx2=canvas.getContext('2d');ctx2.fillStyle='rgba(35,22,12,0.72)';ctx2.fillRect(0,0,256,64);
    ctx2.fillStyle='#FFE08B';ctx2.font='bold 22px sans-serif';ctx2.textAlign='center';
    var towerLabel={zhs:'\u5DF4\u522B\u5854 \u2191 云之岛',zht:'\u5DF4\u5225\u5854 \u2191 雲之島',ja:'\u30D0\u30D9\u30EB\u306E\u5854 \u2191 雲の島',en:'Babel \u2191 Cloud Island'};
    var towerLabelText=towerLabel[_langCode]||towerLabel.en;
    if(ctx2.measureText(towerLabelText).width>236)ctx2.font='bold 20px sans-serif';
    ctx2.fillText(towerLabelText,128,42);
    var tex=new THREE.CanvasTexture(canvas);if(THREE.SRGBColorSpace)tex.colorSpace=THREE.SRGBColorSpace;
    var sign=new THREE.Sprite(new THREE.SpriteMaterial({map:tex,transparent:true,depthWrite:false}));
    sign.scale.set(5,1.2,1);sign.position.set(0,localTopY+6,0);g.add(sign);
    var doorDirs=[{dx:0,dz:1},{dx:0,dz:-1},{dx:1,dz:0},{dx:-1,dz:0}],doors=[];
    for(var di=0;di<4;di++){
        var dd=doorDirs[di],dox=dd.dx*(baseW/2+0.1),doz=dd.dz*(baseD/2+0.1);
        var dFrame=new THREE.Mesh(new THREE.BoxGeometry(dd.dx===0?3.5:0.5,5,dd.dz===0?3.5:0.5),toon(0x69451F));
        dFrame.position.set(dox,2.5,doz);g.add(dFrame);
        var dInner=new THREE.Mesh(new THREE.BoxGeometry(dd.dx===0?2.5:0.3,4,dd.dz===0?2.5:0.3),toon(0x302016));
        dInner.position.set(dox,2,doz);g.add(dInner);
        var dGlow=new THREE.Mesh(new THREE.PlaneGeometry(2,3.5),new THREE.MeshBasicMaterial({color:0x65F0A0,transparent:true,opacity:0.28,side:THREE.DoubleSide,depthWrite:false}));
        dGlow.position.set(dox,2,doz);if(dd.dx!==0)dGlow.rotation.y=Math.PI/2;g.add(dGlow);
        var localX=dox*objectScale,localZ=doz*objectScale;
        doors.push({x:towerX+Math.cos(rotationY)*localX+Math.sin(rotationY)*localZ,z:towerZ-Math.sin(rotationY)*localX+Math.cos(rotationY)*localZ});
    }
    g.position.set(towerX,towerY+riseOffset,towerZ);g.rotation.y=rotationY;g.scale.setScalar(objectScale);
    g.userData.editorSpecialIndex=editorIndex;g.userData.editorSpecialType='babelTower';
    g.traverse(function(child){child.userData.editorSpecialIndex=editorIndex;child.userData.editorSpecialType='babelTower';});
    scene.add(g);
    var worldTopY=towerY+localTopY*objectScale;
    _babylonTower={
        group:g,definition:def,editorSpecialIndex:editorIndex,x:towerX,y:towerY,z:towerZ,baseY:towerY,
        pipeX:towerX,pipeZ:towerZ,topY:worldTopY,baseW:baseW*objectScale,baseD:baseD*objectScale,
        localTopY:localTopY,scale:objectScale,rotationY:rotationY,doors:doors,editorClouds:[],
        interactionRadius:Math.max(2,Math.min(8,_specialNumber(interaction,'radius',4.2))),
        showInstructions:interaction.showInstructions!==false,
        elevatorEnabled:elevator.enabled!==false,
        elevatorSpeed:Math.max(0.15,Math.min(0.8,_specialNumber(elevator,'speed',0.35))),
        riseSpeed:Math.abs(riseOffset)/Math.max(60,Math.max(1,_specialNumber(event,'riseDuration',3))*60),
        _collidersAdded:false
    };
    if(cloudExit.enabled!==false){
        var exitCloud=_makeCloud(towerX,worldTopY+1,towerZ,3,4,3,4,'babel-exit');
        if(exitCloud)_babylonTower.editorClouds.push(exitCloud);
        var bridgeCount=Math.max(0,Math.min(10,Math.round(_specialNumber(cloudExit,'bridgeCloudCount',5))));
        for(var bci=1;bci<=bridgeCount;bci++){
            var localBX=-bci*2.5,localBZ=(Math.random()-0.5)*6;
            var bcx=towerX+Math.cos(rotationY)*localBX+Math.sin(rotationY)*localBZ;
            var bcz=towerZ-Math.sin(rotationY)*localBX+Math.cos(rotationY)*localBZ;
            var bcy=worldTopY-bci*1.5;
            var bridgeCloud=_makeCloud(bcx,bcy,bcz,2,3,2,4,'babel-bridge-'+bci);
            if(bridgeCloud)_babylonTower.editorClouds.push(bridgeCloud);
        }
    }
    return _babylonTower;
}

function _moveBabylonTowerEditorTarget(index,x,y,z){
    var bt=_babylonTower;if(!bt||Number(bt.editorSpecialIndex)!==Number(index))return false;
    var dx=x-bt.x,dy=y-bt.y,dz=z-bt.z;
    bt.x=x;bt.y=y;bt.z=z;bt.baseY=y;bt.pipeX=x;bt.pipeZ=z;bt.topY+=dy;
    bt.group.position.x+=dx;bt.group.position.y+=dy;bt.group.position.z+=dz;
    for(var di=0;di<bt.doors.length;di++){bt.doors[di].x+=dx;bt.doors[di].z+=dz;}
    for(var ci=0;ci<bt.editorClouds.length;ci++){
        var cloud=bt.editorClouds[ci];if(!cloud)continue;
        cloud.x+=dx;cloud.y+=dy;cloud.z+=dz;
        cloud.group.position.x+=dx;cloud.group.position.y+=dy;cloud.group.position.z+=dz;
    }
    return true;
}

function _finalizeBabylonTowerCollision(){
    var bt=_babylonTower;if(!bt||bt._collidersAdded)return;
    bt._collidersAdded=true;
    cityColliders.push({x:bt.x,z:bt.z,hw:bt.baseW/2,hd:bt.baseD/2,h:bt.topY-bt.baseY,_babel:true});
    var babelMeshes=[];bt.group.traverse(function(child){if(child.isMesh)babelMeshes.push(child);});
    cityBuildingMeshes.push({meshes:babelMeshes,x:bt.x,z:bt.z,hw:bt.baseW/2,hd:bt.baseD/2,h:bt.topY-bt.baseY,_babel:true});
}

function _triggerBabylonEvent(){
    if(_babylonTriggered)return;
    var def=_babelTowerDefinition(false);if(!def||currentCityStyle===5)return;
    if(typeof Explorer!=='undefined')Explorer.discoverHidden('babel_tower','\u5DF4\u522B\u5854');
    var event=def.event||{},riseEnabled=event.riseEnabled!==false;
    _babylonTriggered=true;
    if(!riseEnabled){
        _babylonRiseY=0;_babylonRising=false;_buildBabylonTower(def,{riseOffset:0});_finalizeBabylonTowerCollision();return;
    }
    _earthquakeTimer=Math.max(60,Math.round(Math.max(1,_specialNumber(event,'riseDuration',3))*60));
    _earthquakeIntensity=Math.max(0,Math.min(1,_specialNumber(event,'cameraShake',0.5)));
    _babylonRising=true;
    var tower=def.tower||{},layers=Math.max(8,Math.min(14,Math.round(_specialNumber(tower,'layers',12))));
    var layerH=Math.max(4,Math.min(7,_specialNumber(tower,'layerHeight',6.4)));
    var objectScale=Math.max(0.8,Math.min(1.25,_specialNumber(def,'scale',1)));
    _babylonRiseY=-Math.max(30,layers*layerH*objectScale*0.65);
    if(event.rumbleSound!==false)playRumbleSound();
    _buildBabylonTower(def,{riseOffset:_babylonRiseY});
}
