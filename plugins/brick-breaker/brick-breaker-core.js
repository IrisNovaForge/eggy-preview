(function(){
    ...
    var W=960,H=720,PORTRAIT_H=1040,PORTRAIT_Y_START=300,PORTRAIT_Y_END=560,TOUCH_DEAD_ZONE=.16,TOUCH_START_THRESHOLD=.20,TOUCH_STOP_THRESHOLD=.18,TOUCH_RESPONSE_CURVE=1.35,STARTING_LIVES=3,BASE_CATCH_WIDTH=154,CROWN_CATCH_WIDTH=218,CROWN_OPEN_DURATION=.35,CROWN_CLOSE_DURATION=1;
    ...
    var BGM_AUDIO_URL=CORE_ASSET_BASE?new URL('audio/starlight-collision-bgm-simple-retro-2.mp3?v=20260823.1',CORE_ASSET_BASE).href:'';
    ...
    var ITEM_GUIDE_COPY={
        zhs:{title:'本关物件',hazard:{name:'沉壳团',desc:'碰到损失1颗光球',prompt:'避开沉壳团 · 碰到损失1颗光球',result:'光球 −1'},seed:{name:'风之威力',desc:'接住风叶核心后可连续释放7次，本关最多{limit}次',prompt:'接住后可释放风之威力',result:'风之威力已就绪'},life:{name:'元气蛋壳',desc:'光球＋1并获得本关快速移动',prompt:'接住元气蛋壳 · 光球＋1并加快移动',result:'光球＋1 · 快速移动'},slow:{name:'慢风绒',desc:'暂时降低球速',prompt:'接到后暂时降低球速',result:'慢风生效'},clear:{name:'星路芽',desc:'由收集的叶片唤出，接住后清理最多5块砖',prompt:'接住星路芽 · 清理最多5块砖',result:'星路清理5块'},crown:{name:'花冠',desc:'接住后暂时扩大接球范围，本关最多{limit}次',prompt:'接住花冠 · 扩大接球范围',result:'花冠生效 · {used}/{limit}',status:'接球范围扩大 · {used}/{limit} · {seconds}秒'},buff:{name:'叶片收集',desc:'三击砖掉落，接住后唤出星路芽',prompt:'三击砖掉落 · 接住小叶片',result:'叶片汇成星路芽'},multi:{name:'星星点点',desc:'接住后暂时变成2颗光球',prompt:'接住小星星后光球暂时同行',result:'星光同行 · 场上2颗',status:'星光同行 · 场上2颗'}},
        zht:{title:'本關物件',hazard:{name:'沉殼團',desc:'碰到損失1顆光球',prompt:'避開沉殼團 · 碰到損失1顆光球',result:'光球 −1'},seed:{name:'風之威力',desc:'接住風葉核心後可連續釋放7次，本關最多{limit}次',prompt:'接住後可釋放風之威力',result:'風之威力已就緒'},life:{name:'元氣蛋殼',desc:'光球＋1並獲得本關快速移動',prompt:'接住元氣蛋殼 · 光球＋1並加快移動',result:'光球＋1 · 快速移動'},slow:{name:'慢風絨',desc:'暫時降低球速',prompt:'接到後暫時降低球速',result:'慢風生效'},clear:{name:'星路芽',desc:'由收集的葉片喚出，接住後清理最多5塊磚',prompt:'接住星路芽 · 清理最多5塊磚',result:'星路清理5塊'},crown:{name:'花冠',desc:'接住後暫時擴大接球範圍，本關最多{limit}次',prompt:'接住花冠 · 擴大接球範圍',result:'花冠生效 · {used}/{limit}',status:'接球範圍擴大 · {used}/{limit} · {seconds}秒'},buff:{name:'葉片收集',desc:'三擊磚掉落，接住後喚出星路芽',prompt:'三擊磚掉落 · 接住小葉片',result:'葉片匯成星路芽'},multi:{name:'星星點點',desc:'接住後暫時變成2顆光球',prompt:'接住小星星後光球暫時同行',result:'星光同行 · 場上2顆',status:'星光同行 · 場上2顆'}},
        ja:{title:'このステージのアイテム',hazard:{name:'沈み殻の房',desc:'触れるとボールを1つ失う',prompt:'沈み殻の房をよけよう · ボール−1',result:'ボール −1'},seed:{name:'風の力',desc:'風葉コアで7回連続放出・このステージは最大{limit}回',prompt:'受け取ると風の力を放てる',result:'風の力の準備完了'},life:{name:'元気の卵殻',desc:'ボール＋1・このステージ中は高速移動',prompt:'元気の卵殻 · ボール＋1と高速移動',result:'ボール＋1 · 高速移動'},slow:{name:'そよ風の綿',desc:'一時的に速度を下げる',prompt:'受け取ると一時的に速度が下がる',result:'そよ風が発動'},clear:{name:'星路の芽',desc:'集めた葉から生まれ、受け取ると最大5個を清掃',prompt:'星路の芽を受け取る · 最大5個を清掃',result:'星路が5個を清掃'},crown:{name:'花冠',desc:'一時的に受け止める幅が広がる・このステージは最大{limit}回',prompt:'花冠 · 受け止める幅を広げる',result:'花冠が発動 · {used}/{limit}',status:'受け止める幅が拡大中 · {used}/{limit} · {seconds}秒'},buff:{name:'葉っぱ集め',desc:'3回ブロックから落下・受け取ると星路の芽を呼ぶ',prompt:'3回ブロックから落下 · 小さな葉を受け取る',result:'葉が星路の芽になる'},multi:{name:'星のきらめき',desc:'受け取ると一時的にボールが2つになる',prompt:'小さな星を受け取ると光球が一時同行',result:'星光同行 · 2つ',status:'星光同行 · 2つ'}},
        en:{title:'Stage Items',hazard:{name:'Sunk-shell Cluster',desc:'Lose 1 ball on contact',prompt:'Avoid the sunk-shell cluster · Ball −1',result:'Ball −1'},seed:{name:'Wind Power',desc:'Catch the wind-leaf core for a 7-shot stream, up to {limit} times this stage',prompt:'Catch it to ready Wind Power',result:'Wind Power ready'},life:{name:'Vitality Shell',desc:'Ball +1 and fast movement for this stage',prompt:'Catch it · Ball +1 and faster movement',result:'Ball +1 · Fast movement'},slow:{name:'Breeze Fluff',desc:'Temporarily slows the ball',prompt:'Catch to slow the ball briefly',result:'Breeze active'},clear:{name:'Star-path Bud',desc:'Grown from a collected leaf; catch it to clear up to 5 bricks',prompt:'Catch the Star-path Bud · clear up to 5 bricks',result:'Star path cleared 5 bricks'},crown:{name:'Flower Crown',desc:'Temporarily widens the catch area, up to {limit} times this stage',prompt:'Catch the flower crown · widen the catch area',result:'Flower crown active · {used}/{limit}',status:'Catch area widened · {used}/{limit} · {seconds}s'},buff:{name:'Leaf Collection',desc:'Drops from 3-hit bricks; catch it to call a Star-path Bud',prompt:'From 3-hit bricks · catch the small leaf',result:'Leaf grew into a Star-path Bud'},multi:{name:'Scattered Starlight',desc:'Temporarily makes 2 light balls',prompt:'Catch the small stars for brief light-ball company',result:'Starlight company · 2 active',status:'Starlight company · 2 active'}}
    };
    ...
    var COPY={
        zhs:{name:'星光碰撞',sub:'接住光球，完成挑战！',start:'开始挑战',resume:'继续',restart:'重新开始',exit:'返回奇境',score:'得分',best:'最佳',lives:'光球',left:'剩余',seed:'风之威力',seedReady:'就绪',seedWaiting:'等待',seedSpent:'用完',attack:'释放威力',ready:'准备好！',readyHint:'按空格、点击画面或轻触按钮发球',pause:'暂停',paused:'旅程暂停',won:'星光清扫完成',lost:'光球用完了',again:'再来一次',title:'返回标题',controls:'← → / A D 移动蛋宝；手机使用左下摇杆左右移动。',seedControls:'E / 手机版「击」按钮使用接住的风叶核心。',fourthBasic:'第四关 · 弹来弹去 · 最多6次风之威力，每次最多清理3块砖',basic:'基础玩法 · 清空全部砖块通关',launch:'发球'},
        zht:{name:'星光碰撞',sub:'接住光球，完成挑戰！',start:'開始挑戰',resume:'繼續',restart:'重新開始',exit:'返回奇境',score:'得分',best:'最佳',lives:'光球',left:'剩餘',seed:'風之威力',seedReady:'就緒',seedWaiting:'等待',seedSpent:'用完',attack:'釋放威力',ready:'準備發球',readyHint:'按空白鍵、點擊畫面或輕觸按鈕發球',pause:'暫停',paused:'旅程暫停',won:'方塊全部清理完成！',lost:'光球用完了',again:'再來一次',title:'返回標題',controls:'← → / A D 移動蛋寶；手機使用左下搖桿左右移動。',seedControls:'E / 手機版「击」按鈕使用接住的風葉核心。',fourthBasic:'第四關 · 彈來彈去 · 最多6次風之威力，每次最多清理3塊磚',basic:'基礎玩法 · 清空全部磚塊通關',launch:'發球'},
        ja:{name:'星明かりの衝突',sub:'光のボールを受け止め、チャレンジを達成しよう！',start:'チャレンジ開始',resume:'つづける',restart:'もう一度',exit:'世界へ戻る',score:'スコア',best:'ベスト',lives:'ボール',left:'のこり',seed:'風の力',seedReady:'準備OK',seedWaiting:'待機',seedSpent:'終了',attack:'パワーを放つ',ready:'サーブの準備',readyHint:'スペース、画面クリック、またはボタンでスタート',pause:'一時停止',paused:'一時停止中',won:'すべてのブロックを消しました！',lost:'ボールがなくなりました',again:'もう一度',title:'タイトルへ',controls:'← → / A D で移動。スマホは左下のスティックで左右に移動。',seedControls:'E またはモバイルの「击」ボタンで、受け取った風葉コアを使います。',fourthBasic:'ステージ4 · やわらか反射 · 風の力は最大6回',basic:'基本ルール · すべてのブロックを消すとクリア',launch:'スタート'},
        en:{name:'Starlight Collision',sub:'Catch the light ball and complete the challenge!',start:'Start Challenge',resume:'Resume',restart:'Restart',exit:'Return to World',score:'Score',best:'Best',lives:'Balls',left:'Left',seed:'Wind Power',seedReady:'Ready',seedWaiting:'Waiting',seedSpent:'Spent',attack:'Release Power',ready:'Ready to Serve',readyHint:'Press Space, click the board, or tap the button',pause:'Pause',paused:'Journey Paused',won:'All blocks cleared!',lost:'No light balls left',again:'Play Again',title:'Back to Title',controls:'Move with ← → / A D. On mobile, use the lower-left stick to move sideways.',seedControls:'Press E or the mobile Hit button to use the caught wind-leaf core.',fourthBasic:'Stage 4 · Soft deflection · Up to 6 Wind Power uses',basic:'Basic rules · Clear every brick to finish',launch:'Launch'}
    };
    ...
    Game.prototype.applyPresentationCoordinates=function(){
        ...
        [this.paddle,this.ball,this.companionBalls,this.bricks,this.hitEffects,this.hazard,this.earlyLifeDrop,this.catchCrownDrops,this.catchCrownCollectEffects,
         this.stageThreeSlowDrop,this.stageThreeGatherEffects,this.stageThreeCollectEffects,this.seedDrop,this.seedReleaseEcho,this.seedProjectiles,this.seedBursts,
    ...
    Game.prototype.itemGuideHtml=function(){
        ...
        for(var i=0;i<types.length;i++){var type=types[i],copy=this.itemGuideText[type],limit=type==='seed'?this.seedLimit:this.catchCrownLimit,desc=copy.desc.replace('{limit}',String(limit));html+='<article data-guide-item="'+esc(type)+'"><i aria-hidden="true">'+esc(ITEM_GUIDE_ICONS[type])+'</i><span><b>'+esc(copy.name)+'</b><small>'+esc(desc)+'</small></span></article>';}
    ...
    Game.prototype.showItemResult=function(type,x,y,label){
        if(!this.itemGuideText[type])return false;
        this.itemResultEffects.push({type:type,x:x===undefined?this.paddle.x:x,y:y===undefined?this.paddle.y-42:y,label:label||'',age:0,duration:.88});return true;
    };

    Game.prototype.catchCrownCopy=function(template){return String(template||'').replace('{used}',String(this.catchCrownUses||0)).replace('{limit}',String(this.catchCrownLimit||0)).replace('{seconds}',String(Math.max(0,Math.ceil(this.catchCrownTime||0))));};
    ...
    Game.prototype.itemWhisperCandidate=function(){
        ...
        if(!active.length){
            if(this.catchCrownTime>0)return {type:'crown',kind:'status',detail:this.catchCrownCopy(this.itemGuideText.crown.status)};
            return this.level===6&&this.companionBalls.length?{type:'multi',kind:'status'}:null;
        }
    ...
    Game.prototype.updateItemWhisper=function(){
        ...
        if(this.itemWhisperKey!==key){
            ...
            var icon=element.querySelector('[data-item-whisper-icon]'),name=element.querySelector('[data-item-whisper-name]');
            if(icon)icon.textContent=ITEM_GUIDE_ICONS[candidate.type]||'·';
            if(name)name.textContent=copy.name;
            this.itemWhisperKey=key;
        }
        var detail=element.querySelector('[data-item-whisper-copy]');if(detail)detail.textContent=candidate.detail||(candidate.kind==='result'?copy.result:(candidate.kind==='status'?(copy.status||copy.result):(copy.prompt||copy.desc)));
    ...
    Game.prototype.resetBoard=function(){
        ...
        this.paddle={x:W*0.5,y:H-100,w:BASE_CATCH_WIDTH,h:22,speed:690,baseSpeed:690,fastSpeed:980,controlVx:0};
        ...
        this.catchCrownTime=0;this.catchCrownDuration=12;this.catchCrownAge=0;this.catchCrownSpread=0;this.catchCrownImpact=0;this.catchCrownContact=0;
        ...
        for(var cellIndex=0;cellIndex<cells.length;cellIndex++){
            ...
            var cell=cells[cellIndex],key=cell.x+','+cell.y,stageFiveTriple=this.level===5&&stageFiveTripleCells[key]===true,stageSixTriple=this.level===6&&stageSixTripleCells[key]===true,stageSixCrown=this.level===6&&stageSixCrownCells[key]===true,triple=stageFiveTriple||stageSixTriple;
    ...
    Game.prototype.launchSeedAttack=function(){
        ...
        this.seedReleaseEcho={x:this.paddle.x,y:this.paddle.y-108,age:0,duration:.24};
    ...
    Game.prototype.addSeedBrickFeedback=function(brick,impactX,impactY){
        this.seedBursts.push({
            x:impactX,y:impactY,brickX:brick.x,baseY:brick.y,w:brick.w,h:brick.h,
            row:brick.row,col:brick.col,color:brick.color,age:0,duration:.3
        });
        this.playSoftCollision('brick');
    };

    Game.prototype.updateSeedSystem=function(dt){
        if(this.state!=='playing')return;
        if(this.seedReleaseEcho){this.seedReleaseEcho.age+=dt;if(this.seedReleaseEcho.age>=this.seedReleaseEcho.duration)this.seedReleaseEcho=null;}
    ...
    Game.prototype.registerCatchCrownOpportunity=function(source,label){
        if(this.catchCrownEligible>=this.catchCrownLimit)return false;
        this.catchCrownEligible++;
        if(source)this.catchCrownSources.push({x:source.x+(source.w||0)*.5,y:source.y+(source.h||0)*.5,source:label||('stage'+this.level)});
        return true;
    };

    Game.prototype.updateCatchCrownEligibility=function(){
        if(this.level===6)return;
        var marks=this.level===4?CROWN_TIME_MARKS[4]:(CROWN_PROGRESS_MARKS[this.level]||[]),measure=this.level===4?this.catchCrownClock:(this.catchCrownInitialBricks?1-this.remaining/this.catchCrownInitialBricks:0);
        while(this.catchCrownTriggerIndex<marks.length&&measure>=marks[this.catchCrownTriggerIndex]){this.catchCrownTriggerIndex++;this.registerCatchCrownOpportunity(null,this.level===4?'stage4-timed':('stage'+this.level+'-progress'));}
    };

    Game.prototype.spawnCatchCrownDrop=function(){
        if(this.catchCrownDrops.length||this.catchCrownTime>0||this.catchCrownUses>=this.catchCrownLimit||this.catchCrownUses>=this.catchCrownEligible)return false;
        var point=this.catchCrownSources.length?this.catchCrownSources.shift():null;
        if(!point){var candidates=this.bricks.filter(function(brick){return brick.alive;});if(!candidates.length)return false;var brick=candidates[Math.floor(Math.random()*candidates.length)];point={x:brick.x+brick.w*.5,y:brick.y+brick.h*.5,source:this.level===4?'stage4-timed':('stage'+this.level+'-progress')};}
        var drop={type:'crown',source:point.source,x:point.x,y:point.y,r:13,speed:112,age:0,sourceX:point.x,sourceY:point.y};
        this.catchCrownDrops.push(drop);this.showItemHint('crown',drop);return true;
    };

    Game.prototype.collectCatchCrownDrop=function(drop,index){
        if(!drop)return false;
        if(index===undefined)index=this.catchCrownDrops.indexOf(drop);if(index>=0)this.catchCrownDrops.splice(index,1);
        this.catchCrownUses=Math.min(this.catchCrownLimit,this.catchCrownUses+1);this.activateCatchCrown(drop);this.catchCrownCollectEffects.push({x:drop.x,y:this.paddle.y-20,age:0,duration:.72});this.showItemResult('crown',drop.x,this.paddle.y-54,this.catchCrownCopy(this.itemGuideText.crown.result));
        ...
        if(this.options.onEvent)this.options.onEvent('catchCrownCollected',{level:this.level,duration:this.catchCrownDuration,width:this.paddle.w,source:drop.source,used:this.catchCrownUses,limit:this.catchCrownLimit});
        return true;
    };

    Game.prototype.updateCatchCrownDrops=function(dt){
        if(this.state!=='playing')return;
        this.catchCrownClock+=Math.max(0,dt||0);this.updateCatchCrownEligibility();
        if(this.catchCrownTime>0){
            this.catchCrownAge+=dt;this.catchCrownImpact=Math.max(0,this.catchCrownImpact-dt);
            this.catchCrownTime=Math.max(0,this.catchCrownTime-dt);if(this.catchCrownTime===0){this.deactivateCatchCrown();this.catchCrownRetryAt=Math.max(this.catchCrownRetryAt,this.catchCrownClock+2);}else this.updateCatchCrownSpan();
        }
        var catchRect=this.stageFiveCatchRect();
        for(var dropIndex=this.catchCrownDrops.length-1;dropIndex>=0;dropIndex--){
            var drop=this.catchCrownDrops[dropIndex];drop.age+=dt;drop.y+=drop.speed*dt;
            if(this.rules.circleRectHit(drop.x,drop.y,drop.r,catchRect)){this.collectCatchCrownDrop(drop,dropIndex);continue;}
            if(drop.y-drop.r>H+12){this.catchCrownDrops.splice(dropIndex,1);this.catchCrownRetryAt=this.catchCrownClock+6;}
        }
        if(!this.catchCrownDrops.length&&this.catchCrownTime<=0&&this.catchCrownClock>=this.catchCrownRetryAt)this.spawnCatchCrownDrop();
        for(var effectIndex=this.catchCrownCollectEffects.length-1;effectIndex>=0;effectIndex--){var effect=this.catchCrownCollectEffects[effectIndex];effect.age+=dt;if(effect.age>=effect.duration)this.catchCrownCollectEffects.splice(effectIndex,1);}
    };

    Game.prototype.activateCatchCrown=function(drop){
        if(!this.paddle)return false;
        this.catchCrownTime=this.catchCrownDuration;this.catchCrownAge=0;this.catchCrownSpread=0;this.catchCrownImpact=0;this.catchCrownContact=0;
        this.paddle.w=BASE_CATCH_WIDTH;this.paddle.x=clamp(this.paddle.x,26+this.paddle.w*.5,W-26-this.paddle.w*.5);
        return true;
    };

    Game.prototype.updateCatchCrownSpan=function(){
        if(!this.paddle||this.catchCrownTime<=0)return false;
        var opening=clamp(this.catchCrownAge/CROWN_OPEN_DURATION,0,1),closing=clamp(this.catchCrownTime/CROWN_CLOSE_DURATION,0,1);
        opening=opening*opening*(3-2*opening);closing=closing*closing*(3-2*closing);this.catchCrownSpread=Math.min(opening,closing);
        this.paddle.w=BASE_CATCH_WIDTH+(CROWN_CATCH_WIDTH-BASE_CATCH_WIDTH)*this.catchCrownSpread;
        this.paddle.x=clamp(this.paddle.x,26+this.paddle.w*.5,W-26-this.paddle.w*.5);return true;
    };

    Game.prototype.deactivateCatchCrown=function(){
        this.catchCrownTime=0;this.catchCrownSpread=0;this.catchCrownImpact=0;this.paddle.w=BASE_CATCH_WIDTH;this.paddle.x=clamp(this.paddle.x,26+this.paddle.w*.5,W-26-this.paddle.w*.5);
    };
    ...
    Game.prototype.physicsBallStep=function(b,dt){
        ...
        if(b.vy>0&&this.rules.circleRectHit(b.x,b.y,b.r,pr)){
            ...
            if(this.catchCrownTime>0){this.catchCrownImpact=.18;this.catchCrownContact=clamp((b.x-p.x)/(p.w*.5),-1,1);}
    ...
    Game.prototype.drawSoftSeed=function(c,x,y,scale,rotation,alpha){
        ...
        var aura=c.createRadialGradient(0,0,2,0,0,23);aura.addColorStop(0,'rgba(224,253,226,.62)');aura.addColorStop(.58,'rgba(174,229,202,.24)');aura.addColorStop(1,'rgba(174,229,202,0)');c.fillStyle=aura;c.beginPath();c.arc(0,0,23,0,Math.PI*2);c.fill();
        function windLeaf(side,yOffset,fill){c.save();c.translate(side*8,yOffset);c.shadowColor='rgba(61,139,105,.34)';c.shadowBlur=8;c.fillStyle=fill;c.strokeStyle='#3f8e70';c.lineWidth=1.35;c.beginPath();c.moveTo(side*11,0);c.lineTo(side*5,-6);c.lineTo(-side*5,-4);c.lineTo(-side*8,1);c.lineTo(-side*4,6);c.lineTo(side*6,5);c.closePath();c.fill();c.shadowColor='transparent';c.stroke();c.strokeStyle='rgba(53,124,88,.76)';c.lineWidth=.9;c.beginPath();c.moveTo(-side*6,1);c.lineTo(side*8,0);c.stroke();c.restore();}
        windLeaf(-1,1,'#9fd5a6');windLeaf(1,-1,'#bce3a7');
        c.save();c.shadowColor='rgba(123,211,174,.55)';c.shadowBlur=10;var core=c.createLinearGradient(-8,-10,9,10);core.addColorStop(0,'#e8f4a9');core.addColorStop(.5,'#79c89e');core.addColorStop(1,'#3d9a83');c.fillStyle=core;c.strokeStyle='#347d68';c.lineWidth=1.55;c.beginPath();c.moveTo(0,-11);c.lineTo(8,-6);c.lineTo(10,3);c.lineTo(4,10);c.lineTo(-6,8);c.lineTo(-10,-2);c.lineTo(-5,-8);c.closePath();c.fill();c.shadowColor='transparent';c.stroke();
        c.globalAlpha=(alpha===undefined?1:alpha)*.86;c.strokeStyle='#f7f0a8';c.lineWidth=1.35;c.lineCap='round';c.beginPath();c.moveTo(-5,-2);c.quadraticCurveTo(0,-6,5,-3);c.quadraticCurveTo(1,-1,-1,3);c.quadraticCurveTo(2,5,6,4);c.stroke();
        c.fillStyle='rgba(255,251,189,.94)';c.beginPath();c.arc(-3,-5,1.45,0,Math.PI*2);c.fill();c.restore();c.restore();
    };
    Game.prototype.drawSeedSystem=function(c){
        ...
        if(this.seedHeld){
            var heldY=this.paddle.y-108+Math.sin(this.elapsed*5)*2;
            this.drawSoftSeed(c,this.paddle.x,heldY,1.05,Math.sin(this.elapsed*3)*.065,1);
        }
        if(this.seedReleaseEcho){
            var release=this.seedReleaseEcho,releaseProgress=clamp(release.age/release.duration,0,1),releaseFade=Math.pow(1-releaseProgress,2),releaseScale=1.14-releaseProgress*.16;
            this.drawSoftSeed(c,release.x,release.y,releaseScale,-.06+releaseProgress*.04,releaseFade);
        }
        for(var burstIndex=0;burstIndex<this.seedBursts.length;burstIndex++){
            ...
            var vanish=clamp((progress-.18)/.82,0,1),brickScale=1-vanish*.42,brickAlpha=Math.pow(1-vanish,2);
            var centerX=burst.brickX+burst.w*.5,centerY=burst.baseY+burst.h*.5;
            c.save();c.translate(centerX,centerY);c.scale(brickScale,brickScale);c.globalAlpha=brickAlpha;
            c.shadowColor='rgba(255,247,205,.58)';c.shadowBlur=12;
            this.brickShell(c,-burst.w*.5,-burst.h*.5,burst.w,burst.h,burst.color);c.shadowColor='transparent';
            var echoShine=c.createLinearGradient(0,-burst.h*.5,0,burst.h*.5);echoShine.addColorStop(0,'rgba(255,255,245,.9)');echoShine.addColorStop(.52,'rgba(255,246,205,.34)');echoShine.addColorStop(1,'rgba(255,255,255,.04)');
            this.brickShell(c,-burst.w*.5+1.5,-burst.h*.5+1.5,burst.w-3,burst.h-3,echoShine);
            c.globalAlpha=brickAlpha*(.72+.28*Math.sin(progress*Math.PI));this.brickShell(c,-burst.w*.5+4,-burst.h*.5+4,burst.w-8,burst.h-8,null,'rgba(255,255,238,.96)',1.8);c.restore();
            var ringRadius=5+progress*29,ringAlpha=fade*.82;
            c.save();c.translate(burst.x,burst.y);c.globalAlpha=ringAlpha;c.strokeStyle='rgba(255,248,211,.96)';c.lineWidth=1.4+fade*.9;c.lineCap='round';c.lineJoin='round';c.shadowColor='rgba(255,232,166,.46)';c.shadowBlur=8;
            c.beginPath();c.moveTo(0,-ringRadius*.88);c.bezierCurveTo(ringRadius*.7,-ringRadius*.9,ringRadius*1.02,-ringRadius*.25,ringRadius*.92,ringRadius*.34);c.bezierCurveTo(ringRadius*.8,ringRadius*.84,ringRadius*.3,ringRadius,0,ringRadius*.96);c.bezierCurveTo(-ringRadius*.42,ringRadius,-ringRadius*.96,ringRadius*.68,-ringRadius*.94,ringRadius*.16);c.bezierCurveTo(-ringRadius*1.02,-ringRadius*.38,-ringRadius*.58,-ringRadius*.86,0,-ringRadius*.88);c.stroke();c.restore();
        }
    };
    ...
    Game.prototype.drawCatchCrownDrop=function(c,drop){
        var bob=Math.sin(drop.age*5.2)*1.2,tilt=Math.sin(drop.age*3.1)*.07;c.save();c.translate(drop.x,drop.y+bob);c.rotate(tilt);
        var aura=c.createRadialGradient(0,1,1,0,1,22);aura.addColorStop(0,'rgba(225,250,235,.72)');aura.addColorStop(1,'rgba(225,250,235,0)');c.fillStyle=aura;c.beginPath();c.arc(0,1,22,0,Math.PI*2);c.fill();
        function crownLeaf(side){c.save();c.translate(side*7,2);c.shadowColor='rgba(87,151,126,.34)';c.shadowBlur=9;c.fillStyle=side<0?'#cde9c7':'#bde0bd';c.strokeStyle='#659a79';c.lineWidth=1.25;c.beginPath();c.moveTo(side*10,0);c.lineTo(side*5,-5);c.lineTo(-side*5,-3);c.lineTo(-side*8,0);c.lineTo(-side*5,4);c.lineTo(side*5,5);c.closePath();c.fill();c.shadowColor='transparent';c.stroke();c.strokeStyle='rgba(71,132,89,.72)';c.lineWidth=.9;c.beginPath();c.moveTo(-side*6,0);c.lineTo(side*7,0);c.stroke();c.restore();}
        crownLeaf(-1);crownLeaf(1);
        c.save();c.translate(0,-4);c.shadowColor='rgba(225,245,168,.46)';c.shadowBlur=8;c.fillStyle='#edf2a9';c.strokeStyle='#659a79';c.lineWidth=1.25;c.beginPath();c.moveTo(0,-10);c.lineTo(5,-5);c.lineTo(6,2);c.lineTo(3,6);c.lineTo(-3,6);c.lineTo(-6,2);c.lineTo(-5,-5);c.closePath();c.fill();c.shadowColor='transparent';c.stroke();c.strokeStyle='rgba(105,151,94,.7)';c.lineWidth=.85;c.beginPath();c.moveTo(0,-8);c.lineTo(0,3);c.stroke();c.restore();
        c.fillStyle='#fff7df';c.strokeStyle='#8bb59a';c.lineWidth=1.1;c.beginPath();c.moveTo(-7,4);c.lineTo(-4,2);c.lineTo(0,5);c.lineTo(4,2);c.lineTo(7,4);c.lineTo(5,9);c.lineTo(-5,9);c.closePath();c.fill();c.stroke();c.restore();
    };

    Game.prototype.drawCatchCrownItems=function(c){
        for(var dropIndex=0;dropIndex<this.catchCrownDrops.length;dropIndex++){
            var drop=this.catchCrownDrops[dropIndex];c.save();c.globalAlpha=.22;c.strokeStyle='#71aa94';c.lineWidth=1.3;c.beginPath();c.moveTo(drop.x,drop.y-19);c.quadraticCurveTo(drop.sourceX,drop.y-31,drop.sourceX,Math.max(drop.sourceY,drop.y-54));c.stroke();c.restore();this.drawCatchCrownDrop(c,drop);
        }
        for(var effectIndex=0;effectIndex<this.catchCrownCollectEffects.length;effectIndex++){
            var effect=this.catchCrownCollectEffects[effectIndex],progress=clamp(effect.age/effect.duration,0,1),fade=Math.pow(1-progress,2);c.save();c.translate(effect.x,effect.y-progress*28);c.globalAlpha=fade;c.fillStyle='#4f8977';c.font='800 19px system-ui,sans-serif';c.textAlign='center';c.textBaseline='middle';c.fillText('⌒·⌒',0,0);c.restore();
        }
    };
    Game.prototype.drawCatchCrownEquipment=function(c){
        ...
        var reach=BASE_CATCH_WIDTH*.5+(CROWN_CATCH_WIDTH-BASE_CATCH_WIDTH)*.5*open,sideCenter=reach-24;
        c.save();c.translate(p.x,p.y-10);
        if(open>0){var glow=c.createRadialGradient(0,0,4,0,0,reach);glow.addColorStop(0,'rgba(234,255,240,'+(.12*ending)+')');glow.addColorStop(.78,'rgba(205,242,222,'+(.1*open*ending)+')');glow.addColorStop(1,'rgba(205,242,222,0)');c.fillStyle=glow;c.beginPath();c.ellipse(0,0,reach,20,0,0,Math.PI*2);c.fill();}
        c.globalAlpha=.34+open*.46;c.strokeStyle=open>0?'#b6e0b6':'#6f957b';c.lineWidth=1.8+open*.6;c.beginPath();c.moveTo(-reach,1);c.lineTo(reach,1);c.stroke();c.globalAlpha=1;
        function crownLeaf(x,y,w,h,side){
            var localImpact=impact*(contact<-.24&&side<0||contact>.24&&side>0?1:.42);
            c.save();c.translate(x,y+localImpact*2.5);c.scale(1+localImpact*.06,1-localImpact*.2);c.globalAlpha=.84+open*.16;
            if(open>0){c.shadowColor='rgba(94,166,137,'+(.16+open*.12+localImpact*.28)+')';c.shadowBlur=5+open*5+localImpact*5;}
            var fill=c.createLinearGradient(0,-h*.5,0,h*.5);fill.addColorStop(0,localImpact?'#ffffff':(open>0?'#f7f7d8':'#dce5b7'));fill.addColorStop(1,open>0?'#b8ddc1':'#90b79a');c.fillStyle=fill;c.strokeStyle=open>0?'#6fa58b':'#698d7b';c.lineWidth=1.25;
            c.beginPath();if(side<0){c.moveTo(-w*.5,0);c.lineTo(-w*.29,-h*.52);c.lineTo(w*.38,-h*.34);c.lineTo(w*.5,0);c.lineTo(w*.36,h*.38);c.lineTo(-w*.24,h*.52);}else{c.moveTo(w*.5,0);c.lineTo(w*.29,-h*.52);c.lineTo(-w*.38,-h*.34);c.lineTo(-w*.5,0);c.lineTo(-w*.36,h*.38);c.lineTo(w*.24,h*.52);}c.closePath();c.fill();c.shadowColor='transparent';c.stroke();
            c.globalAlpha*=.72;c.strokeStyle=open>0?'rgba(82,139,94,.78)':'rgba(82,122,91,.68)';c.lineWidth=.9;c.beginPath();c.moveTo(-side*w*.4,0);c.lineTo(side*w*.43,0);c.stroke();c.restore();
        }
        function centerBud(){
            var localImpact=impact*.7;c.save();c.translate(0,-4+localImpact*2);c.scale(1+localImpact*.04,1-localImpact*.15);c.globalAlpha=.88+open*.12;
            if(open>0){c.shadowColor='rgba(225,245,168,'+(.2+.24*open)+')';c.shadowBlur=6+open*5;}
            c.fillStyle=open>0?'#edf2a9':'#c5d79a';c.strokeStyle=open>0?'#6fa079':'#6f8973';c.lineWidth=1.2;c.beginPath();c.moveTo(0,-11);c.lineTo(6,-5);c.lineTo(7,2);c.lineTo(4,7);c.lineTo(-4,7);c.lineTo(-7,2);c.lineTo(-6,-5);c.closePath();c.fill();c.shadowColor='transparent';c.stroke();
            c.strokeStyle='rgba(91,139,87,.68)';c.lineWidth=.85;c.beginPath();c.moveTo(0,-9);c.lineTo(0,4);c.stroke();
            c.fillStyle=open>0?'#fff8df':'#e9e6cf';c.strokeStyle=open>0?'#91b69a':'#829889';c.lineWidth=1;c.beginPath();c.moveTo(-9,4);c.lineTo(-5,1);c.lineTo(0,5);c.lineTo(5,1);c.lineTo(9,4);c.lineTo(6,9);c.lineTo(-6,9);c.closePath();c.fill();c.stroke();c.restore();
        }
        crownLeaf(-sideCenter,1,48,12,-1);centerBud();crownLeaf(sideCenter,1,48,12,1);
        if(open>0){
            var tipPulse=this.catchCrownTime<1 ? .45+.55*(.5+.5*Math.cos(this.catchCrownTime*Math.PI*4)) : 1;c.globalAlpha=open*ending*tipPulse;
            for(var tipSide=-1;tipSide<=1;tipSide+=2){c.save();c.translate(tipSide*reach,0);c.shadowColor='rgba(223,248,169,.72)';c.shadowBlur=8;c.fillStyle='#e9f4a8';c.strokeStyle='rgba(93,151,111,.8)';c.lineWidth=.9;c.beginPath();c.moveTo(tipSide*5.5,0);c.bezierCurveTo(tipSide*2.6,-3.1,tipSide*-2.8,-3.1,tipSide*-5.5,0);c.bezierCurveTo(tipSide*-2.8,3.1,tipSide*2.6,3.1,tipSide*5.5,0);c.closePath();c.fill();c.shadowColor='transparent';c.stroke();c.restore();}
        }
        c.restore();
    };
    ...
    Game.prototype.drawItemNotices=function(c){
        ...
        for(var resultIndex=0;resultIndex<this.itemResultEffects.length;resultIndex++){
            var result=this.itemResultEffects[resultIndex],resultCopy=this.itemGuideText[result.type],resultLabel=result.label||resultCopy.result,resultColors=ITEM_NOTICE_COLORS[result.type]||ITEM_NOTICE_COLORS.seed,resultProgress=clamp(result.age/result.duration,0,1),resultFade=Math.pow(1-resultProgress,2);
            c.save();c.font='900 12px system-ui,sans-serif';var resultWidth=Math.max(72,c.measureText(resultLabel).width+28),resultY=result.y-resultProgress*24,resultX=clamp(result.x,resultWidth*.5+20,W-resultWidth*.5-20);c.globalAlpha=resultFade;c.shadowColor='rgba(48,79,72,.18)';c.shadowBlur=9;this.roundRect(c,resultX-resultWidth*.5,resultY-14,resultWidth,28,12,'rgba(255,255,255,.92)',resultColors.soft);c.shadowColor='transparent';c.fillStyle=resultColors.ink;c.textAlign='center';c.textBaseline='middle';c.fillText(resultLabel,resultX,resultY);c.restore();
        }
    };
    ...
    Game.prototype.render=function(){
        ...
        for(var j=0;j<this.bricks.length;j++){
            ...
            if(br.buffCarrier||br.multiCarrier){
                if(br.hitsRemaining===3){
                    ...
                    c.globalAlpha=.42;this.brickShell(c,br.x+8,br.y+7,br.w-16,br.h-14,null,'rgba(238,149,166,.86)',1.3);
                }else if(br.hitsRemaining===2){
                    c.globalAlpha=.75;this.brickShell(c,br.x+5,br.y+5,br.w-10,br.h-10,null,'rgba(177,132,203,.9)',2);
                }else{
                    c.globalAlpha=.5;this.brickShell(c,br.x+2,br.y+2,br.w-4,br.h-4,'rgba(251,241,255,.92)');
                ...
                c.globalAlpha=.9;this.brickShell(c,br.x+5,br.y+5,br.w-10,br.h-10,null,'rgba(255,255,244,.9)',2.1);
                c.globalAlpha=.35;this.brickShell(c,br.x+8,br.y+8,br.w-16,br.h-16,null,'rgba(45,77,67,.75)',1.3);
            }else if(br.reinforced&&br.hitsRemaining===1){
                c.globalAlpha=.5;this.brickShell(c,br.x+2,br.y+2,br.w-4,br.h-4,'rgba(255,251,231,.92)');
                ...
                c.globalAlpha=.46;this.brickShell(c,br.x+5,br.y+5,br.w-10,br.h-10,null,'rgba(255,255,249,.9)',1.5);
            }
            c.restore();
            if(br.shellPulseAge>=0&&br.shellPulseAge<br.shellPulseDuration){
        ...
        this.drawHazard(c);this.drawEarlyLife(c);this.drawCatchCrownItems(c);this.drawSeedSystem(c);this.drawStageThreeSlow(c);this.drawStageFiveItems(c);this.drawStageSixItems(c);this.drawItemNotices(c);
        for(var f=0;f<this.hitEffects.length;f++){
            ...
            this.brickShell(c,-effect.w*.5,-effect.h*.5,effect.w,effect.h,effect.color);c.shadowColor='transparent';
            ...
            this.brickShell(c,-effect.w*.5+1.5,-effect.h*.5+1.5,effect.w-3,effect.h-3,effectShine);
        ...
        this.drawCatchCrownEquipment(c);
        var b=this.ball;
