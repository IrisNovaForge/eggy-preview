// ============================================================
//  旅人杂货铺 — Cosmetic Shop (SYSTEM PLUGIN)
// ------------------------------------------------------------
...
//    - interior: ctx.api.interior  (enter/isActive/isShop/getKeeperPos/setNearKeeper)
//    - portal:   ctx.api.portal    (confirm/isConfirmOpen/isDismissed/clearDismissed)
//    - ui:       ctx.api.ui        (toast/isWorldMapOpen/setModalOpen)
//  Core -> plugin calls go through registered actions (host.invokeSystem):
//    'openShop' / 'enterShopHouse' / 'buildKeeper'.
//  Engine primitives (THREE/toon/scene) come from ctx (ambient render context).
// ============================================================
(function(){
    'use strict';
    if(!window.DANBO_PLUGIN_HOST||!window.DANBO_PLUGIN_HOST.registerSystem){
        console.warn('[cosmetic-shop] plugin host / registerSystem missing');
        return;
    }
    ...
    var _SHOP_NAME={
        zhs:'\u65C5\u4EBA\u6742\u8D27\u94FA',zht:'\u65C5\u4EBA\u96DC\u8CA8\u8216',
        ja:'\u65C5\u4EBA\u306E\u96D1\u8CA8\u5E97',en:'Traveler\'s General Store'
    };
    var _SHOP_ENTER_DESC={
        zhs:'\u8FDB\u5165\u65C5\u4EBA\u6742\u8D27\u94FA\uFF1F',zht:'\u9032\u5165\u65C5\u4EBA\u96DC\u8CA8\u8216\uFF1F',
        ja:'\u65C5\u4EBA\u306E\u96D1\u8CA8\u5E97\u306B\u5165\u308A\u307E\u3059\u304B\uFF1F',en:'Enter the Traveler\'s General Store?'
    };
    function _shopLang(){
    ...
    function _setNearKeeper(v){ _nearKeeper=!!v; api.interior.setNearKeeper(_nearKeeper); api.publish('nearKeeper',_nearKeeper); }
    ...
    var Cosmetics=(function(){
        ...
        function norm(o){
            ...
            if(typeof o.coins==='number')api.economy.setCoins(o.coins); // restore persistent coins via API
            return o;
        }
        var data=load();
        function save(){ data.coins=api.economy.getCoins(); try{localStorage.setItem(KEY,JSON.stringify(data));}catch(e){} }
        return {
            data:function(){return data;},
            save:save,
            isOwned:function(id){ if(data.owned[id])return true; var ex=_EXTERNAL_OWN[id]; if(ex&&api.progress.isRewardOwned(ex))return true; return false; },
            equipment:function(){return data.equipment;},
            equip:function(cat,id){data.equipment[cat]=id;save();_applyCosmetics();},
            unequip:function(cat){data.equipment[cat]=null;save();_applyCosmetics();},
            buy:function(id){
                var it=_ITEM_BY_ID[id]; if(!it)return false;
                if(Cosmetics.isOwned(id))return true;
                if(!api.economy.spendCoins(it.price))return false; // checks affordability + updates HUD
                data.owned[id]=true; save();
                return true;
            }
        };
    })();
    ...
    // ============================================================
    //  APPLY EQUIPMENT TO THE PLAYER  (via ctx.api.player)
    // ============================================================
    var _cosLastBody=null,_cosSpinGroups=[];
    function _cosBody(){ return api.player.getBody(); }
    ...
    function _spawnFootprint(type){
        var ps=api.player.getState(); if(!ps.exists)return;
        var col,rise=false;
        ...
        m.position.set(ps.x+(Math.random()-0.5)*0.4,0.1,ps.z+(Math.random()-0.5)*0.4);
        if(geo.type==='CircleGeometry')m.rotation.x=-Math.PI/2;
        if(ctx.scene)ctx.scene.add(m);
    ...
    function _updateFootprints(){
        for(var i=_fpParticles.length-1;i>=0;i--){
            ...
            if(fp.life<=0){if(ctx.scene)ctx.scene.remove(fp.mesh);_fpParticles.splice(i,1);continue;}
    ...
    function _coinsNow(){return api.economy.getCoins();}
    function _openShop(){
        if(_shopOpen)return;
        _setShopOpen(true);
        ...
        card.innerHTML=
            ...
            '<div style="display:flex;flex:1;min-height:0;">'+
              '<div id="shop-cats" style="width:96px;background:#EFE1C6;padding:8px 6px;overflow:auto;"></div>'+
    ...
    function _closeShop(){
        ...
        _setShopOpen(false);_shopSel=null;
    ...
    // ============================================================
    //  IN-SHOP KEEPER + BUTTON + PROMPT + PER-FRAME UPDATE
    // ============================================================
    ...
    function _enterShopHouse(){ api.interior.enter({shop:true}); }
    ...
    function _maybeAutoShopConfirm(mode){
        if(_shopOpen||api.ui.isWorldMapOpen())return false;
        if(api.portal.isConfirmOpen())return false;
        var isKeeper=(mode==='keeper'&&api.interior.isActive()&&api.interior.isShop()&&_nearKeeper);
        if(!isKeeper)return false;
        if(api.portal.isDismissed('hidden:shopKeeper:-97'))return false;
        api.portal.confirm({
            name:'\uD83C\uDFEA \u9009\u8D2D',
            desc:'\u548C\u8001\u677F\u9009\u8D2D\u5916\u89C2\uFF1F',
            raceIndex:-1,
            _hiddenType:'shopKeeper',
            _targetStyle:-97
        });
        return true;
    }
    ...
    function _cosUpdate(){
        ...
        _layoutShopButton();
        var _body=api.player.getBody();
        if(_body&&_body!==_cosLastBody){_applyCosmetics(_shopOpen?_shopSelCat():null,_shopOpen?_shopSel:null);}
        for(var i=0;i<_cosSpinGroups.length;i++){if(_cosSpinGroups[i])_cosSpinGroups[i].rotation.y+=0.03;}
        if(++_coinSaveTick>=120){_coinSaveTick=0;if(Cosmetics.data().coins!==api.economy.getCoins())Cosmetics.save();}
        _updateFootprints();
        var ps=api.player.getState();
        var kpos=api.interior.getKeeperPos();
        // INSIDE the shop: approach the keeper to browse
        if(api.interior.isActive()&&api.interior.isShop()&&kpos&&ps.exists){
            var kdx=ps.x-kpos.x,kdz=ps.z-kpos.z;
            _setNearKeeper((kdx*kdx+kdz*kdz)<6.25);
            if(_nearKeeper&&!_shopOpen&&_maybeAutoShopConfirm('keeper'))_showShopPrompt(false);
            else _showShopPrompt(_nearKeeper&&!_shopOpen,'keeper');
        } else { _setNearKeeper(false); if(api.portal.isDismissed('hidden:shopKeeper:-97'))api.portal.clearDismissed(); }
        var sb=document.getElementById('shop-btn');if(sb)sb.style.display=(api.interior.isActive()&&api.interior.isShop()&&_nearKeeper&&!_shopOpen)?'block':'none';
        var inCity=(api.world.isInCity()&&!api.interior.isActive());
        if(!inCity){ if(!_nearKeeper)_showShopPrompt(false); return; }
        // footprints while walking
        var eqfp=Cosmetics.equipment().footprint;
        if(eqfp&&ps.exists&&ps.grounded){
            var sp=Math.abs(ps.vx)+Math.abs(ps.vz);
    ...
    function _showShopPrompt(show,mode){
        ...
        if(show){
            if(api.portal.isConfirmOpen()){if(el)el.style.display='none';return;}
    ...
    // ---- Register CORE->plugin actions (invoked via host.invokeSystem; no globals) ----
    api.registerAction('openShop',_openShop);
    api.registerAction('enterShopHouse',_enterShopHouse);
    api.registerAction('buildKeeper',_buildShopKeeper);

    // E key: browse near the keeper inside the shop. (window.addEventListener is a DOM API,
    // not a game global — allowed. Handler reads only ctx.api, no game globals.)
    function _onShopKey(e){
        if(e.code!=='KeyE'&&e.key!=='e'&&e.key!=='E')return;
        if(_shopOpen)return;
        if(!api.world.isInCity()||api.ui.isWorldMapOpen())return;
        if(api.interior.isActive()){ if(api.interior.isShop()&&_nearKeeper)_openShop(); }
    }
    window.addEventListener('keydown',_onShopKey);
    api.lifecycle.onDestroy(function(){ window.removeEventListener('keydown',_onShopKey); });

    // Initialise mirrored core flags.
    _setShopOpen(false);

    // Drive per-frame logic through the host system dispatch.
    api.lifecycle.onFrame(_cosUpdate);
    return {};
        }
    });
})();
