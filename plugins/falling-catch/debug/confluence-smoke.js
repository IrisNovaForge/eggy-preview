(function(){
    'use strict';
    var report=document.getElementById('report'),lines=[],confluencePhases=[],windPhases=[],combos=[];
    ...
    try{
        ...
        var game=window.DanboFallingCatch.create({mount:document.getElementById('mount'),rules:rules,levels:levels,startLevelId:'starwind-confluence',lang:'en',seed:409,character:{id:'herbTraveler'},targetScore:999,lives:9,onEvent:function(type,payload){if(type==='confluencePhase')confluencePhases.push(payload);if(type==='crosswindPhase')windPhases.push(payload);if(type==='combo')combos.push(payload);}});
