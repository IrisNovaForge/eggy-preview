(function(){
    ...
    function create(options){
        ...
        function drawLeaf(item){
            ...
            context.fillStyle='rgba(45,75,62,.12)';context.beginPath();context.ellipse(.12,.58,3.3,2.7,-.18,0,Math.PI*2);context.fill();
            var leaf=context.createLinearGradient(-2.8,2.2,2.8,-2.2);leaf.addColorStop(0,'#73a96f');leaf.addColorStop(.54,'#a9d59a');leaf.addColorStop(1,'#d8ebb5');context.fillStyle=leaf;
            context.beginPath();context.moveTo(-3.1,1.18);context.bezierCurveTo(-2.36,-1.62,.12,-3.16,3.18,-1.08);context.bezierCurveTo(1.82,1.64,-.66,3.02,-3.1,1.18);context.closePath();context.fill();
            context.strokeStyle='#568a62';context.lineWidth=.42;context.lineCap='round';context.beginPath();context.moveTo(-3.25,1.45);context.quadraticCurveTo(-.55,.28,2.55,-.78);context.stroke();
            context.strokeStyle='rgba(245,255,220,.7)';context.lineWidth=.3;context.beginPath();context.moveTo(-.82,.32);context.quadraticCurveTo(-.55,-.66,.2,-1.38);context.moveTo(.35,-.1);context.quadraticCurveTo(.8,.54,1.46,.78);context.stroke();
            context.fillStyle='rgba(255,255,231,.55)';context.beginPath();context.ellipse(.58,-1.55,.72,.28,-.35,0,Math.PI*2);context.fill();context.restore();
        }
        function drawBerry(item){
            context.save();context.translate(item.x,item.y);context.rotate(item.rotation);context.scale(fallingObjectScale(item.kind),fallingObjectScale(item.kind));
            context.fillStyle='rgba(73,45,58,.12)';context.beginPath();context.ellipse(.1,.62,3.05,2.85,.04,0,Math.PI*2);context.fill();
            var petal=context.createLinearGradient(-2.5,-2.7,2.2,2.8);petal.addColorStop(0,'#efbec5');petal.addColorStop(.5,'#d9879d');petal.addColorStop(1,'#b85f7c');context.fillStyle=petal;
            context.beginPath();context.moveTo(0,3.18);context.bezierCurveTo(-1.52,2.18,-3.08,.5,-2.54,-1.48);context.bezierCurveTo(-2.08,-3.02,-.48,-2.84,.02,-1.5);context.bezierCurveTo(.65,-2.86,2.18,-2.72,2.72,-1.18);context.bezierCurveTo(3.2,.45,1.62,2.32,0,3.18);context.closePath();context.fill();
            context.strokeStyle='rgba(151,65,94,.68)';context.lineWidth=.38;context.lineCap='round';context.beginPath();context.moveTo(0,2.72);context.quadraticCurveTo(-.2,.3,.02,-1.48);context.moveTo(-.16,.78);context.quadraticCurveTo(-1.02,.06,-1.62,-.88);context.moveTo(.12,.45);context.quadraticCurveTo(.84,-.12,1.48,-.94);context.stroke();
            context.fillStyle='rgba(255,239,232,.58)';context.beginPath();context.ellipse(-1.28,-1.32,.52,.78,-.55,0,Math.PI*2);context.fill();context.restore();
        }
        function drawAcorn(item){
            context.save();context.translate(item.x,item.y);context.rotate(item.rotation);context.scale(fallingObjectScale(item.kind),fallingObjectScale(item.kind));
            context.fillStyle='rgba(76,61,35,.13)';context.beginPath();context.ellipse(.15,.5,2.55,3.15,-.18,0,Math.PI*2);context.fill();
            var grain=context.createLinearGradient(-2,-2.8,2.3,2.9);grain.addColorStop(0,'#f3dc8c');grain.addColorStop(.55,'#ddb15d');grain.addColorStop(1,'#bd843d');context.fillStyle=grain;
            context.beginPath();context.moveTo(-.62,-3.18);context.bezierCurveTo(1.08,-2.9,2.62,-1.25,2.48,.52);context.bezierCurveTo(2.32,2.12,.78,2.9,-.85,3.18);context.bezierCurveTo(-2.38,2.1,-2.7,.18,-1.92,-1.42);context.bezierCurveTo(-1.58,-2.12,-1.1,-2.76,-.62,-3.18);context.closePath();context.fill();
            context.fillStyle='#9f7139';context.beginPath();context.moveTo(-.55,-2.92);context.quadraticCurveTo(-1.9,-3.72,-2.8,-2.74);context.quadraticCurveTo(-1.72,-2.34,-.72,-2.1);context.closePath();context.fill();
            context.strokeStyle='rgba(139,92,39,.72)';context.lineWidth=.36;context.lineCap='round';context.beginPath();context.moveTo(-.72,-1.7);context.quadraticCurveTo(-.2,.35,-.45,2.05);context.moveTo(.3,-1.9);context.quadraticCurveTo(.82,-.12,.48,1.7);context.moveTo(1.18,-1.18);context.quadraticCurveTo(1.42,.04,1.05,1.12);context.stroke();
            context.fillStyle='rgba(255,246,187,.58)';context.beginPath();context.ellipse(-1.18,-.78,.42,.82,-.3,0,Math.PI*2);context.fill();context.restore();
        }
        function drawStone(item){
            context.save();context.translate(item.x,item.y);context.rotate(item.rotation);context.scale(fallingObjectScale(item.kind),fallingObjectScale(item.kind));
            context.fillStyle='rgba(34,57,51,.19)';context.beginPath();context.ellipse(.18,.62,3.62,2.9,-.06,0,Math.PI*2);context.fill();
            var rock=context.createLinearGradient(-3,-2.3,3,2.6);rock.addColorStop(0,'#6f7d72');rock.addColorStop(.48,'#56675e');rock.addColorStop(1,'#3f544c');context.fillStyle=rock;
            context.beginPath();context.moveTo(-3.34,.62);context.quadraticCurveTo(-3.08,-1.22,-1.62,-2.32);context.quadraticCurveTo(-.32,-2.94,1.18,-2.42);context.quadraticCurveTo(2.82,-2.02,3.34,-.46);context.quadraticCurveTo(3.5,1.15,2.18,2.38);context.quadraticCurveTo(.62,3.02,-1.28,2.66);context.quadraticCurveTo(-2.92,2.25,-3.34,.62);context.closePath();context.fill();
            context.fillStyle='#788b72';context.beginPath();context.moveTo(-3.02,.4);context.quadraticCurveTo(-2.76,-1.08,-1.48,-1.88);context.quadraticCurveTo(-.78,-1.82,-.48,-1.12);context.quadraticCurveTo(-1.05,-.3,-.82,.72);context.quadraticCurveTo(-1.62,1.52,-2.7,1.42);context.quadraticCurveTo(-3.08,1.02,-3.02,.4);context.closePath();context.fill();
            context.fillStyle='rgba(166,183,137,.62)';context.beginPath();context.arc(-2.32,-.62,.38,0,Math.PI*2);context.arc(-1.8,-1.22,.25,0,Math.PI*2);context.arc(-2.52,.5,.22,0,Math.PI*2);context.fill();
            context.strokeStyle='rgba(194,211,188,.58)';context.lineWidth=.38;context.lineCap='round';context.beginPath();context.moveTo(-.18,-1.28);context.bezierCurveTo(.62,-1.62,1.62,-1.48,2.28,-.92);context.moveTo(.02,-.36);context.bezierCurveTo(.88,-.72,1.9,-.56,2.54,.02);context.moveTo(.08,.58);context.bezierCurveTo(.92,.22,1.82,.38,2.28,.88);context.stroke();
            context.fillStyle='rgba(45,64,56,.62)';context.beginPath();context.ellipse(.92,1.55,.64,.34,-.28,0,Math.PI*2);context.ellipse(2.35,1.1,.32,.2,.12,0,Math.PI*2);context.fill();context.restore();
        }
        function drawReturningBreezeDewlight(item){
            var pulse=.97+Math.sin(performance.now()/270+item.floatPhase)*.04;
            context.save();context.translate(item.x,item.y);context.rotate(Math.sin(item.rotation*.62)*.09);context.scale(pulse,pulse);
            var aura=context.createRadialGradient(-.25,.15,.2,-.1,.1,5.15);aura.addColorStop(0,'rgba(255,252,208,.72)');aura.addColorStop(.45,'rgba(218,248,220,.27)');aura.addColorStop(1,'rgba(184,233,218,0)');context.fillStyle=aura;context.beginPath();context.arc(-.1,.1,5.15,0,Math.PI*2);context.fill();
            context.lineCap='round';context.lineJoin='round';context.strokeStyle='rgba(164,222,201,.55)';context.lineWidth=.58;context.beginPath();context.moveTo(-3.55,.86);context.bezierCurveTo(-2.48,.16,-1.68,.28,-.92,-.18);context.stroke();context.beginPath();context.moveTo(.98,1.55);context.bezierCurveTo(1.82,.92,2.72,1.08,3.5,.38);context.stroke();
            context.strokeStyle='rgba(229,255,225,.72)';context.lineWidth=.46;context.beginPath();context.moveTo(-2.76,-1.02);context.bezierCurveTo(-1.74,-1.5,-.82,-1.22,-.14,-1.58);context.bezierCurveTo(.58,-1.95,1.38,-1.66,2.18,-1.12);context.stroke();
            var dew=context.createLinearGradient(-1.25,-2.35,1.3,2.15);dew.addColorStop(0,'#fffbd5');dew.addColorStop(.48,'#dff0b8');dew.addColorStop(1,'#8fd0af');context.fillStyle=dew;context.beginPath();context.moveTo(-.28,-2.38);context.bezierCurveTo(.92,-1.42,1.62,-.28,1.18,.92);context.bezierCurveTo(.82,2.02,-.58,2.42,-1.28,1.42);context.bezierCurveTo(-1.9,.52,-1.18,-.72,-.28,-2.38);context.closePath();context.fill();
            context.fillStyle='rgba(255,255,239,.76)';context.beginPath();context.ellipse(-.52,-.75,.4,.72,-.48,0,Math.PI*2);context.fill();
            context.strokeStyle='rgba(244,255,222,.8)';context.lineWidth=.42;context.beginPath();context.moveTo(-1.72,2.18);context.bezierCurveTo(-.55,1.5,.68,1.82,1.78,1.12);context.stroke();
            context.fillStyle='rgba(248,255,208,.72)';context.beginPath();context.arc(-2.72,1.68,.24,0,Math.PI*2);context.arc(2.68,-.42,.17,0,Math.PI*2);context.fill();
            context.restore();
        }
        function drawWindSprout(item){
            var stretch=.98+Math.sin(performance.now()/230+item.floatPhase)*.035;
            context.save();context.translate(item.x,item.y);context.rotate(Math.sin(item.rotation*.48)*.1);context.scale(1/stretch,stretch);
            var aura=context.createRadialGradient(.1,-.25,.2,.1,-.25,4.8);aura.addColorStop(0,'rgba(224,255,218,.55)');aura.addColorStop(.52,'rgba(147,224,190,.2)');aura.addColorStop(1,'rgba(106,198,174,0)');context.fillStyle=aura;context.save();context.scale(.72,1.08);context.beginPath();context.arc(.12,-.2,4.6,0,Math.PI*2);context.fill();context.restore();
            var stem=context.createLinearGradient(-1.2,2.9,1,-3.2);stem.addColorStop(0,'#5ca67f');stem.addColorStop(.55,'#8fd19a');stem.addColorStop(1,'#dff2a8');context.strokeStyle=stem;context.lineWidth=1.05;context.lineCap='round';context.beginPath();context.moveTo(-1.12,2.82);context.bezierCurveTo(.48,1.76,-.72,.18,.08,-.92);context.bezierCurveTo(.72,-1.72,.46,-2.48,1.02,-3.08);context.stroke();
            context.fillStyle='#79c492';context.beginPath();context.moveTo(-.04,-1.08);context.bezierCurveTo(-1.12,-3.04,-3.04,-3.18,-3.1,-1.22);context.bezierCurveTo(-2.08,-.52,-.9,-.5,-.04,-1.08);context.closePath();context.fill();
            context.fillStyle='#a9dc9d';context.beginPath();context.moveTo(-.24,.12);context.bezierCurveTo(.78,-1.18,2.62,-1.28,2.78,.22);context.bezierCurveTo(1.8,.86,.68,.72,-.24,.12);context.closePath();context.fill();
            context.strokeStyle='rgba(239,255,210,.72)';context.lineWidth=.34;context.beginPath();context.moveTo(-.1,-1.02);context.quadraticCurveTo(-1.34,-1.82,-2.46,-1.72);context.moveTo(-.18,.16);context.quadraticCurveTo(.88,-.12,2.08,.08);context.stroke();
            context.strokeStyle='rgba(206,250,221,.58)';context.lineWidth=.48;context.beginPath();context.moveTo(-2.66,2.35);context.bezierCurveTo(-1.78,1.88,-1.02,2.02,-.38,1.58);context.moveTo(.72,2.42);context.bezierCurveTo(1.38,1.98,2.06,2.02,2.72,1.48);context.stroke();
            context.fillStyle='rgba(244,255,199,.78)';context.beginPath();context.arc(1.14,-3.2,.25,0,Math.PI*2);context.arc(2.52,1.02,.14,0,Math.PI*2);context.fill();
            context.restore();
        }
        ...
        function draw(){
            ...
            for(var i=0;i<objects.length;i++){
                var item=objects[i];drawAirflowAura(item);if(item.kind==='shell-glimmer')drawReturningBreezeDewlight(item);else if(item.kind==='wind-sprout')drawWindSprout(item);else if(item.kind==='leaf')drawLeaf(item);else if(item.kind==='berry')drawBerry(item);else if(item.kind==='acorn')drawAcorn(item);else drawStone(item);
            }
        ...
        function showTitle(){
            ...
            card.appendChild(makeTitleEmblem('harvest',false));if(text.eyebrow)card.appendChild(make('p','dfc-card-eyebrow',text.eyebrow));card.appendChild(make('h1','',text.title));card.appendChild(make('p','dfc-card-body',text.titleIntro));
        ...
        function showStageTitle(reference,notify){
            ...
            card.appendChild(makeTitleEmblem(currentLevel.titleTheme||currentLevel.mechanics||'base',false));card.appendChild(make('p','dfc-card-eyebrow',format(text.level,{current:currentLevelIndex+1,total:levels.length})));card.appendChild(make('h1','',localized(currentLevel.name)||text.title));
        ...
        function finishRound(status){
            ...
            overlay.classList.remove('dfc-hidden');play(won?'level-clear':'round-fail');
