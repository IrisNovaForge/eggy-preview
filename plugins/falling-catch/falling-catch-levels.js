(function(){
    ...
    var STAGE_ONE_GUIDE=itemGuide([
        guideItem('harvest',{zhs:'自然采样物',zht:'自然採樣物',ja:'自然の採集物',en:'Natural Samples'},{zhs:'青风嫩叶、莓霞花瓣与金穗种子；接住任意一种获得1分。',zht:'青風嫩葉、莓霞花瓣與金穗種子；接住任意一種獲得1分。',ja:'青風の若葉・莓霞の花びら・金穂の種。どれを取っても1点。',en:'Breezeleaves, Berryglow Petals and Goldengrain Seeds; each catch awards 1 point.'}),
        guideItem('stone',{zhs:'风蚀苔岩',zht:'風蝕苔岩',ja:'風蝕の苔岩',en:'Windworn Mossrock'},{zhs:'碰到后失去1次机会。',zht:'碰到後失去1次機會。',ja:'触れるとチャンスを1回失います。',en:'Touching it removes 1 chance.'}),
        guideItem('glimmer',{zhs:'回风露光',zht:'回風露光',ja:'回風の露光',en:'Returning Breeze Dewlight'},{zhs:'机会不足3次时可能出现；接住恢复1次，本关最多出现1次。',zht:'機會不足3次時可能出現；接住恢復1次，本關最多出現1次。',ja:'チャンスが3未満の時に現れることがあります。取ると1回回復し、このステージでは最大1回。',en:'May appear below 3 chances; catch it to restore 1. Up to 1 can appear in this stage.'})
    ]);
    var STAGE_TWO_GUIDE=itemGuide([
        guideItem('airflow-collectibles',{zhs:'轻盈自然物',zht:'輕盈自然物',ja:'軽い自然物',en:'Light Naturals'},{zhs:'青风嫩叶与莓霞花瓣；接住获得1分，会被上升气流托起。',zht:'青風嫩葉與莓霞花瓣；接住獲得1分，會被上升氣流托起。',ja:'青風の若葉と莓霞の花びら。1点になり、上昇気流で持ち上がります。',en:'Breezeleaves and Berryglow Petals award 1 point and can be lifted by the updraft.'}),
        guideItem('seed',{zhs:'金穗种子',zht:'金穗種子',ja:'金穂の種',en:'Goldengrain Seed'},{zhs:'接住获得1分，不受上升气流影响。',zht:'接住獲得1分，不受上升氣流影響。',ja:'取ると1点。上昇気流の影響を受けません。',en:'Catch it for 1 point; it is not affected by the updraft.'}),
        guideItem('stone',{zhs:'风蚀苔岩',zht:'風蝕苔岩',ja:'風蝕の苔岩',en:'Windworn Mossrock'},{zhs:'碰到后失去1次机会。',zht:'碰到後失去1次機會。',ja:'触れるとチャンスを1回失います。',en:'Touching it removes 1 chance.'}),
        guideItem('glimmer',{zhs:'回风露光',zht:'回風露光',ja:'回風の露光',en:'Returning Breeze Dewlight'},{zhs:'机会不足3次时可能出现；接住恢复1次，本关最多出现2次。',zht:'機會不足3次時可能出現；接住恢復1次，本關最多出現2次。',ja:'チャンスが3未満の時に現れることがあります。取ると1回回復し、最大2回。',en:'May appear below 3 chances; catch it to restore 1. Up to 2 can appear in this stage.'})
    ]);
    var STAGE_THREE_GUIDE=itemGuide([
        guideItem('wind-collectibles',{zhs:'风行自然物',zht:'風行自然物',ja:'風流れの自然物',en:'Windborne Naturals'},{zhs:'青风嫩叶、莓霞花瓣与金穗种子均为1分，并会随周期横风产生偏移。',zht:'青風嫩葉、莓霞花瓣與金穗種子均為1分，並會隨週期橫風產生偏移。',ja:'3種の自然物は各1点。周期横風で横へ流されます。',en:'All three natural items award 1 point and drift with the periodic crosswind.'}),
        guideItem('stone',{zhs:'风蚀苔岩',zht:'風蝕苔岩',ja:'風蝕の苔岩',en:'Windworn Mossrock'},{zhs:'碰到失去1次机会，也会随横风偏移。',zht:'碰到失去1次機會，也會隨橫風偏移。',ja:'触れるとチャンスを1回失い、横風でも流されます。',en:'Touching it removes 1 chance; it also drifts with the crosswind.'}),
        guideItem('glimmer',{zhs:'回风露光',zht:'回風露光',ja:'回風の露光',en:'Returning Breeze Dewlight'},{zhs:'机会不足3次时可能出现；接住恢复1次，不受横风影响，本关最多出现2次。',zht:'機會不足3次時可能出現；接住恢復1次，不受橫風影響，本關最多出現2次。',ja:'チャンスが3未満の時に現れ、取ると1回回復。横風の影響を受けず、最大2回。',en:'May appear below 3 chances; restores 1, ignores crosswind, and can appear up to 2 times.'}),
    ...
    var STAGE_FOUR_GUIDE=itemGuide([
        guideItem('combo-collectibles',{zhs:'汇流自然物',zht:'匯流自然物',ja:'合流の自然物',en:'Confluence Naturals'},{zhs:'三种自然物各1分；连续接住3个额外获得1分，并会受到场地气流影响。',zht:'三種自然物各1分；連續接住3個額外獲得1分，並會受到場地氣流影響。',ja:'3種の自然物は各1点。3個連続で追加1点になり、場の気流にも影響されます。',en:'All three natural items award 1 point; every 3-catch chain adds 1 bonus point. Field winds affect them.'}),
        guideItem('stone',{zhs:'风蚀苔岩',zht:'風蝕苔岩',ja:'風蝕の苔岩',en:'Windworn Mossrock'},{zhs:'碰到失去1次机会，并会随汇流横风偏移。',zht:'碰到失去1次機會，並會隨匯流橫風偏移。',ja:'触れるとチャンスを1回失い、合流する横風でも流されます。',en:'Touching it removes 1 chance, and confluence crosswinds shift it sideways.'}),
        guideItem('glimmer',{zhs:'回风露光',zht:'回風露光',ja:'回風の露光',en:'Returning Breeze Dewlight'},{zhs:'机会不足3次时可能出现；接住恢复1次，不受风影响，本关最多出现3次。',zht:'機會不足3次時可能出現；接住恢復1次，不受風影響，本關最多出現3次。',ja:'チャンスが3未満の時に現れ、取ると1回回復。風の影響を受けず、最大3回。',en:'May appear below 3 chances; restores 1, ignores wind, and can appear up to 3 times.'}),
