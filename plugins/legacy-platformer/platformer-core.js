...
var _pfSavedCity=-1;
var _pfTile=4;
// The platformer reuses the race scene container, but it can be the first lazy
// loaded minigame. Create the shared container when the race plugin has not yet
// been loaded so the Exploration Gate always works on a fresh session.
var raceGroup=window.raceGroup;
if(!raceGroup){
    raceGroup=new THREE.Group();
    raceGroup.visible=false;
    window.raceGroup=raceGroup;
    scene.add(raceGroup);
}
...
function _pfStart(){try{
    ...
    if(playerEgg){
        ...
        playerEgg.throwTimer=0;playerEgg._stunTimer=0;
    ...
    playerEgg=createEgg(4,0,ch.color,ch.accent,true,undefined,ch.id);
    ...
    for(var ni=1;ni<total;ni++){
        ...
        var npc=createEgg(2+ni*3,0,nch.color,nch.accent,false,undefined,nch.id);
...
function _pfBuildLevel(){try{
    ...
    // Leaf-spring platforms: flexible forest growths that launch travellers upward.
    var leafSprings=[[10,0],[25,0],[40,0],[53,0]];
    for(var mi=0;mi<leafSprings.length;mi++){
        var mx=leafSprings[mi][0]*T,my=leafSprings[mi][1];
        var spring=new THREE.Group();
        var base=new THREE.Mesh(new THREE.CylinderGeometry(0.42,0.62,1.7,7),toon(0x6F5137,{roughness:0.96}));
        base.position.y=0.85;spring.add(base);
        var bud=new THREE.Mesh(new THREE.SphereGeometry(0.48,8,6),toon(0x9FCB69,{roughness:0.88}));
        bud.scale.set(0.9,0.72,0.9);bud.position.y=1.72;spring.add(bud);
        for(var lp=0;lp<3;lp++){
            var leaf=new THREE.Mesh(new THREE.SphereGeometry(0.78,8,5),toon(lp===1?0x77A957:0x4F8B48,{roughness:0.92}));
            leaf.scale.set(1.35,0.22,0.7);
            leaf.rotation.y=lp*2.18+mi*0.31;
            leaf.rotation.z=(lp-1)*0.14;
            leaf.position.set(Math.cos(leaf.rotation.y)*0.45,1.94+lp*0.08,Math.sin(leaf.rotation.y)*0.32);
            spring.add(leaf);
        }
        spring.position.set(mx,my,0);spring.rotation.y=mi*0.73;raceGroup.add(spring);
        var springCol={x:mx,z:0,hw:1.2,hd:1.2,h:2.5+my,y:my,_leafSpring:true};
        cityColliders.push(springCol);
        window._pfLeafSpringColliders.push(springCol);
    }
    ...
    for(var ei=0;ei<enemyPositions.length;ei++){
        ...
        var enemy=createEgg(ex,0,0xCC3333,0x880000,false,scene,'blossomTraveler');
    ...
    // ================================================================
    //  ZONE 3: CLOUD TRAIL (segments 130-200)
    // ================================================================
    ...
    var bossEnemy=createEgg(bossX1+bossW/2,0,0xFF2222,0x880000,false,scene,'blossomTraveler');
    ...
    // Journey observatory: a neutral destination landmark at the end of the cloud trail.
    // castleX remains an external WASM ABI name; the active runtime uses a neutral local name.
    var destinationX=_pfw?_pfw.castleX(T,L):(L-3)*T;
    var lodge=new THREE.Mesh(new THREE.CylinderGeometry(T*4.6,T*5,T*6,12),toon(0xD1B48A,{roughness:0.92}));
    lodge.position.set(destinationX,T*3,0);raceGroup.add(lodge);
    cityColliders.push({x:destinationX,z:0,hw:T*5,hd:D,h:T*7,y:0});
    var entry=new THREE.Mesh(new THREE.BoxGeometry(T*2,T*3.2,0.7),toon(0x4C6B67,{roughness:0.9}));
    entry.position.set(destinationX,T*1.6,D*0.98);raceGroup.add(entry);
    var dome=new THREE.Mesh(new THREE.SphereGeometry(T*3.7,14,8,0,Math.PI*2,0,Math.PI/2),toon(0x6FA6A0,{roughness:0.72,metalness:0.04}));
    dome.position.set(destinationX,T*6,0);raceGroup.add(dome);
    var compassRing=new THREE.Mesh(new THREE.TorusGeometry(T*1.15,T*0.16,8,20),toon(0xF2D675,{emissive:0x8B6D21,emissiveIntensity:0.22,roughness:0.56}));
    compassRing.rotation.x=Math.PI/2;compassRing.position.set(destinationX,T*8.2,0);raceGroup.add(compassRing);
    var beacon=new THREE.Mesh(new THREE.OctahedronGeometry(T*0.52,0),toon(0xB9F1E5,{emissive:0x4BAA9A,emissiveIntensity:0.5,roughness:0.42}));
    beacon.position.set(destinationX,T*8.2,0);raceGroup.add(beacon);
    // The established goal coordinate is retained; only its visible landmark changes.
    window._pfGoalX=_pfw?_pfw.goalX(T,L):(destinationX-T*3);
...
function _pfUpdateCamera(){try{
    ...
    // Leaf-spring bounce detection
    if(window._pfLeafSpringColliders&&playerEgg&&playerEgg.vy<=0){
        for(var mi=0;mi<window._pfLeafSpringColliders.length;mi++){
            var mc=window._pfLeafSpringColliders[mi];
            var _pfw5=_pfWasm();
            if(_pfw5?_pfw5.leafSpringBounce(playerEgg.mesh.position.x,playerEgg.mesh.position.y,playerEgg.vy,mc.x,mc.h,mc.hw):(DANBO_WASM.absDeltaWithin(playerEgg.mesh.position.x,mc.x,mc.hw+0.5)&&
