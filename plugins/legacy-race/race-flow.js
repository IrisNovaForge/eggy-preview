...
function enterRace(raceIndex){
    ...
    // Save player position for journey-bridge animation
    ...
    // City stays visible during journey-bridge animation — hidden in Phase 3 flash
    ...
    // Save race start positions BEFORE journey bridge moves travelers around
    window._bfRacePositions=[];
    playerEgg=createEgg(0, -2, skin.color, skin.accent, true, undefined, skin.id);
    window._bfRacePositions.push({egg:playerEgg, x:0, y:playerEgg.mesh.position.y, z:-2});
    // Player starts at saved city position for journey-bridge rising animation
    ...
    for(let i=1;i<total;i++){
        ...
        var _rEgg=createEgg(_sx, _sz, AI_COLORS[ci], AI_COLORS[(ci+3)%AI_COLORS.length], false, undefined, CHARACTERS[i%CHARACTERS.length].id);
    ...
    // ---- Journey Bridge transition (3D in Three.js scene) ----
    gameState='raceIntro';
    stopBGM();
    ...
    // Save player position for journey-bridge origin
    ...
    // --- Build 3D journey bridge objects ---
    var _journeyBridgeGroup=new THREE.Group();
    scene.add(_journeyBridgeGroup);
    ...
    for(var _rni=0;_rni<6;_rni++){
        ...
        _journeyBridgeGroup.add(_rune);
        _bfRunes.push(_rune);
    }

    // Light pillars (7 colored cylinders, initially tiny)
    // One pillar per participant — together they form the large journey bridge
    ...
    for(var _bpi=0;_bpi<_bfPillarCount;_bpi++){
        ...
        _journeyBridgeGroup.add(_pillar);
    ...
    function _animateJourneyBridge3D(){
        var elapsed=Date.now()-_bfStart;
        var t=elapsed/_bfDuration;
        if(t>=1){
            // --- Cleanup ---
            if(_bfAnimId)cancelAnimationFrame(_bfAnimId);
            // Remove all journey-bridge meshes
            for(var _ci=_journeyBridgeGroup.children.length-1;_ci>=0;_ci--){
                var _ch=_journeyBridgeGroup.children[_ci];
                if(_ch.geometry)_ch.geometry.dispose();
                if(_ch.material)_ch.material.dispose();
                _journeyBridgeGroup.remove(_ch);
            }
            scene.remove(_journeyBridgeGroup);
        ...
        // --- Phase 3: Reverse Phase 2 then Phase 1 at race track (0.6 - 1.0 = 4s) ---
            ...
            if(!_bfCityHidden){
                ...
                // Move journey bridge to race track center
                ...
                // Only transport the eggs created for this race.  allEggs also
                // contains the hidden city NPCs while the race plugin is active.
                var _bfTransportEggs=window._bfRacePositions||[];
                for(var _se=0;_se<_bfTransportEggs.length;_se++){
                    _bfTransportEggs[_se].egg.mesh.visible=true;
                    _bfTransportEggs[_se].egg.mesh.position.set(0,50+_se*2,0);
                }
            }
        ...
        _bfAnimId=requestAnimationFrame(_animateJourneyBridge3D);
    }
    _animateJourneyBridge3D();
...
function updateRaceHUD(){
    ...
    var routeFindIcon={4:'🌱',6:'❄️',7:'🪁',10:'☁️'}[currentRaceIndex]||'🪙';
    document.getElementById('race-coin-hud').textContent=routeFindIcon+' '+raceCoinScore;
}
