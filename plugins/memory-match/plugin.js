        ...
        create:function(ctx){
            ...
            window.DanboMemoryMatchWasm.load(runtimeBase+'wasm/danbo_memory_match.wasm?'+assetVersion).then(function(wasm){
                ...
                game=window.DanboMemoryMatch.create({
                    ...
                    onResult:function(result){if(ctx.net)ctx.net.send('minigame.finishIntent',{pluginId:ctx.pluginId,status:result.status,level:result.level,attempts:result.attempts,pairs:result.pairs,elapsedSeconds:result.elapsedSeconds,targetSeconds:result.targetSeconds,withinTarget:result.withinTarget});},
