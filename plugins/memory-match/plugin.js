        ...
        create:function(ctx){
            ...
            window.DanboMemoryMatchWasm.load(runtimeBase+'wasm/danbo_memory_match.wasm?'+assetVersion).then(function(wasm){
                ...
                game=window.DanboMemoryMatch.create({
                    mount:ctx.mount,assetBase:runtimeBase,rules:window.DanboMemoryMatchRules.create({wasm:wasm}),pairCount:8,
                    onResult:function(result){if(ctx.net)ctx.net.send('minigame.finishIntent',{pluginId:ctx.pluginId,status:result.status,attempts:result.attempts,pairs:result.pairs});},
