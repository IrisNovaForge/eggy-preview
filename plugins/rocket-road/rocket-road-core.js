...
(function(){
    ...
    DanboRocketRoad.prototype.showScores=function(){
        ...
        var scores=this.getScores();var rows=scores.length?scores.map(function(s,i){return '<div><b>#'+(i+1)+'</b> '+esc(s.name||'Danbo')+' — '+esc(s.score)+' '+esc(rrText('points'))+' <span style="opacity:.75">'+esc(routeName(s.stage||0))+'</span> <span style="opacity:.55">'+esc(s.date||'')+'</span></div>';}).join(''):'<div style="text-align:center;opacity:.75">'+esc(rrText('noScores'))+'</div>';
    ...
    DanboRocketRoad.prototype.saveScore=function(score){var list=this.getScores();list.push({stage:this.stageId||0,stageName:routeName(this.stageId||0),score:score,name:this.ch.displayName||this.ch.name||'Danbo',date:new Date().toISOString().slice(0,10)});list.sort(function(a,b){return b.score-a.score;});list=list.slice(0,12);if(this.ctx.storage)this.ctx.storage.set('rocketRoadScores',list);};
